...
#Region "Standards"
    ...
    Protected Overrides Sub Build_Workfile()
            ...
            For Each OS As String In New String() {"O", "S"}
                If Absx1.chkFor("CHKINCL_" & OS).Checked Then
                    ...
                    If Absx1.optFor("OPTSHOW").Value = "O" Then
                        If OS = "O" Then
                            sql_filter2 &= "" & " and POTORDR1.PO_STATUS = 'O'" & vbCrLf & " and POTORDR2.PO_STATUS = 'O'" & vbCrLf & " and NVL(POTORDR2.PO_QTY_OPN,0) <> 0" & vbCrLf
                        Else
                            sql_filter2 &= "" & " and POTSHIP2.PO_SHIP_STATUS = 'O'" & vbCrLf & " and NVL(POTSHIP3.PO_QTY_REC,0) = 0" & vbCrLf
                        End If
                    End If
        ...
        If Absx1.chkFor("CHKQTYOH").Checked Then
            ...
            S.AppendLine("SELECT")
            S.AppendLine(sql_SELECT_cols)
            S.AppendLine(", '000000' AS PO_ORDER_NO")
            S.AppendLine(", 1 AS PO_ORDER_LNO")
            S.AppendLine(", 'ONHAND' PO_SHIPMENT_NO")
            S.AppendLine(", 0 PO_SHIPMENT_LNO")
            S.AppendLine(", 0 AS PO_QTY_ORD")
            S.AppendLine(", 0 AS PO_QTY_SHP")
            S.AppendLine(", 0 AS PO_QTY_REC")
            S.AppendLine(", 0 AS PO_QTY_OPN")
            ...
            S.AppendLine("FROM ICTSTYL1, ICTSTAT2")
...
#Region "Custom Methods"
        ...
        For Each rowPOTORDRX As DataRow In dst.Tables("POTORDRX").Select()
            ...
            For i As Integer = 1 To maxPeriod
                If (etaDate >= BDate(i - 1) And etaDate <= EDate(i - 1)) Then
                    ...
                    rowPOTORDRX.Item("PO_QTY_OPN" & i) = Val(rowPOTORDRX.Item("PO_QTY_OPN" & i) & "") + Val(rowPOTORDRX.Item("PO_QTY_OPN") & "")
                    'UpdateASTSRPT1(rowPOTORDRX.Item("PO_ORDER_NO"), rowPOTORDRX.Item("PO_ORDER_LNO"), i)
                End If
            Next
        Next
    ...
    Private Function buildDateTitles(maxPeriod As Integer) As List(Of String)
        ...
        If Absx1.chkFor("CHKPO_DATE_ETA_F1").Checked Then
            date1 = "First"
        Else
            date1 = Format(CDate(Absx1.dteFor("PO_DATE_ETA_F1").Value), "MM/dd")
        End If
        date2 = Format(CDate(Absx1.dteFor("PO_DATE_ETA_L1").Value), "MM/dd")
        retVal.Add(date1 & " to " & date2)

        For i As Integer = 2 To maxPeriod
            date1 = Format(CDate(Absx1.dteFor("PO_DATE_ETA_F" & i).Value), "MM/dd")
            If Absx1.chkFor("CHKPO_DATE_ETA_L" & i).Checked Then
                date2 = "Last"
            Else
                date2 = Format(CDate(Absx1.dteFor("PO_DATE_ETA_L" & i).Value), "MM/dd")
            End If
            retVal.Add(date1 & " to " & date2)
        Next
    ...
    Function Get_Dates(TYPE As String, ByVal Period As Integer) As String
        ...
        For Each COLUMN_NAME As String In COL_NAMEs
            ...
            If CHKUSESHIPDATE.Checked And COLUMN_NAME = "PO_SHIP_ETA" Then
                sql = sql & " and A.PO_DATE_SHIPPED >= '" & BPeriod & "'"
                sql = sql & " and A.PO_DATE_SHIPPED <= '" & LPeriod & "'"
            Else
    ...
    Private Sub setETAValues(ByVal Group As Integer)
        ...
        Absx1.dteFor("PO_DATE_ETA_F" & Group).DateTime = priorFirst.AddMonths(1)
...
#End Region
    ...
    Private Function GetBestETA(ByVal PO_ORDER_NO As String, ByVal PO_ORDER_LNO As Integer) As Date
        ...
        If Not IsNothing(rowPOTSHIPX) Then
            If CHKUSESHIPDATE.Checked Then
                If IsDate(rowPOTSHIPX.Item("PO_DATE_SHIPPED").ToString()) Then
                    RetVal = CDate(rowPOTSHIPX.Item("PO_DATE_SHIPPED").ToString())
                End If
            Else
                If IsDate(rowPOTSHIPX.Item("PO_SHIP_ETA").ToString()) Then
                    RetVal = CDate(rowPOTSHIPX.Item("PO_SHIP_ETA").ToString())
                End If
            End If
        End If
        Return RetVal
    ...
    Private Sub AbsCheckBox2_CheckedChanged(sender As Object, e As EventArgs) Handles AbsCheckBox2.CheckedChanged
