...
Public Class POFPACK1
    ...
    ''Dim PO_REFERENCE4 As String = ""
    ''Dim STYLE_CODE_PFX2 As String = ""
    ''Dim STYLE_CODE_PFX3 As String = ""
    ''Dim STYLE_CODE_PFX4 As String = ""
    ''Dim PO_ORDER_NO2 As String = ""
    ''Dim PO_ORDER_NO3 As String = ""
    ''Dim PO_ORDER_NO4 As String = ""

    Dim POREF_NOS As New List(Of String)

    Dim PO_REFERENCE_T(8) As String
    Dim STYLE_CODE_PFX_T(8) As String
    Dim PO_ORDER_NO_T(8) As String
...
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        With dst
            ...
            ASCMAIN1.sql = "Select  POTORDR1.PO_REFERENCE,POTORDR1.PO_ORDER_NO,POTORDR1.PO_STATUS,SUM(PO_QTY_ORD) PO_QTY_ORD,SUM(nvl(PO_QTY_SHP,0)) PO_QTY_SHP,SUM(PO_QTY_OPN) PO_QTY_OPN" & vbCrLf _
            & " from POTORDR1,POTORDR2 where POTORDR1.PO_ORDER_NO = POTORDR2.PO_ORDER_NO" & vbCrLf _
            & " AND VEND_CODE = :PARM1 And POTORDR2.PO_STATUS = 'O' AND POTORDR1.PO_STATUS = 'O'" & vbCrLf _
            & " GROUP BY POTORDR1.PO_REFERENCE,POTORDR1.PO_ORDER_NO,POTORDR1.PO_STATUS"
            ...
            ASCMAIN1.sql = "SELECT PO_REFERENCE, STYLE_CODE,COLOR_CODE,SUM(PO_QTY_ORD) PO_QTY_ORD,SUM(nvl(PO_QTY_SHP,0)) PO_QTY_SHP,SUM(PO_QTY_OPN) PO_QTY_OPN,SUM(PACKED) PACKED, SUM(BOOKED) BOOKED FROM (" & vbCrLf _
               ...
               & " where POTORDR1.VEND_CODE = :PARM1" & vbCrLf _
               & " AND POTPACK1.VBKG_NO Is Null" & vbCrLf _
               & " AND POTPACK1.PACK_LIST_NO = POTPACK3.PACK_LIST_NO" & vbCrLf _
               & " AND POTPACK1.INITIAL_ORDER = '0'" & vbCrLf _
               & " AND POTPACK1.PACK_LIST_STATUS <> 'D'" & vbCrLf _
               & " AND POTPACK1.MANUAL_SHIPMENT IS NULL" & vbCrLf _
            ...
            With .Tables("POTORDSD")
                .Columns.Add("REMAINING", GetType(System.Int32), "PO_QTY_ORD-BOOKED")
                .Columns.Add("NET_OPEN", GetType(System.Int32), "REMAINING-PACKED")
            End With



            ASCMAIN1.sql = "SELECT PO_REFERENCE,STYLE_CODE,COLOR_CODE,PACK_LIST_NO,VBKG_NO,SUM(PO_QTY_ORD) PO_QTY_ORD,SUM(nvl(PO_QTY_SHP,0)) PO_QTY_SHP,SUM(PO_QTY_OPN) PO_QTY_OPN,SUM(PACKED) PACKED, SUM(BOOKED) BOOKED FROM (" & vbCrLf _
        ...
        grdPOTORDSH.DataSource = dst.Tables("POTORDSH")
        grdPOTORDSD.DataSource = dst.Tables("POTORDSD")
        grdPOTORDSS.DataSource = dst.Tables("POTORDSS")
        ...
        Create_Summary(grdPOTORDSH, "PO_REFERENCE", "Count")
        Create_Summary(grdPOTORDSH, New String() {"PO_QTY_ORD", "PO_QTY_SHP", "PO_QTY_OPN"})
            ...
            Case "New"
                ...
                Else
                    ...
                    If INITIAL_ORDER = "1" Then
                        ...
                        ' NEW 
                        POREF_NOS.Clear
                        POREF_NOS.Add(Absx1.txtFor("PO_REFERENCE").Text.Trim)
                        For I = 2 To 8
                            If Absx1.txtFor("PO_REFERENCE" & Format$(I, "0")).Text.Length <> 0 Then
                                PO_REFERENCE_T(I) = Absx1.txtFor("PO_REFERENCE" & Format$(I, "0")).Text
                                If POREF_NOS.Contains(PO_REFERENCE_T(I)) Then
                                    EMsg &= vbCr & $"PO Reference cannot be the same as a prior PO Reference {PO_REFERENCE_T(I)}"
                                    PO_REFERENCE_T(I) = ""
                                Else
                                    POREF_NOS.Add(PO_REFERENCE_T(I).Trim)

                                    ASCMAIN1.sql = "Select * from POTORDR1 where PO_REFERENCE = :PARM1"
                                    Dim row As DataRow = ASCDATA1.GetDataRow(ASCMAIN1.sql, False, "V", New String() {PO_REFERENCE_T(I)})
                                    If row Is Nothing Then
                                        EMsg &= vbCr & $"No record of PO Reference {PO_REFERENCE_T(I)}"
                                    Else
                                        If row.Item("VEND_CODE") & "" <> VEND_CODE Then
                                            EMsg &= vbCr & $"Invalid Vendor for PO Reference {PO_REFERENCE_T(I)}"
                                        ElseIf row.Item("PO_STATUS") & "" <> "O" Then
                                            EMsg &= vbCr & $"PO Reference {PO_REFERENCE_T(I)} is not Open"
                                        Else
                                            PO_ORDER_NO_T(I) = row.Item("PO_ORDER_NO")
                                            STYLE_CODE_PFX_T(I) = row.Item("STYLE_CODE_PFX") & ""
                                        End If
                                    End If
                        ...
                        '''    If (Absx1.txtFor("PO_REFERENCE2").Text.Trim = Absx1.txtFor("PO_REFERENCE").Text.Trim) Or (Absx1.txtFor("PO_REFERENCE2").Text.Trim = Absx1.txtFor("PO_REFERENCE3").Text.Trim) Or (Absx1.txtFor("PO_REFERENCE2").Text.Trim = Absx1.txtFor("PO_REFERENCE4").Text.Trim) Then
                        ...
                        '''    If (Absx1.txtFor("PO_REFERENCE3").Text.Trim = Absx1.txtFor("PO_REFERENCE").Text.Trim) Or (Absx1.txtFor("PO_REFERENCE3").Text.Trim = Absx1.txtFor("PO_REFERENCE2").Text.Trim) Or (Absx1.txtFor("PO_REFERENCE3").Text.Trim = Absx1.txtFor("PO_REFERENCE4").Text.Trim) Then
                        ...
                        '''' LIST OF PO_REFERNECES
                        '''If Absx1.txtFor("PO_REFERENCE4").Text.Length <> 0 Then
                        '''    If (Absx1.txtFor("PO_REFERENCE4").Text.Trim = Absx1.txtFor("PO_REFERENCE").Text.Trim) Or (Absx1.txtFor("PO_REFERENCE4").Text.Trim = Absx1.txtFor("PO_REFERENCE2").Text.Trim) Or (Absx1.txtFor("PO_REFERENCE4").Text.Trim = Absx1.txtFor("PO_REFERENCE3").Text.Trim) Then
                        '''        EMsg &= vbCr & $"PO Reference cannot be the same as prior PO References"
                        '''    Else
                        '''        PO_REFERENCE4 = Absx1.txtFor("PO_REFERENCE4").Text
                        '''        ASCMAIN1.sql = "Select * from POTORDR1 where PO_REFERENCE = :PARM1"
                        '''        Dim row As DataRow = ASCDATA1.GetDataRow(ASCMAIN1.sql, False, "V", New String() {PO_REFERENCE4})
                        '''        If row Is Nothing Then
                        '''            EMsg &= vbCr & $"No record 4th PO Reference {PO_REFERENCE4}"
                        '''        Else
                        '''            If row.Item("VEND_CODE") & "" <> VEND_CODE Then
                        '''                EMsg &= vbCr & $"Invalid 4th PO Reference {PO_REFERENCE4}"
                        '''            ElseIf row.Item("PO_STATUS") & "" <> "O" Then
                        '''                EMsg &= vbCr & $"4th PO Reference {PO_REFERENCE4} is not Open"
                        '''            Else
                        '''                PO_ORDER_NO4 = row.Item("PO_ORDER_NO")
                        '''                STYLE_CODE_PFX4 = row.Item("STYLE_CODE_PFX") & ""
                        '''            End If
                        '''        End If
                        '''    End If
                        '''End If
    ...
    Overrides Sub Mode_Settings(ByVal tf As Boolean, Optional ByVal MODE_description As String = "")
        ...
        If Not ScreenMode Then
            ...
            txtSTYLE_CODE_PFX4.Visible = False
            txtSTYLE_CODE_PFX5.Visible = False
            txtSTYLE_CODE_PFX6.Visible = False
            txtSTYLE_CODE_PFX7.Visible = False
            txtSTYLE_CODE_PFX8.Visible = False
        End If
    ...
    Sub Load_Record()
        ...
        If EntryMode = "N" Then
            ...
            With rowPOTPACK1
                ...
                If INITIAL_ORDER = "1" Then
                    ...
                    For I = 2 To 8
                        PO_REFERENCE_T(I) = HFs("PO_REFERENCE" & Format$(I, "0")) & ""
                        If PO_REFERENCE_T(I) <> "" Then
                    ...
                    '''    STYLE_CODE_PFX3 = row.Item("STYLE_CODE_PFX") & ""
                    '''End If
                    '''PO_REFERENCE4 = HFs("PO_REFERENCE4") & ""
                    '''If PO_REFERENCE4 <> "" Then
                    '''    ASCMAIN1.sql = "Select PO_ORDER_NO, STYLE_CODE_PFX from POTORDR1 where PO_REFERENCE = :PARM1"
                    '''    Dim row As DataRow = ASCDATA1.GetDataRow(ASCMAIN1.sql, False, "V", New String() {PO_REFERENCE4})
                    '''    PO_ORDER_NO4 = row.Item("PO_ORDER_NO") & ""
                    '''    STYLE_CODE_PFX4 = row.Item("STYLE_CODE_PFX") & ""
                ...
                End If
                For I = 2 To 8
                    .Item("STYLE_CODE_PFX" & Format$(I, "0")) = STYLE_CODE_PFX_T(I)
                    .Item("PO_REFERENCE" & Format$(I, "0")) = PO_REFERENCE_T(I)
                    .Item("PO_ORDER_NO" & Format$(I, "0")) = PO_ORDER_NO_T(I)
                Next
                ...
                '''.Item("STYLE_CODE_PFX4") = STYLE_CODE_PFX4
                '''.Item("PO_REFERENCE4") = PO_REFERENCE4
                '''.Item("PO_ORDER_NO4") = PO_ORDER_NO4
        ...
        Else
            ...
            For I = 2 To 8
                PO_REFERENCE_T(I) = rowPOTPACK1.Item("PO_REFERENCE" & Format$(I, "0")) & ""
                STYLE_CODE_PFX_T(I) = rowPOTPACK1.Item("STYLE_CODE_PFX" & Format$(I, "0")) & ""
                PO_ORDER_NO_T(I) = rowPOTPACK1.Item("PO_ORDER_NO" & Format$(I, "0")) & ""
            Next
            ...
            '''PO_REFERENCE4 = rowPOTPACK1.Item("PO_REFERENCE4") & ""
            '''STYLE_CODE_PFX4 = rowPOTPACK1.Item("STYLE_CODE_PFX4") & ""
            '''PO_ORDER_NO4 = rowPOTPACK1.Item("PO_ORDER_NO4") & ""
        ...
        If EntryMode = "N" Then
            ...
            If INITIAL_ORDER = "1" Then
                ...
                For Each COLOR_CODE As String In COLOR_CODEs
                    ...
                    For PO As Integer = 1 To 8
                        If PO = 2 Then
                            If PO_ORDER_NO_T(2) = "" Then Exit For
                            Fill_Records("POTORDRD", PO_ORDER_NO_T(2))
                            PO_ORDER_NO_to_use = PO_ORDER_NO_T(2)
                        End If

                        If PO = 3 Then
                            If PO_ORDER_NO_T(3) = "" Then Exit For
                            Fill_Records("POTORDRD", PO_ORDER_NO_T(3))
                            PO_ORDER_NO_to_use = PO_ORDER_NO_T(3)
                        End If

                        If PO = 4 Then
                            If PO_ORDER_NO_T(4) = "" Then Exit For
                            Fill_Records("POTORDRD", PO_ORDER_NO_T(4))
                            PO_ORDER_NO_to_use = PO_ORDER_NO_T(4)
                        End If

                        If PO = 5 Then
                            If PO_ORDER_NO_T(5) = "" Then Exit For
                            Fill_Records("POTORDRD", PO_ORDER_NO_T(5))
                            PO_ORDER_NO_to_use = PO_ORDER_NO_T(5)
                        End If

                        If PO = 6 Then
                            If PO_ORDER_NO_T(6) = "" Then Exit For
                            Fill_Records("POTORDRD", PO_ORDER_NO_T(6))
                            PO_ORDER_NO_to_use = PO_ORDER_NO_T(6)
                        End If

                        If PO = 7 Then
                            If PO_ORDER_NO_T(7) = "" Then Exit For
                            Fill_Records("POTORDRD", PO_ORDER_NO_T(7))
                            PO_ORDER_NO_to_use = PO_ORDER_NO_T(7)
                        End If

                        If PO = 8 Then
                            If PO_ORDER_NO_T(8) = "" Then Exit For
                            Fill_Records("POTORDRD", PO_ORDER_NO_T(8))
                            PO_ORDER_NO_to_use = PO_ORDER_NO_T(8)
                        End If
        ...
        If EntryMode = "L" Then
            ...
            grdPOTLPNL1.Text = $"LPNs for Packing List {PACK_LIST_NO} - {PACK_LIST_DESC}, PO {PO_REFERENCE}" & IIf(PO_REFERENCE_T(2) = "", "", $", {PO_REFERENCE_T(2)}" & IIf(PO_REFERENCE_T(3) = "", "", $", {PO_REFERENCE_T(3)}" & IIf(PO_REFERENCE_T(4) = "", "", $", {PO_REFERENCE_T(4)}" & IIf(PO_REFERENCE_T(5) = "", "", $", {PO_REFERENCE_T(5)}" & IIf(PO_REFERENCE_T(6) = "", "", $", {PO_REFERENCE_T(6)}" & IIf(PO_REFERENCE_T(7) = "", "", $", {PO_REFERENCE_T(7)}" & IIf(PO_REFERENCE_T(8) = "", "", $", {PO_REFERENCE_T(8)}")))))))
        End If
...
#Region "Popup Menus"
    ...
    Overrides Sub Load_Popup_Menus()
        ...
        Load_Popup_Menu(grdPOTORDSD, "SS", "Show Filter", "Show GroupBox") ', "Confirm/Remove Cartons")
        Load_Popup_Menu(grdPOTORDSS, "BB", "Packing Lists", "Bookings") ', "Confirm/Remove Cartons")
    ...
    Overrides Sub tlb_ToolClick(ByVal sender As System.Object, ByVal e As Infragistics.Win.UltraWinToolbars.ToolClickEventArgs)
        ...
        Select Case e.Tool.Key
            ...
            Case "Packing Lists"
                ...
                End If
            Case "Bookings"
                Dim VBKG_NO As String = grd.ActiveRow.Cells("VBKG_NO").Value & ""
                Dim rowPOTVBKG1 As DataRow = LookUp("POTVBKG1", VBKG_NO)
                If rowPOTVBKG1 IsNot Nothing Then
                    Context_Launch("View", VBKG_NO, e.Tool.Key, "POFVBKG1")
                End If
...
#Region "ABSColumn Controls"
    ...
    Public Overrides Sub txt_Leave(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Select Case Absx1.GetABSColumnName(sender)
            ...
            Case "PO_REFERENCE4"
                Absx1.txtFor("PO_REFERENCE4").Text = Absx1.txtFor("PO_REFERENCE4").Text.ToUpper

            Case "PO_REFERENCE5"
                Absx1.txtFor("PO_REFERENCE5").Text = Absx1.txtFor("PO_REFERENCE5").Text.ToUpper

            Case "PO_REFERENCE6"
                Absx1.txtFor("PO_REFERENCE6").Text = Absx1.txtFor("PO_REFERENCE6").Text.ToUpper

            Case "PO_REFERENCE7"
                Absx1.txtFor("PO_REFERENCE7").Text = Absx1.txtFor("PO_REFERENCE7").Text.ToUpper

            Case "PO_REFERENCE8"
                Absx1.txtFor("PO_REFERENCE8").Text = Absx1.txtFor("PO_REFERENCE8").Text.ToUpper

            Case "STYLE_CODE_PFX"
                Absx1.txtFor("STYLE_CODE_PFX").Text = Absx1.txtFor("STYLE_CODE_PFX").Text.ToUpper

            Case "STYLE_CODE_PFX2"
                Absx1.txtFor("STYLE_CODE_PFX2").Text = Absx1.txtFor("STYLE_CODE_PFX2").Text.ToUpper

            Case "STYLE_CODE_PFX3"
                Absx1.txtFor("STYLE_CODE_PFX3").Text = Absx1.txtFor("STYLE_CODE_PFX3").Text.ToUpper

            Case "STYLE_CODE_PFX4"
                Absx1.txtFor("STYLE_CODE_PFX4").Text = Absx1.txtFor("STYLE_CODE_PFX4").Text.ToUpper

            Case "STYLE_CODE_PFX5"
                Absx1.txtFor("STYLE_CODE_PFX5").Text = Absx1.txtFor("STYLE_CODE_PFX5").Text.ToUpper

            Case "STYLE_CODE_PFX6"
                Absx1.txtFor("STYLE_CODE_PFX6").Text = Absx1.txtFor("STYLE_CODE_PFX6").Text.ToUpper

            Case "STYLE_CODE_PFX7"
                Absx1.txtFor("STYLE_CODE_PFX7").Text = Absx1.txtFor("STYLE_CODE_PFX7").Text.ToUpper

            Case "STYLE_CODE_PFX8"
                Absx1.txtFor("STYLE_CODE_PFX8").Text = Absx1.txtFor("STYLE_CODE_PFX8").Text.ToUpper
    ...
    Sub Check_for_MultiPO()
        ...
        lblPO4.Visible = multi
        lblPO5.Visible = multi
        lblPO6.Visible = multi
        lblPO7.Visible = multi
        lblPO8.Visible = multi

        txtPO_REFERENCE2.Visible = multi
        txtPO_REFERENCE3.Visible = multi
        txtPO_REFERENCE4.Visible = multi
        txtPO_REFERENCE5.Visible = multi
        txtPO_REFERENCE6.Visible = multi
        txtPO_REFERENCE7.Visible = multi
        txtPO_REFERENCE8.Visible = multi
        txtSTYLE_CODE_PFX2.Visible = multi And ScreenMode
        txtSTYLE_CODE_PFX3.Visible = multi And ScreenMode
        txtSTYLE_CODE_PFX4.Visible = multi And ScreenMode
        txtSTYLE_CODE_PFX5.Visible = multi And ScreenMode
        txtSTYLE_CODE_PFX6.Visible = multi And ScreenMode
        txtSTYLE_CODE_PFX7.Visible = multi And ScreenMode
        txtSTYLE_CODE_PFX8.Visible = multi And ScreenMode
        If multi Then
...
#End Region
    ...
    Public Function Produce_XLS(frmASFBASE0 As ASFBASE0, VAN_REF As String) As SpreadsheetGear.IWorkbook
        ...
        For Each rowPOTPACK2 As DataRow In dst.Tables("POTPACK2").Select("", "PACK_LIST_SHEET_NO")
            ...
            worksheet.Cells(6, 9).Value = STYLE_CODE_PFX & IIf(STYLE_CODE_PFX_T(2) = "", "", " & " & STYLE_CODE_PFX_T(2)) & IIf(STYLE_CODE_PFX_T(3) = "", "", " & " & STYLE_CODE_PFX_T(3)) & IIf(STYLE_CODE_PFX_T(4) = "", "", " & " & STYLE_CODE_PFX_T(4)) & IIf(STYLE_CODE_PFX_T(5) = "", "", " & " & STYLE_CODE_PFX_T(5)) & IIf(STYLE_CODE_PFX_T(6) = "", "", " & " & STYLE_CODE_PFX_T(6)) & IIf(STYLE_CODE_PFX_T(7) = "", "", " & " & STYLE_CODE_PFX_T(7)) & IIf(STYLE_CODE_PFX_T(8) = "", "", " & " & STYLE_CODE_PFX_T(8))
            worksheet.Cells(7, 9).Value = PO_REFERENCE & IIf(PO_REFERENCE_T(2) = "", "", " & " & PO_REFERENCE_T(2)) & IIf(PO_REFERENCE_T(3) = "", "", " & " & PO_REFERENCE_T(3)) & IIf(PO_REFERENCE_T(4) = "", "", " & " & PO_REFERENCE_T(4)) & IIf(PO_REFERENCE_T(5) = "", "", " & " & PO_REFERENCE_T(5)) & IIf(PO_REFERENCE_T(6) = "", "", " & " & PO_REFERENCE_T(6)) & IIf(PO_REFERENCE_T(7) = "", "", " & " & PO_REFERENCE_T(7)) & IIf(PO_REFERENCE_T(8) = "", "", " & " & PO_REFERENCE_T(8))
    ...
    Private Sub optPOStatus_ValueChanged(sender As Object, e As EventArgs) Handles optPOStatus.ValueChanged
        ' If Me.SELECTION_NO = 0 Then Exit Sub
        If optPOStatus.Value = "P" Then
            splPOTPACKX.Visible = True
            SplitContainer6.Visible = False
            spl.Panel1Collapsed = False
            optShow.Visible = True
            UltraExplorerBar1.Groups("Screen Control").Visible = True
        Else
            splPOTPACKX.Visible = False
            SplitContainer6.Panel1Collapsed = True
            SplitContainer6.Visible = True
            spl.Panel1Collapsed = True
            optShow.Visible = False
            UltraExplorerBar1.Groups("Screen Control").Visible = False
            VEND_CODE = Absx1.txtFor("VEND_CODE").Text
            Fill_Records("POTORDSH", VEND_CODE)
            Fill_Records("POTORDSD", VEND_CODE)
            Fill_Records("POTORDSS", VEND_CODE)
        End If
        ' Mode_Settings("FALSE")
    End Sub
    Sub Setup_grdPOTORDSS()
        If grdPOTORDSD.ActiveRow Is Nothing OrElse Not grdPOTORDSD.ActiveRow.IsDataRow Then
            grdPOTORDSS.Visible = False
        Else
            Dim PO_REFERENCE As String = grdPOTORDSD.ActiveRow.Cells("PO_REFERENCE").Value & ""
            Dim STYLE_CODE As String = grdPOTORDSD.ActiveRow.Cells("STYLE_CODE").Value & ""
            Dim COLOR_CODE As String = grdPOTORDSD.ActiveRow.Cells("COLOR_CODE").Value & ""

            Dim dvw As DataView = DirectCast(grdPOTORDSS.DataSource, DataTable).DefaultView
            dvw.RowFilter = "PO_REFERENCE = '" & PO_REFERENCE & "' And STYLE_CODE = '" & STYLE_CODE & "'"
    ...
    Private Sub grdPOTORDSD_AfterRowActivate(sender As Object, e As EventArgs) Handles grdPOTORDSD.AfterRowActivate
        Setup_grdPOTORDSS()
    End Sub
