...
Public Class ARFPYMT2
    ...
    Dim rowARTSHOP1 As DataRow
    Dim PAYOUT_ID As String = ""
    Dim SHOPIFY_CUST As String = "SKINCOM"
    Dim shopify_in_process As Boolean
    Dim Global_in_process As Boolean
...
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        With dst
            ...
            If ASCMAIN1.CLIENT = "VAN" Then
                ...
                ASCMAIN1.sql = "Select ARTSHOP1.*" & vbCrLf _
                 & " from ARTSHOP1" & vbCrLf _
                 & " where ARTSHOP1.PAYOUT_ID = :PARM1"
                Create_TDA(.Tables.Add, "ARTSHOP1", "**", 0, False, "V", 1)

                ASCMAIN1.sql = "Select ARTSHOP2.*" & vbCrLf _
                 & " from ARTSHOP2" & vbCrLf _
                 & " where ARTSHOP2.PAYOUT_ID = :PARM1"
                Create_TDA(.Tables.Add, "ARTSHOP2", "**", 0, False, "V", 2)
            ...
            ASCMAIN1.sql = "Select * from TATEVNT1 where TABLE_NAME = 'ARTPYMT1' and TABLE_KEY = :PARM1"
            ...
            'End With
            If ASCMAIN1.CLIENT = "VAN" Then
                ASCMAIN1.sql = "Select ARTSHOP1.* from ARTSHOP1" & vbCrLf _
                & " where NVL(PROCESSED_IND, '0') = '0'" & vbCrLf
                Create_TDA(.Tables.Add, "ARTSHOPX", "**", 0, False, "V", 2)
        ...
        If ASCMAIN1.CLIENT = "VAN" Then
            grdARTSHOPX.DataSource = dst.Tables("ARTSHOPX")
        End If
        ...
        If ASCMAIN1.CLIENT = "VAN" Then
            Create_Summary(grdARTSHOPX, "PAYOUT_ID", "Count")
            Create_Summary(grdARTSHOPX, "AMOUNT")
            Create_Summary(grdARTSHOPX, "ADJUSTMENTS_FEE_AMOUNT")
            Create_Summary(grdARTSHOPX, "ADJUSTMENTS_GROSS_AMOUNT")
            Create_Summary(grdARTSHOPX, "CHARGES_FEE_AMOUNT")
            Create_Summary(grdARTSHOPX, "CHARGES_GROSS_AMOUNT")
            Create_Summary(grdARTSHOPX, "REFUNDS_FEE_AMOUNT")
            Create_Summary(grdARTSHOPX, "REFUNDS_GROSS_AMOUNT")
            Create_Summary(grdARTSHOPX, "RESERVED_FUNDS_FEE_AMOUNT")
            Create_Summary(grdARTSHOPX, "RESERVED_FUNDS_GROSS_AMOUNT")
            Create_Summary(grdARTSHOPX, "RETRIED_PAYOUTS_FEE_AMOUNT")
            Create_Summary(grdARTSHOPX, "RETRIED_PAYOUTS_GROSS_AMOUNT")
        End If
        ...
        If ASCMAIN1.CLIENT = "VAN" Then
            Sort_grdColumns(grdARTSHOPX, "PAYOUT_DATE")
        End If
        ...
        tabMain.Tabs("Shopify").Visible = ASCMAIN1.CLIENT = "VAN"
    ...
    Overrides Sub Proceed_PreReq(ByVal eItemKey As String)
        ...
        Select Case eItemKey
            ...
            Case "Update", "Update 820", "Update Shopify"
            ...
            Case "Delete Shopify"
                MsgBox("Deleting a Shopify Payment is not currently supported", MsgBoxStyle.OkOnly, "")
                Exit Sub
                If grdARTSHOPX.Selected.Rows.Count = 0 Then
                    If grdARTSHOPX.ActiveRow IsNot Nothing Then
                        grdARTSHOPX.ActiveRow.Selected = True
                    End If
                End If
                ...
                Else
                    ...
                    For Each grow As UltraWinGrid.UltraGridRow In grdARTSHOPX.Selected.Rows
                        Dim PAYOUT_ID As String = grow.Cells("PAYOUT_ID").Value
                        If Not ASCMAIN1.Logical_Lock("ARTSHOP1", PAYOUT_ID) Then
                            Exit Sub
                        End If
                    Next

                    If MsgBox("OK to Delete the " & CStr(grdARTSHOPX.Selected.Rows.Count) & " Shopify Records Selected", MsgBoxStyle.YesNo, "Verification to Delete") = MsgBoxResult.No Then
                        ASCMAIN1.MultiTask_Release()
                        Exit Sub
                    End If
                End If
            ...
            Case "Process Shopify"
                ...
                If grdARTSHOPX.Selected.Rows.Count = 0 Then
                    EMsg &= vbCr & "You Must First Select Records to Proceed"

                ElseIf grdARTSHOPX.Selected.Rows.Count > 1 Then
                    EMsg &= vbCr & "You May Process Only 1 Record at a Time"

                ElseIf grdARTSHOPX.ActiveRow Is Nothing OrElse grdARTSHOPX.ActiveRow IsNot grdARTSHOPX.Selected.Rows(0) Then
                    EMsg &= vbCr & "Not Clear on which row to Process"
                ...
                If EMsg = "" Then
                    'Stop
                    PAYOUT_ID = grdARTSHOPX.ActiveRow.Cells("PAYOUT_ID").Value
                    If Not ASCMAIN1.Logical_Lock("ARTSHOP1", PAYOUT_ID) Then
                        Exit Sub
                    End If
                    ...
                    Dim PYMT_BATCH_DATE As Date = grdARTSHOPX.ActiveRow.Cells("PAYOUT_DATE").Value
                    ...
                    If Format(PYMT_BATCH_DATE, "yyyyMMdd") > Format(DTES_LAST, "yyyyMMdd") Then
                        EMsg &= vbCr & "Cannot Process Payout Id " & PAYOUT_ID & " - " & PYMT_BATCH_DATE & ", Not Part Of Current Period"
                    End If
    ...
    Sub Proceed(ByVal eItemKey As String)
        ...
        Select Case eItemKey
            ...
            Case "Cancel", "Done", "Cancel 820", "Cancel Shopify"
            ...
            Case "Process Shopify"
                Call CREATE_ON_ACCOUT_SHO()
                Mode_Settings(False)

                Call APPLY_GIFT_CARD_SHS()
                Mode_Settings(False)

                EntryMode = "E"
                shopify_in_process = True

                rowARTSHOP1 = Fill_Record("ARTSHOP1", PAYOUT_ID)
                EnforceConstraints()

                ' Fill_Records("ARTSHOP1", New String() {PAYOUT_ID})

                Create_ARTCASH1_SHOPIFY()
                ...
                ' auto Cash Apply Shopify
            Case "Upload Global"
                EntryMode = "E"
                Global_in_process = True
                GLOBAL_EXTRACT_EXCEL()
        End Select
    End Sub
        ...
        If UltraExplorerBar1.Groups.Count > 0 Then
            With UltraExplorerBar1
                If Not ScreenMode Or EntryMode = "E" Or EntryMode = "V" Or EntryMode = "X" Then
                    ...
                    With .Groups("Shopify")
                        .Items("Process Shopify").Visible = Not ScreenMode And ASCMAIN1.CLIENT = "VAN"
                        .Items("Delete Shopify").Visible = Not ScreenMode And ASCMAIN1.CLIENT = "VAN"
    ...
    Sub Clear_Record()
        ...
        {"ARTPYMT1", "ARTPYMT2", "ARTPYMT2_SPLIT", "ARTPYMT3",
         "ARTPYMT4", "ARTPYMT5", "ARTOPEN1", "TATEVNT1"}
        ...
        shopify_in_process = False
        Global_in_process = False
    ...
    Sub Update_Record()
        ...
        If shopify_in_process Then
            'rowEDT820T1.Item("EDI_PROCESS_IND") = "1"
            'Update_Record_TDA("EDT820T1")
            ASCMAIN1.sql = "Update ARTSHOP1 set PROCESSED_IND  = '1' where PAYOUT_ID = '" & PAYOUT_ID & "' and NVL(PROCESSED_IND, '0') = '0'"
            ASCDATA1.ExecuteSQL()
...
#Region "Popup Menus"
    ...
    Overrides Sub Load_Popup_Menus()
        ...
        If ASCMAIN1.CLIENT = "VAN" Then
            Load_Popup_Menu(grdARTSHOPX, "SSS", "Show Filter", "Show GroupBox", "Show Pins")
        End If
        Load_Popup_Menu(grdTATEVNT1, "B", "Show Document")
        ...
        Select Case e.SourceControl.Name
            ...
            Case "grdARTPYMT5"
                ...
                If ASCMAIN1.CLIENT = "VAN" Then
                    ...
                    If txtARTCUST1.Text = "AMAZONFBA" Or txtARTCUST1.Text = "AMAZFBA03" Then
...
#End Region
    ...
    Sub Setup_Control_Panel()
        ...
        With UltraExplorerBar1
            ...
            .Groups("Shopify").Visible = (tabMain.SelectedTab.Text = "Shopify")
        End With
...
#End Region
    ...
    ByVal PYMT_BATCH_DATE As Date)
        ...
        If shopify_in_process Or rowARTPYMT5.Item("OUR_REFERENCE") & "" = "SFO" Then
            POST_CODE = "GC"
        End If
...
#Region "EDI (820)"
    ...
    Sub Delete_Shopify()
        ...
        For Each grow As UltraWinGrid.UltraGridRow In grdARTSHOPX.Selected.Rows
            Dim PAYOUT_ID As String = grow.Cells("PAYOUT_ID").Value
            ASCMAIN1.sql = "Update ARTSHOP1 set PROCESSED_IND = 'D' where PAYOUT_ID = '" & PAYOUT_ID & "' and NVL(PROCESSED_IND, '0') = '0'"
            ASCDATA1.ExecuteSQL()
        Next
        CommitTrans()

        ASCMAIN1.MultiTask_Release()
        MsgBox(CStr(grdARTSHOPX.Selected.Rows.Count) & " Records have been Deleted", MsgBoxStyle.OkOnly, "Verification")
    ...
    End Sub
    Sub Load_Shopify_Grid()
        Fill_Records("ARTSHOPX")
    End Sub
#End Region
    ...
    Private Sub cmdApplyXLS_Click(sender As Object, e As EventArgs) Handles cmdApplyXLS.Click
        ...
        Using openFileDialog1 As New OpenFileDialog
            ...
            If ASCMAIN1.CLIENT = "RGI" And (HFs("CUST_CODE") = "031013" Or HFs("CUST_CODE") = "171659") Then
        ...
        Try
            ...
            If FILENAME <> "" Then
                ...
                If ASCMAIN1.CLIENT = "RGI" Then
                    ...
                    If HFs("CUST_CODE") = "110451" Or HFs("CUST_CODE") = "311823" Then
                    ...
                    'QVC.COM (310921) & Q.V.C., INC. (171659).
                    If (HFs("CUST_CODE") = "310921" Or HFs("CUST_CODE") = "171659") Then
                        ...
                        Dim DetailsBegin As Boolean = False
                        'Do While oSheet.Cells(r, 0).Value.ToString.ToUpper & "" <> "TOTALS"
                        Do While BlankCount <= 100
                            If IsNothing(oSheet.Cells(r, 0).Value) Then
                                BlankCount += 1
                            Else
                                If oSheet.Cells(r, 0).Value.ToString.ToUpper & String.Empty = "TOTALS" Then
                                    CHECK_NUM = oSheet.Cells(r + 1, 7).Value & String.Empty
                                    CHECK_AMT = Val(oSheet.Cells(r, 5).Value & String.Empty)
                            ...
                            r = r + 1
                            If DetailsBegin = True Then
                                ' PROCESS FILL invamts array with invoices
                                Dim COL1 As String = ""
                                If Not IsNothing(oSheet.Cells(r, 0).Value) Then
                                    COL1 = oSheet.Cells(r, 0).Value & String.Empty
                                End If
                                If Not (COL1 = "" Or COL1 = "TOTALS") Then
                                    If COL1.Contains("-") Then
                                        Dim p As Int64 = COL1.IndexOf("-")
                                        If p > 0 Then
                                            COL1 = COL1.Substring(0, p)
                                        End If
                                    End If

                                    Dim INV_NO As String = dst.Tables.Item("ARTPYMT3").Select($"INV_CUST_PO = '{COL1}'").FirstOrDefault.Item("INV_NUM").ToString & String.Empty
                                    Dim INV_LINE_AMT As Decimal = oSheet.Cells(r, 5).Value & ""
                    ...
                    'WAYFAIR LLC - See OldWayfairApplyXLS() If Need Be.
                    If HFs("CUST_CODE") = "031013" Then
                        ...
                        For Each line As String In lines
                            ...
                            ElseIf line.Length > 10 AndAlso line.StartsWith("Deduction") Then
                                Dim RecCB As Boolean = True
                                Dim Pmt As Decimal = 0
                                Dim Ref As String = fields(1).Replace(Chr(34), "")
                                If IsNumeric(fields(13)) Then
                                    Pmt = 1 * Val(fields(13))
                                Else
                                    RecCB = False
                                End If
                                If RecCB Then
                                    Dim rowD As DataRow = Record_Chargeback(PYMT_BATCH_DLNO, "ALLOW", Pmt, "Damage (RTN)")
                                    rowD.Item("CHARGEBACK_IND") = "0"
                                End If
                            ...
                            ElseIf line.Length > 20 AndAlso line.Contains(",") Then
                                ...
                                If INV_CUST_PO.Contains(Chr(34)) Then
                                    INV_CUST_PO = INV_CUST_PO.Replace(Chr(34), "")
                                End If
                                ...
                                If INV_PMT > 0 Or INV_PMT < 0 Then ' PAID ITEM - ADDED <0 BECAUSE THERE ARE NEGATIVE INVOICES
                                    If fields.Length >= 15 Then
                                        If IsNumeric(fields(3)) And IsNumeric(fields(13)) Then
                                            ALLOW = ALLOW + (Val(fields(3)) - Val(fields(13)))
                                        End If
                                    End If

                                    'If INV_CUST_PO = "CS629865358" Then
                                    '    Stop
                                    'End If

                                    Dim Sql As String = "INV_CUST_PO = '" & INV_CUST_PO & "'"
                                    Dim rows() As DataRow = dst.Tables("ARTPYMT3").Select(Sql)
                                    ...
                                    If rows.Length = 1 Then
                                        ...
                                        Dim INV_TOTAL_AMT As Decimal = Val(rowARTPYMT3.Item("INV_TOTAL_AMOUNT_CURR") & "")

                                        rowARTPYMT3.Item("INV_PMT") = Val(rowARTPYMT3.Item("INV_PMT").ToString & String.Empty) + INV_PMT
                                        rowARTPYMT3.Item("INV_PMT_CURR") = Val(rowARTPYMT3.Item("INV_PMT_CURR").ToString & String.Empty) + INV_PMT
                                        rowARTPYMT3.Item("INV_WRITE_OFF") = 0
                                        rowARTPYMT3.Item("INV_BALANCE_NEW") = Val(rowARTPYMT3.Item("INV_BALANCE_NEW") & "") - INV_PMT
                                        rowARTPYMT3.Item("INV_DISC_TAKEN_CURR") = 0
                                        rowARTPYMT3.Item("INV_DISC_TAKEN") = 0
                                        rowARTPYMT3.Item("INV_WRITE_OFF_CURR") = 0
                        ...
                        Dim row As DataRow = Record_Chargeback(PYMT_BATCH_DLNO + 1, "ALLOW", ALLOW * -1, "Allowance (5%)")
                        row.Item("CHARGEBACK_IND") = "0"
                    End If
                Else
                    'INT
    ...
    Function Record_Chargeback(
        ...
        Return rowARTPYMT5
    End Function
    Function Record_Chargeback_SHO(
        ByRef PYMT_BATCH_DLNO As Int32,
        REASON_CODE As String,
        INV_PMT As Decimal,
        CUST_REFERENCE As String,
    ...
    Private Sub grdEDTXREF1_AfterCellUpdate(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.CellEventArgs) Handles grdEDTXREF1.AfterCellUpdate
        ...
        End Select
    End Sub
    Sub Create_ARTCASH1_SHOPIFY()


        ' Fill_Records("ARTSHOP1", New String() {PAYOUT_ID})

        '      Dim rowARTSHOP1 As DataRow = dst.Tables("ARTSHOP1").Select("")
        'For Each rowARTSHOP1 As DataRow In dst.Tables("ARTSHOP1").Select("")
        ...
        If dst.Tables("ARTPYMT1").Select("").Length <> 0 _
            ...
            Or dst.Tables("ARTPYMT4").Select("").Length <> 0 _
            Or dst.Tables("ARTPYMT5").Select("").Length <> 0 _
            Or dst.Tables("ARTOPEN1").Select("").Length <> 0 Then Stop
        ...
        With rowARTPYMT1
            ...
            .Item("BANK_CODE") = BANK_CODE
            .Item("CURR_CODE") = rowARTSHOP1.Item("CURRENCY") & ""
            If .Item("CURR_CODE") & "" = "" Then
                .Item("CURR_CODE") = ROWs("GLTPARM1").Item("GL_PARM_CURR_CODE")
            End If
            If .Item("CURR_CODE") = ROWs("GLTPARM1").Item("GL_PARM_CURR_CODE") Then
            ...
            .Item("PYMT_BATCH_DATE") = CDate(rowARTSHOP1.Item("PAYOUT_DATE") & "").Date
            .Item("STATUS") = "1"
            .Item("INIT_OPER") = ASCMAIN1.USER_ID
            .Item("INIT_DATE") = DATETIME_STAMP
            .Item("PYMT_SOURCE") = "SFY"
            .Item("OPS_YYYYPP") = ASCMAIN1.CYP
            ' .Item("EDI_DOC_SEQ_NO") = EDI_DOC_SEQ_NO
        End With
        dst.Tables("ARTPYMT1").Rows.Add(rowARTPYMT1)
        ...
        With rowARTPYMT2
            ...
            .Item("CUST_CODE") = SHOPIFY_CUST
            Dim rowARTCUST1 As DataRow = LookUp("ARTCUST1", SHOPIFY_CUST)
            .Item("CUST_NAME") = rowARTCUST1.Item("CUST_NAME")
            .Item("CUST_PYMT_REF_NO") = rowARTSHOP1.Item("SHOP_CTL_NO")
            .Item("CUST_PYMT_REF_DATE") = rowARTSHOP1.Item("PAYOUT_DATE")
            .Item("CUST_PYMT_AMT") = rowARTSHOP1.Item("AMOUNT")
            .Item("CUST_PYMT_AMT_CURR") = rowARTSHOP1.Item("AMOUNT")
            .Item("PYMT_STATUS") = "D"
            .Item("PYMT_NOTE") = "Shopify Payment Receipt Record"
            .Item("CUST_PYMT_ROUTING_NO") = ""
            .Item("CUST_PYMT_BANK_ACCT_NO") = ""
            .Item("EDI_DOC_SEQ_NO") = ""
    ...
    End Sub
    Sub Create_ARTCASH1_GLOBAL(PAYOUT_AMT As Double, PAYOUT_FILE As String)
        ...
        Dim PAYOUT_DATE As Date = CDate(Now().ToShortDateString)
        ...
        With rowARTPYMT1
            ...
            .Item("PYMT_BATCH_DATE") = PAYOUT_DATE
            .Item("STATUS") = "1"
            .Item("INIT_OPER") = ASCMAIN1.USER_ID
            .Item("INIT_DATE") = DATETIME_STAMP
            .Item("PYMT_SOURCE") = "GLB"
            .Item("OPS_YYYYPP") = ASCMAIN1.CYP
            ' .Item("EDI_DOC_SEQ_NO") = EDI_DOC_SEQ_NO
        End With
        dst.Tables("ARTPYMT1").Rows.Add(rowARTPYMT1)
        ...
        With rowARTPYMT2
            ...
            Dim rowARTCUST1 As DataRow = LookUp("ARTCUST1", SHOPIFY_CUST)
            .Item("CUST_NAME") = rowARTCUST1.Item("CUST_NAME")
            .Item("CUST_PYMT_REF_NO") = "GLOBAL"
            .Item("CUST_PYMT_REF_DATE") = PAYOUT_DATE
            .Item("CUST_PYMT_AMT") = PAYOUT_AMT
            .Item("CUST_PYMT_AMT_CURR") = PAYOUT_AMT
            .Item("PYMT_STATUS") = "D"
            .Item("PYMT_NOTE") = PAYOUT_FILE
            .Item("CUST_PYMT_ROUTING_NO") = ""
            .Item("CUST_PYMT_BANK_ACCT_NO") = ""
            .Item("EDI_DOC_SEQ_NO") = ""
            .Item("PAYOUT_ID") = ""
        ...
        CommitTrans()
    End Sub
    Function SHOPIFY_Process() As Boolean

        Me.Cursor = Cursors.WaitCursor
        ASCMAIN1.Progress("Now Processing Shopify", "")
        ...
        Dim ctrShop2 As Int64 = 0
        Dim ctrShop2_matched As Int64 = 0

        Dim PAYOUT_DATE As String = rowARTSHOP1.Item("PAYOUT_DATE")
        '  Dim EDI_TP_ID As String = rowEDT820T1.Item("EDI_TP_ID")


        '  Fill_Records("ARTSHOP1", New String() {PAYOUT_ID})
        Fill_Records("ARTSHOP2", New String() {PAYOUT_ID})
        ...
        Dim PYMT_BATCH_DLNO_ctr As Integer = 0
        Dim PYMT_BATCH_ILNO As Integer = 0
        Dim FEE_TOTAL As Decimal = 0
        Dim GC_FEE_TOTAL As Decimal = 0
        ...
        For Each rowARTSHOP1 As DataRow In dst.Tables("ARTSHOP1").Select("")
            ...
            For Each rowARTSHOP2 As DataRow In dst.Tables("ARTSHOP2").Select("")
                '.Select("EDI_ENT_NO = " & CStr(EDI_ENT_NO))

                Dim TRANS_TYPE As String = rowARTSHOP2.Item("TRANS_TYPE") & ""

                If TRANS_TYPE = "payout" Then
                Else
                    Dim AMT_NET_DUE As Decimal = Val(rowARTSHOP2.Item("NET") & "")
                    ...
                    ''If INV_NUM = "0008202770" Then
                    ''    Stop
                    ''End If

                    Dim rowARTPYMT3s() As DataRow

                    Dim SQLC As String = "(INV_TYPE = 'I' OR INV_TYPE = 'R') and INV_NO_CONS = '" & INV_NUM & "'"
                    rowARTPYMT3s = dst.Tables("ARTPYMT3").Select(SQLC)
                    ...
                    If rowARTPYMT3s.Length = 0 Then
                        ...
                        If row IsNot Nothing Then
                            ...
                            rowARTPYMT3s = New DataRow() {rowARTPYMT3}
                        Else
                            SQLC = "(INV_TYPE = 'I' OR INV_TYPE = 'R') and INV_NUM = '" & INV_NUM & "'"
                            rowARTPYMT3s = dst.Tables("ARTPYMT3").Select(SQLC)
                            If rowARTPYMT3s.Length = 0 Then
                                SQLC = "(INV_TYPE = 'I' OR INV_TYPE = 'R') and INV_NO_CONS = '" & INV_NUM & "'"
                                rowARTPYMT3s = dst.Tables("ARTPYMT3").Select(SQLC)
                            End If
                            If rowARTPYMT3s.Length = 0 Then
                                SQLC = "(INV_TYPE = 'I' OR INV_TYPE = 'R') and PARTNER_ORDR_NO = '" & INV_NUM & "'"
                                rowARTPYMT3s = dst.Tables("ARTPYMT3").Select(SQLC)
                            End If
                            If rowARTPYMT3s.Length = 0 Then
                                SQLC = "INV_TYPE = 'B' and INV_CUST_PO LIKE '*" & INV_NUM & "'"
                    ...
                    If rowARTPYMT3s.Length = 0 Then
                        ...
                        If TRANS_TYPE = "charge" And SHOPIFY_INVOICE_NO = "" Then
                            ...
                            Dim TOTAL_PER_ORDER As Decimal = 0
                            ...
                            For Each rowSOTGIFTC As DataRow In ASCDATA1.GetDataTable.Rows
                                ...
                                GC_FEE_TOTAL = GC_FEE_TOTAL + FEE_GC
                                TOTAL_PER_ORDER = TOTAL_PER_ORDER + GC_AMOUNT

                            Next
                            ''If TOTAL_PER_ORDER <> AMT_GROSS Then
                            ''    MsgBox("Gift Card Amount for Ordr Id " & ORDR_WEB_ID & " does not equal Payment Gift Card Amount", MsgBoxStyle.OkOnly, "Cannot Apply Gift Cards -  Hit Cancel and Call ABS")
                            ''    Exit Function
                            ''End If
            ...
            If GC_FEE_TOTAL <> 0 Then
                ...
                With rowARTPYMT5
                    ...
                    If rowARTREAS1 IsNot Nothing Then .Item("REASON_DESC") = rowARTREAS1.Item("REASON_DESC")
                    .Item("ACCT_CODE") = DBNull.Value
                    .Item("GL_DIST_AMT") = GC_FEE_TOTAL * CURR_EXCH_RATE
                    .Item("GL_DIST_COMMENT") = ""
                    .Item("CHARGEBACK_IND") = "0"
                    .Item("CHARGEBACK_NO") = DBNull.Value
                    .Item("CUST_REFERENCE") = ""
                    ...
                    .Item("GL_DIST_AMT_CURR") = GC_FEE_TOTAL * CURR_EXCH_RATE
            ...
            If FEE_TOTAL <> 0 Then
                ...
                With rowARTPYMT5
                    ...
                    .Item("PYMT_BATCH_LNO") = PYMT_BATCH_LNO
                    .Item("PYMT_BATCH_DLNO") = PYMT_BATCH_DLNO_ctr
                    .Item("REASON_CODE") = "SHOPF"
                    rowARTREAS1 = dst.Tables("ARTREAS1").Rows.Find(.Item("REASON_CODE"))
                    If rowARTREAS1 IsNot Nothing Then .Item("REASON_DESC") = rowARTREAS1.Item("REASON_DESC")
                    .Item("ACCT_CODE") = DBNull.Value
                    .Item("GL_DIST_AMT") = FEE_TOTAL * CURR_EXCH_RATE
                    .Item("GL_DIST_COMMENT") = "Shopify Fee"
                    .Item("CHARGEBACK_IND") = "0"
                    .Item("CHARGEBACK_NO") = DBNull.Value
                    .Item("CUST_REFERENCE") = "Shopify Fee"
        ...
        MsgBox("Auto-Application of Shopify Remittance has Completed" _
               & vbCrLf & vbCrLf & CStr(ctrShop2) & " A/R Items Indicated on Remittance, " _
               & CStr(ctrShop2_nogo) & " Items Not Matched" _
               & vbCrLf & CStr(ctrShop2_matched) & " Deductions Indicated in Remittance Advice, " _
               & vbCrLf & vbCrLf & zMSG, vbOKOnly, "Verification")

        '       & CStr(ctrShop2_matched) & " Matched against Credits Anticipating Deduction" _
    ...
    Private Sub grdARTSHOPX_DoubleClickRow(sender As Object, e As DoubleClickRowEventArgs) Handles grdARTSHOPX.DoubleClickRow
        If Not ScreenMode And grdARTSHOPX.ActiveRow IsNot Nothing Then Click_Command("Process Shopify")

    End Sub
    Sub GLOBAL_EXTRACT_EXCEL()
        If dst.Tables("ARTPYMT5").Select("").Length <> 0 Or dst.Tables("ARTPYMT4").Select("").Length <> 0 Or dst.Tables("ARTPYMT3").Select("INV_PMT<> 0").Length <> 0 Then
            MsgBox("This function is only applicable at the start of Payment Application", MsgBoxStyle.OkOnly, "Cannot Apply XLS if application has been started")
            Exit Sub
        End If
        ...
        Dim FILENAME As String = ""
        Using openFileDialog1 As New OpenFileDialog
            openFileDialog1.Title = "Select a Global Excel Spreadsheet to Import"
            Dim filter As String = "xlsx files (*.xlsx)|*.xlsx|xls files (*.xls)|*.xls"
        ...
        Try
            ...
            If FILENAME <> "" Then
                ...
                If Global_in_process Then
                    Dim r As Integer = 1 ' NO HEADING

                    ' create payments 1st

                    '   Dim PAYOUT_FILE As String = FILENAME.Substring(FILENAME.IndexOf("SettlementFile_") + marker.Length)
                    Dim marker As String = "SettlementFile_"
                    Dim pos As Integer = FILENAME.IndexOf(marker)
                    Dim PAYOUT_FILE As String = "GE SF -" & FILENAME.Substring(pos + marker.Length)
                    Dim PAYOUT_AMT As Double = 0
                    ' PAYOUT_FILE = PAYOUT_FILE.Substring(0, 60)

                    '       1ST PASS TO GET AMOUNT FOR PYMT1 & PYMT 2
                    Do While oSheet.Cells(r, 0).Value & "" <> ""
                        '   Math.Round(INV_PMT, 2)
                        PAYOUT_AMT = PAYOUT_AMT + Math.Round(Val(Trim(oSheet.Cells(r, 10).Value & "")), 2)
                        r = r + 1
                    Loop

                    If MsgBox("OK to Generate Payment Application for Global, Payment " & Format(PAYOUT_AMT, "$###,##0.00") & "", MsgBoxStyle.YesNo, "Verification to Generate") = MsgBoxResult.No Then
                        ASCMAIN1.MultiTask_Release()
                        Exit Sub
                    End If
                    ...
                    Create_ARTCASH1_GLOBAL(PAYOUT_AMT, PAYOUT_FILE)
                    ...
                    Dim SERVICE_FEE As Double = 0
                    Dim RECONCILE_FEE As Decimal = 0
                    Dim SERVICE_FEE_ADJ As Decimal = 0
                    Dim DUTYTAXES_PAID_CUST As Decimal = 0
                    Dim SHIP_PAID_CUST As Decimal = 0
                    Dim HANDLING_FEE As Decimal = 0 ' NOT USED YET
                    Dim SHIP_CHARGES_MERCH As Decimal = 0
                    Dim DUTYTAXES_MERCH As Decimal = 0
                    Dim RET_SHP_CHARGES_MERCH As Decimal = 0 ' NOT USED YET
                    Dim PYMT_BATCH_DLNO_ctr As Integer = 0
                    Dim INTER_DIFF_TOT As Decimal = 0
                    Dim GIFT_CARD As Decimal = 0
                    ...
                    Do While oSheet.Cells(r, 0).Value & "" <> ""
                        ...
                        ASCMAIN1.Progress("-", GLOBAL_INVOICE_NO)


                        '   Math.Round(INV_PMT, 2)
                        Dim INTER_DIFF_CURR As Decimal = 0

                        SERVICE_FEE = SERVICE_FEE + Val(Trim(oSheet.Cells(r, 17).Value & ""))
                        RECONCILE_FEE = RECONCILE_FEE + Val(Trim(oSheet.Cells(r, 18).Value & ""))
                        SERVICE_FEE_ADJ = SERVICE_FEE_ADJ + Val(Trim(oSheet.Cells(r, 19).Value & ""))
                        DUTYTAXES_PAID_CUST = DUTYTAXES_PAID_CUST + Val(Trim(oSheet.Cells(r, 13).Value & ""))
                        SHIP_PAID_CUST = SHIP_PAID_CUST + Val(Trim(oSheet.Cells(r, 12).Value & ""))
                        HANDLING_FEE = HANDLING_FEE + Val(Trim(oSheet.Cells(r, 22).Value & "")) ' NOT USED YET
                        SHIP_CHARGES_MERCH = SHIP_CHARGES_MERCH + Val(Trim(oSheet.Cells(r, 20).Value & ""))
                        DUTYTAXES_MERCH = DUTYTAXES_MERCH + Val(Trim(oSheet.Cells(r, 23).Value & ""))
                        ...
                        If TRANS_TYPE = "Refund" Or TRANS_TYPE = "Adjustment" Then
                        ...
                        If rowARTPYMT3s.Length = 0 Then
                            Dim row As DataRow = dst.Tables("EDTINVC1").Rows.Find(INV_CUST_PO)
                        ...
                        If rowARTPYMT3s.Length = 0 Then
                            Dim Chargeback As String = "0"
                            Dim GL_COMMENT As String = ""
                            Dim SHOP_REASON_CODE As String = "GEREF"
                            ctrShop2_nogo = ctrShop2_nogo + 1

                            If TRANS_TYPE = "Adjustment" Then
                                MsgBox("The adjustment for Order Number " & INV_CUST_PO & " has been applied to GEREF – Global Refunds based on the Global Excel file.", vbOKOnly, "Verification")
                            End If
                            ...
                            With rowARTPYMT5
                                ...
                                .Item("ACCT_CODE") = DBNull.Value
                                .Item("GL_DIST_AMT") = INV_PMT * CURR_EXCH_RATE * -1
                                .Item("GL_DIST_COMMENT") = INV_CUST_PO
                                .Item("CHARGEBACK_IND") = Chargeback
                                .Item("CHARGEBACK_NO") = DBNull.Value
                                .Item("CUST_REFERENCE") = INV_CUST_PO
                                ...
                                .Item("SEG4_CODE") = ROWs("GLTPARM1").Item("GL_PARM_DEF_SEG4")
                                .Item("INV_TYPE_CB") = DBNull.Value
                                .Item("OUR_REFERENCE") = INV_CUST_PO
                                .Item("GL_DIST_AMT_CURR") = INV_PMT * -1
                            End With
                            dst.Tables("ARTPYMT5").Rows.Add(rowARTPYMT5)
                        ...
                        Else
                            ...
                            For Each rowARTPYMT3 As DataRow In rowARTPYMT3s
                                ...
                                With rowARTPYMT3
                                    ...
                                    End If
                                    If rowARTPYMT3s.Length > 1 Then
                                        If (INV_PMT_invoice - INV_BALANCE) <> 0 And INV_BALANCE <> 0 And Math.Abs(INV_PMT_invoice - INV_BALANCE) < 1 Then
                                    ...
                                    If INV_PMT_invoice > INV_BALANCE Then
                                        ...
                                        '   INV_WRITE_OFF = -1 * (INV_PMT_invoice - INV_BALANCE)
                                    End If

                                    If INV_PMT_invoice <> INV_BALANCE Then
                                        'Dim diff As Double = Math.Abs(INV_PMT_invoice - INV_BALANCE)
                                        'If diff > 5 Then
                                        '    If MsgBox("Offset is greater than $5, Payment >> " & CStr(INV_PMT_invoice) & " Open A/R >> " & CStr(INV_BALANCE), MsgBoxStyle.YesNo, "Verification to Continue") = MsgBoxResult.No Then
                                        '        ASCMAIN1.MultiTask_Release()
                                        '        Exit Sub
                                        '    End If
                                        'End If
                                        INTER_DIFF_CURR = -1 * (INV_PMT_invoice - INV_BALANCE)
                                        INV_PMT_invoice = INV_BALANCE
                                    ...
                                    .Item("INV_WRITE_OFF_CURR") = INV_WRITE_OFF
                                    .Item("INV_BALANCE_NEW_CURR") = Val(.Item("INV_BALANCE_NEW_CURR") & "") - (INV_PMT_invoice + INV_WRITE_OFF)
                                    INTER_DIFF_TOT = INTER_DIFF_TOT + INTER_DIFF_CURR
                    ...
                    ' CALCULATE Deductions From Totals

                    If SERVICE_FEE <> 0 Then
                        PYMT_BATCH_DLNO_ctr = PYMT_BATCH_DLNO_ctr + 1
                        WRITE_GLOBAL_DEDUCTION("SERVICE_FEE", SERVICE_FEE * -1, PYMT_BATCH_DLNO_ctr, CURR_EXCH_RATE)
                        ctrShop2_matched = +1
                    End If
                    If RECONCILE_FEE <> 0 Then
                        PYMT_BATCH_DLNO_ctr = PYMT_BATCH_DLNO_ctr + 1
                        WRITE_GLOBAL_DEDUCTION("RECONCILE_FEE", RECONCILE_FEE * -1, PYMT_BATCH_DLNO_ctr, CURR_EXCH_RATE)
                        ctrShop2_matched = +1
                    End If
                    If SERVICE_FEE_ADJ <> 0 Then
                        PYMT_BATCH_DLNO_ctr = PYMT_BATCH_DLNO_ctr + 1
                        WRITE_GLOBAL_DEDUCTION("SERVICE_FEE_ADJ", SERVICE_FEE_ADJ * -1, PYMT_BATCH_DLNO_ctr, CURR_EXCH_RATE)
                        ctrShop2_matched = +1
                    End If
                    If HANDLING_FEE <> 0 Then
                        PYMT_BATCH_DLNO_ctr = PYMT_BATCH_DLNO_ctr + 1
                        WRITE_GLOBAL_DEDUCTION("HANDLING_FEE", HANDLING_FEE * -1, PYMT_BATCH_DLNO_ctr, CURR_EXCH_RATE)
                        ctrShop2_matched = +1
                    End If
                    If SHIP_CHARGES_MERCH <> 0 Then
                        PYMT_BATCH_DLNO_ctr = PYMT_BATCH_DLNO_ctr + 1
                        WRITE_GLOBAL_DEDUCTION("SHIP_CHARGES_MERCH", SHIP_CHARGES_MERCH * -1, PYMT_BATCH_DLNO_ctr, CURR_EXCH_RATE)
                        ctrShop2_matched = +1
                    End If
                    If DUTYTAXES_MERCH <> 0 Then
                        PYMT_BATCH_DLNO_ctr = PYMT_BATCH_DLNO_ctr + 1
                        WRITE_GLOBAL_DEDUCTION("DUTYTAXES_MERCH", DUTYTAXES_MERCH * -1, PYMT_BATCH_DLNO_ctr, CURR_EXCH_RATE)
                        ctrShop2_matched = +1
                    End If
                    ...
                    If SHIP_PAID_CUST <> 0 Then
                        PYMT_BATCH_DLNO_ctr = PYMT_BATCH_DLNO_ctr + 1
                        WRITE_GLOBAL_DEDUCTION("SHIP_PAID_CUST", SHIP_PAID_CUST, PYMT_BATCH_DLNO_ctr, CURR_EXCH_RATE)
                        ctrShop2_matched = +1
                    End If
                    If DUTYTAXES_PAID_CUST <> 0 Then
                        PYMT_BATCH_DLNO_ctr = PYMT_BATCH_DLNO_ctr + 1
                        WRITE_GLOBAL_DEDUCTION("DUTYTAXES_PAID_CUST", DUTYTAXES_PAID_CUST, PYMT_BATCH_DLNO_ctr, CURR_EXCH_RATE)
                        ctrShop2_matched = +1
                    End If
                    If INTER_DIFF_TOT <> 0 Then
                        PYMT_BATCH_DLNO_ctr = PYMT_BATCH_DLNO_ctr + 1
                        WRITE_GLOBAL_DEDUCTION("INT_OFFSET", INTER_DIFF_TOT, PYMT_BATCH_DLNO_ctr, CURR_EXCH_RATE)
                        ctrShop2_matched = +1
                    End If
                    If GIFT_CARD <> 0 Then
                        PYMT_BATCH_DLNO_ctr = PYMT_BATCH_DLNO_ctr + 1
                        WRITE_GLOBAL_DEDUCTION("GIFT_CARD", GIFT_CARD * -1, PYMT_BATCH_DLNO_ctr, CURR_EXCH_RATE)
                        ctrShop2_matched = +1
                    End If


                Else
                    ' OTher Deductions perhaps
                End If
        ...
        '   TAC.TACMAIN1.Record_Event("ARTPYMT1", PYMT_BATCH_NO, DATETIME_STAMP, ASCMAIN1.USER_ID, "PYMT_BATCH_NO" & EntryMode, "GE Extract Payment")
        Record_Event("New", "Global Pymt Batch: " & PYMT_BATCH_NO, FILENAME)

        Mode_Settings(True)

    End Sub

    Sub WRITE_GLOBAL_DEDUCTION(DEDUCT_TYPE As String, DEDUCT_AMT As Double, PYMT_BATCH_DLNO_ctr As Integer, CURR_EXCH_RATE As Double)
        ...
        If DEDUCT_TYPE = "SERVICE_FEE" Then
            REASONCD = "GESFE"
        ElseIf DEDUCT_TYPE = "RECONCILE_FEE" Then
            REASONCD = "GEREC"
        ElseIf DEDUCT_TYPE = "SERVICE_FEE_ADJ" Then
            REASONCD = "GEADJ"
        ElseIf DEDUCT_TYPE = "HANDLING_FEE" Then
            REASONCD = "GEHAN"
        ElseIf DEDUCT_TYPE = "SHIP_CHARGES_MERCH" Then
            REASONCD = "GESHIP"
        ...
        ElseIf DEDUCT_TYPE = "SHIP_PAID_CUST" Then
            REASONCD = "GESPC"
        ElseIf DEDUCT_TYPE = "DUTYTAXES_PAID_CUST" Then
            REASONCD = "GEDTP"
        ElseIf DEDUCT_TYPE = "INT_OFFSET" Then
            REASONCD = "GEICC"
        ElseIf DEDUCT_TYPE = "GIFT_CARD" Then
            REASONCD = "GEGIF"
        End If
        ...
        With rowARTPYMT5
            ...
            .Item("PYMT_BATCH_LNO") = PYMT_BATCH_LNO
            .Item("PYMT_BATCH_DLNO") = PYMT_BATCH_DLNO_ctr
            .Item("REASON_CODE") = REASONCD
            rowARTREAS1 = dst.Tables("ARTREAS1").Rows.Find(.Item("REASON_CODE"))
            If rowARTREAS1 IsNot Nothing Then .Item("REASON_DESC") = rowARTREAS1.Item("REASON_DESC")
            .Item("ACCT_CODE") = DBNull.Value
            .Item("GL_DIST_AMT") = DEDUCT_AMT * CURR_EXCH_RATE
            .Item("GL_DIST_COMMENT") = DEDUCT_TYPE
            .Item("CHARGEBACK_IND") = "0"
            .Item("CHARGEBACK_NO") = DBNull.Value
            .Item("CUST_REFERENCE") = DEDUCT_TYPE
            ...
            .Item("SEG4_CODE") = ROWs("GLTPARM1").Item("GL_PARM_DEF_SEG4")
            .Item("INV_TYPE_CB") = DBNull.Value
            .Item("OUR_REFERENCE") = DEDUCT_TYPE
            .Item("GL_DIST_AMT_CURR") = DEDUCT_AMT
            .Item("TRANSACTION_LEGEND") = "DR (Expense)"
        End With
        dst.Tables("ARTPYMT5").Rows.Add(rowARTPYMT5)
    ...
    End Sub
    Sub Load_Events()
        Fill_Records("TATEVNT1", PYMT_BATCH_NO)

        ''If EntryMode = "N" Then
        ''    Record_Event("INIT", "Global Pymt Batch Started")
        ''ElseIf EntryMode = "E" Then
        ''    Record_Event("LAST", "Global Pymt Batch Edited")
        ''End If

        ''If ASCMAIN1.CLIENT = "NYA" Then
        ''    ASCMAIN1.sql = "Select * from SOTAUTH1 where ORDR_GROUP_NO = '" & PYMT_BATCH_NO & "'"
        ''    For Each row As DataRow In ASCDATA1.GetDataTable().Select("")
        ''        Dim rowTATEVNT1 As DataRow = dst.Tables("TATEVNT1").NewRow
        ''        With rowTATEVNT1
        ''            .Item("TABLE_NAME") = "ARTPYMT1"
        ''            .Item("TABLE_KEY") = PYMT_BATCH_NO
    ...
    End Sub
    Sub Record_Event(EVENT_TYPE As String, EVENT_DESC As String, FILENAME As String)
        Dim rowTATEVNT1 As DataRow = dst.Tables("TATEVNT1").NewRow
        rowTATEVNT1.Item("TABLE_NAME") = "ARTPYMT1"
        rowTATEVNT1.Item("TABLE_KEY") = PYMT_BATCH_NO
        rowTATEVNT1.Item("INIT_DATE") = DATETIME_STAMP
        rowTATEVNT1.Item("INIT_OPER") = ASCMAIN1.USER_ID
        rowTATEVNT1.Item("EVENT_TYPE") = EVENT_DESC
        rowTATEVNT1.Item("EVENT_DESC") = FILENAME
    ...
    '    '    Next
    'End Sub
    Sub APPLY_GIFT_CARD_SHS()

        Dim INV_NO As String = ""
        Dim INV_NO_OA As String = ""
        Dim PASSED_AMT As Decimal = 0
        ...
        ASCMAIN1.sql = "Select SOTINVHC.* From SOTINVHC, SOTINVH1 Where SOTINVH1.INV_NO = SOTINVHC.INV_NO" _
         & " And   NVL(SOTINVHC.STATUS, '0') = '0'" _
         & " And SOTINVH1.ORDR_YYYYPP_UPDATED = '" & ASCMAIN1.CYP & "'"
        Dim TOTROWS As Integer = ASCDATA1.GetDataTable.Rows.Count
        If TOTROWS = 0 Then
            Exit Sub
        Else
            BeginTrans()
            ASCMAIN1.sql = "Select Max(PYMT_BATCH_NO) from ARTPYMT1 where STATUS = '1' and PYMT_SOURCE = 'SFS' AND OPS_YYYYPP = '" & ASCMAIN1.CYP & "'"
            PYMT_BATCH_NO_CURR = ASCDATA1.GetDataValue
            If PYMT_BATCH_NO_CURR = "" Then
            Else
                rowARTPYMT1 = Fill_Record("ARTPYMT1", PYMT_BATCH_NO_CURR)
                ASCMAIN1.sql = "Select Max(PYMT_BATCH_LNO) from ARTPYMT2 where PYMT_BATCH_NO = '" & PYMT_BATCH_NO_CURR & "'"
                PYMT_BATCH_NO = rowARTPYMT1.Item("PYMT_BATCH_NO")
                PYMT_BATCH_LNO = Val(ASCDATA1.GetDataValue) + 1

                rowARTPYMT1.Item("LAST_OPER") = ASCMAIN1.USER_ID
                rowARTPYMT1.Item("LAST_DATE") = PYMT_BATCH_DATE
            ...
            If NEWBATCH Then
                ...
                rowARTPYMT1 = dst.Tables("ARTPYMT1").NewRow
                With rowARTPYMT1
                    .Item("PYMT_BATCH_NO") = PYMT_BATCH_NO
                    ...
                    .Item("STATUS") = "1"
                    .Item("INIT_OPER") = ASCMAIN1.USER_ID
                    .Item("INIT_DATE") = PYMT_BATCH_DATE
                    .Item("PYMT_SOURCE") = "SFS"
                    .Item("OPS_YYYYPP") = ASCMAIN1.CYP
                    ' .Item("EDI_DOC_SEQ_NO") = EDI_DOC_SEQ_NO
                End With
                dst.Tables("ARTPYMT1").Rows.Add(rowARTPYMT1)
        ...
        rowARTPYMT2 = dst.Tables("ARTPYMT2").NewRow
        With rowARTPYMT2
            .Item("PYMT_BATCH_NO") = PYMT_BATCH_NO
            .Item("PYMT_BATCH_LNO") = PYMT_BATCH_LNO
            ...
            ''         .Item("CUST_PYMT_AMT") = PASSED_AMT
            ''    .Item("CUST_PYMT_AMT_CURR") = PASSED_AMT
            .Item("CUST_PYMT_AMT") = 0
            .Item("CUST_PYMT_AMT_CURR") = 0

            .Item("PYMT_STATUS") = "2"
            .Item("PYMT_NOTE") = "Shopify Apply Gift Card Inv No " & INV_NO
            .Item("CUST_PYMT_ROUTING_NO") = ""
            .Item("CUST_PYMT_BANK_ACCT_NO") = ""
            .Item("EDI_DOC_SEQ_NO") = ""
            .Item("PAYOUT_ID") = ""
            ...
            .Item("INIT_DATE") = PYMT_BATCH_DATE
            .Item("LAST_OPER") = ASCMAIN1.USER_ID
            .Item("LAST_DATE") = PYMT_BATCH_DATE


        End With
        dst.Tables("ARTPYMT2").Rows.Add(rowARTPYMT2)
        '  Determine If New batch Or Not look for OPS_YYYYPP = Curr_period and STATUS = '1' and PYMT_SOURCE = 'SFS'
        ' THEN GRAB THE MAX RECORD PYMT_BATCH_LNO FROM ARTPYMT2 And ADD 1, NEWBATCH = FALSE, ORA TO ACC PYMT1 & PYMT2 

        ASCMAIN1.sql = "Select SOTINVHC.* From SOTINVHC, SOTINVH1 Where SOTINVH1.INV_NO = SOTINVHC.INV_NO" _
         & " And   NVL(SOTINVHC.STATUS, '0') = '0'" _
         & " And SOTINVH1.ORDR_YYYYPP_UPDATED = '" & ASCMAIN1.CYP & "'"
        For Each rowSOTINVHC As DataRow In ASCDATA1.GetDataTable.Rows
            CURR_CODE = rowARTPYMT1.Item("CURR_CODE")
            CURR_EXCH_RATE = rowARTPYMT1.Item("CURR_EXCH_RATE")
            ...
            For Each rowARTOPEN1 As DataRow In dst.Tables("ARTOPEN1").Select("", "INV_DATE,INV_TYPE,INV_NUM")
                ...
                With rowARTPYMT3
                    ...
                    Dim INV_BALANCE As Decimal = Val(.Item("INV_BALANCE") & "")
                    Dim INV_PMT_invoice As Decimal = PASSED_AMT
                    Dim INV_DISC_TAKEN As Decimal = 0
                    Dim INV_WRITE_OFF As Decimal = 0
                    If .Item("INV_TYPE") = "O" Then
                        INV_PMT_invoice = INV_PMT_invoice * -1
                    End If
                ...
                End With
            Next
            ASCMAIN1.sql = "Update SOTINVHC set STATUS = '1' WHERE GIFT_CTL_NO = '" & GIFT_CTL_NO & "' AND INV_TYPE = '" & rowSOTINVHC.Item("INV_TYPE") & "" & "' AND INV_NO = '" & rowSOTINVHC.Item("INV_NO") & "" & "'"
        ...
        PYMT_BATCH_ILNO = 0
        For Each rowARTPYMT3 As DataRow In dst.Tables("ARTPYMT3").Select("", "PYMT_BATCH_ILNO")
            Dim rowARTOPEN1 As DataRow = dst.Tables("ARTOPEN1").Rows.Find(New String() {CUST_CODE, rowARTPYMT3.Item("INV_TYPE"), rowARTPYMT3.Item("INV_NUM")})
            ...
            rowARTOPEN1.Item("INV_WRITE_OFF_CURR") = Val(rowARTOPEN1.Item("INV_WRITE_OFF_CURR") & "") + Val(rowARTPYMT3.Item("INV_WRITE_OFF_CURR") & "")
            rowARTOPEN1.Item("LAST_OPER") = ASCMAIN1.USER_ID
            rowARTOPEN1.Item("LAST_DATE") = PYMT_BATCH_DATE
        Next
        Update_Record_TDA("ARTPYMT3")
        Update_Record_TDA("ARTOPEN1")
        Update_Record_TDA("ARTPYMT2")
    ...
    End Sub
    Sub CREATE_ON_ACCOUT_SHO(INV_NO As String, PASSED_AMT As Decimal)

        BeginTrans()

        Dim SHOPIFY_CUST As String = "SKINCOM"
        Dim INV_NO_TO_CONSIDER As String = "('" & INV_NO & "')"
        Dim CURR_EXCH_RATE As Decimal = 1
        ...
        ''ASCMAIN1.sql = "Select Max(PYMT_BATCH_NO) from ARTPYMT1 where STATUS = '1' and PYMT_SOURCE = 'SFO' AND OPS_YYYYPP = '" & ASCMAIN1.CYP & "'"
        ''PYMT_BATCH_NO_CURR = ASCDATA1.GetDataValue
        ''If PYMT_BATCH_NO_CURR = "" Then
        ''Else
        ''    rowARTPYMT1 = Fill_Record("ARTPYMT1", PYMT_BATCH_NO_CURR)
        ...
        With rowARTPYMT2
            ...
            Dim rowARTCUST1 As DataRow = LookUp("ARTCUST1", SHOPIFY_CUST)
            .Item("CUST_NAME") = rowARTCUST1.Item("CUST_NAME")
            .Item("CUST_PYMT_REF_NO") = INV_NO
            .Item("CUST_PYMT_REF_DATE") = PYMT_BATCH_DATE
            .Item("CUST_PYMT_AMT") = 0
            .Item("CUST_PYMT_AMT_CURR") = 0

            .Item("PYMT_STATUS") = "2"
            .Item("PYMT_NOTE") = "Shopify Payment Receipt Record Auto"
            .Item("CUST_PYMT_ROUTING_NO") = ""
            .Item("CUST_PYMT_BANK_ACCT_NO") = ""
            .Item("EDI_DOC_SEQ_NO") = ""
            .Item("PAYOUT_ID") = ""
        ...
        For Each rowARTOPEN1 As DataRow In dst.Tables("ARTOPEN1").Select("", "INV_DATE,INV_TYPE,INV_NUM")
            ...
            End With
        Next
        ' Update ARTPYMT5 WITH On Account
        Dim GC_AMOUNT As Decimal = PASSED_AMT * -1
        Dim GC_ID As String = "GenOA_GiftCd"
        Dim GC_DESC As String = "Generation of On Account from Gift Card"

        Dim PYMT_BATCH_DLNO_ctr As Integer = 0
        Record_Chargeback_SHO(PYMT_BATCH_DLNO_ctr, "ONACT", GC_AMOUNT, GC_ID, "SFO", GC_DESC)
    ...
    End Sub
    Private Sub btnDGJ_Click(sender As Object, e As EventArgs) Handles btnDGJ.Click
        ''Dim INV_NO As String = "0008313601"
        ''Dim INV_NO_OA As String = "0000872055"
        ''Dim AMT As Decimal = 4

        '0000872056
        'Dim INV_NO As String = "0008194858"
        'Dim INV_NO_OA As String = "0000872056"
        'Dim AMT As Decimal = 5
    ...
    Private Sub btnDGJ2_Click(sender As Object, e As EventArgs) Handles btnDGJ2.Click

        'CREATE_ON_ACCOUT_SHO_BUTTON2()
        '''Stop
        '''Dim INV_NO As String = "0008313610"
        '''Dim AMT As Decimal = -137.2



        '''Call CREATE_ON_ACCOUT_SHO(INV_NO, AMT)
    End Sub
    Sub CREATE_ON_ACCOUT_SHO()


        Dim INV_NO As String
        Dim PASSED_AMT As Decimal
        ...
        ASCMAIN1.sql = "Select SOTGIFTC.*, SOTRTNL1.ORDR_NO, SOTRTNL1.INV_NO, SOTRTNL1.RTRN_NO From SOTGIFTC, SOTRTNL1" _
        & " Where SOTGIFTC.GIFT_CARD_TYPE In ('R')" _
        & " And NVL(SOTGIFTC.GIFT_CARD_PRE, '0') <> '1'" _
        & " And INV_NO_OA Is NULL And SOTGIFTC.RETURN_ID = SOTRTNL1.RETURN_ID"
        Dim TOTROWS As Integer = ASCDATA1.GetDataTable.Rows.Count
        If TOTROWS = 0 Then
            Exit Sub
        Else
            ' Determine If New batch Or Not look for OPS_YYYYPP = Curr_period And STATUS = '1' and PYMT_SOURCE = 'SFO'
            '  THEN GRAB THE MAX RECORD PYMT_BATCH_LNO FROM ARTPYMT2 And ADD 1, NEWBATCH = FALSE, ORA TO ACC PYMT1 & PYMT2 
            BeginTrans()
        ...
        For Each rowSOTGIFTC As DataRow In ASCDATA1.GetDataTable.Rows
            CURR_CODE = rowARTPYMT1.Item("CURR_CODE")
            CURR_EXCH_RATE = rowARTPYMT1.Item("CURR_EXCH_RATE")
            INV_NO = rowSOTGIFTC.Item("INV_NO") & ""
            INV_NO_TO_CONSIDER = "('" & INV_NO & "')"
            PASSED_AMT = Val(rowSOTGIFTC.Item("INITIAL_VALUE") & "")
            GIFT_CTL_NO = rowSOTGIFTC.Item("GIFT_CTL_NO") & ""
            ...
            & " where ARTOPEN1.CUST_CODE = '" & CUST_CODE & "'" _
            & " and ARTOPEN1.INV_NUM IN " & INV_NO_TO_CONSIDER & ""
            Fill_Records("ARTOPEN1", "", False, ASCMAIN1.sql)

            For Each rowARTOPEN1 As DataRow In dst.Tables("ARTOPEN1").Select("INV_NUM = " & CStr(INV_NO))
                '  For Each rowARTOPEN1 As DataRow In dst.Tables("ARTOPEN1").Select("", "INV_DATE, INV_TYPE, INV_NUM")
                '  Select Case ("PO_SHIPMENT_LNO = " & CStr(PO_SHIPMENT_LNO))
                Dim rowARTPYMT3 As DataRow = dst.Tables("ARTPYMT3").NewRow
                With rowARTPYMT3
                    ...
                    Dim INV_BALANCE As Decimal = Val(.Item("INV_BALANCE") & "")
                    Dim INV_PMT_invoice As Decimal = PASSED_AMT * -1
                    Dim INV_DISC_TAKEN As Decimal = 0
                    Dim INV_WRITE_OFF As Decimal = 0
                    If .Item("INV_TYPE") = "O" Then
                        INV_PMT_invoice = INV_PMT_invoice
                    End If
            ...
            Next
            ' Update ARTPYMT5 WITH On Account
            Dim GC_AMOUNT As Decimal = PASSED_AMT
            Dim GC_ID As String = GIFT_CTL_NO
            Dim GC_DESC As String = "Generation of On Account from Gift Card"
        ...
        For Each rowARTPYMT5 As DataRow In dst.Tables("ARTPYMT5").Rows
            ...
            If rowARTPYMT5.Item("CHARGEBACK_IND") & "" = "1" Then
                ...
                '      Dim GIFT_CARD_ID As String = rowARTPYMT5.Item("CUST_REFERENCE") & ""
                Dim INV_NO_OA As String = rowARTPYMT5.Item("CHARGEBACK_NO") & ""
                GIFT_CTL_NO = rowARTPYMT5.Item("CUST_REFERENCE") & ""

                If GIFT_CTL_NO <> "" Then
                    ASCMAIN1.sql = "Update SOTGIFTC set INV_NO_OA  = '" & INV_NO_OA & "' WHERE GIFT_CTL_NO = '" & GIFT_CTL_NO & "' AND INV_NO_OA IS NULL"
                    ASCDATA1.ExecuteSQL()

                    ''Dim GIFT_CTL_NO As String = ""
                    ''ASCMAIN1.sql = "Select SOTGIFTC.* From SOTGIFTC  ' WHERE GIFT_CARD_ID = '" & GIFT_CARD_ID & "'"
                    ''For Each rowSOTGIFTC As DataRow In ASCDATA1.GetDataTable.Rows
                    ''    GIFT_CTL_NO = rowSOTGIFTC("GIFT_CTL_NO").ToString()
                    ''Next
