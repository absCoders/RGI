Public Class WHCRF003
    ' Move Cartons from Gun to Location
    ...
    Sub New(g As GunEnvironment)
        ...
        Me.MENU_ITEM_TYPE = "C"
        Me.MENU_ITEM_OBJECT = "WHCRF003"

        AppStates.Add("SCAN_LOC", "Scan Location|EXIT|")
        AppStates.Add("SCAN_LPN", "Scan Case ID to Locate|DONE|EXIT|")
            ...
            Select Case AppState
                ...
                Case "SCAN_LOC"
                    ...
                    If rowWHTLOCM1.Item("LOCATION_SINGLE_LOAD") & "" = "1" Then
                        ASCMAIN1.sql = "Select Distinct WHTBARC1.LOAD_NO from WHTLOCB1,WHTBARC1" & vbCrLf _
                        ...
                        If rows.Length > 0 Then
                            CreateResponse("", "R", "Location is marked for Single Load " & SCANTEXT)
                            Exit Select
                        End If
                    End If
                ...
                Case "SCAN_LPN"
                    ...
                    Else
                        ...
                        ElseIf rows(0).Item("LOCATION_CODE") & "" <> G.GUN_LOC Then
                            CreateResponse("", "R", "Case ID " & BAR_CODE & " not located to this gun")
                            Exit Select
                        End If
                ...
                Case "VERIFY"
                    If SCANTEXT = "Y" Then
                        ...
                        CreateResponse("SCAN_LOC", "B", CStr(tbl.Select("").Count) & " Cartons Sucessfully moved from Gun to Location")
                        tbl.Rows.Clear()
    ...
    Sub Update_Record()
        ...
        With rowWHTBARC0
            ...
            .Item("LAST_OPER") = G.USER_ID
            .Item("LOAD_STATUS") = "A"
            .Item("LOAD_COMMENT") = "Move Cartons from Gun to Location"
