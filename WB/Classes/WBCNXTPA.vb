...
#Region "Contructor"
    ...
    Private Sub createAttributes()
        ...
        sql.AppendLine("SELECT L3.STYLE_CODE, A1.ATTR_DESC")
...
#Region "Custom Methods"
    Private Sub CreateFeedBody()
        ...
        channelElement.AppendChild(makeChannelNode("link", "www.regency-rib.com"))
        channelElement.AppendChild(makeChannelNode("description", "Product feed for Nextopia"))
        For Each dr As DataRow In _DT.Select("WEB_IND = '1'")
    ...
    Private Function makeItemNode(ByVal dr As DataRow) As XmlNode
        Dim rowARTCUST1 As DataRow = Nothing
        Dim BASE As New ASFBASE0
        Dim Discounts As List(Of DISCOUNTS)
        'Discounts = SOCMAIN2.Price_Discounts(BASE, "", rowARTCUST1, dr.Item("STYLE_CODE"), False)
        Discounts = NTP_Price_Discounts(dr.Item("STYLE_CODE"))

        Dim itemElement As System.Xml.XmlElement = XmlDoc.CreateElement("item")
        itemElement.AppendChild(makeChannelNode("id", dr.Item("STYLE_CODE")))
        itemElement.AppendChild(makeChannelNode("title", dr.Item("STYLE_FULL_DESC")))
        itemElement.AppendChild(makeChannelNode("link", "http://www.regency-rib.com/" & dr.Item("STYLE_CODE") & ".html"))
        itemElement.AppendChild(makeChannelNode("image_link", "http://www.regency-rib.com/media/product/" & Replace(dr.Item("DEFAULT_IMAGE"), ".JPG", ".jpg")))
        itemElement.AppendChild(makeChannelNode("price", Discounts.Item(3).DISCOUNT_PRICE))
        itemElement.AppendChild(makeChannelNode("description", dr.Item("STYLE_FULL_DESC")))
        itemElement.AppendChild(makeChannelNode("product_type", "Home > " & makeCategory(dr.Item("STYLE_CLASS_CODE"))))
        itemElement.AppendChild(makeAttributesNode(dr.Item("STYLE_CODE")))
        itemElement.AppendChild(makeColorsNode(dr.Item("STYLE_CODE")))
        Return itemElement
    End Function
    ...
    Private Function makeAttributesNode(ByVal STYLE_CODE As String) As XmlNode
        Dim retVal As XmlNode = XmlDoc.CreateElement("attributes")
        For Each rowTABLE_NAME As DataRow In _Atributes.Select("STYLE_CODE = '" & STYLE_CODE & "'")
    ...
    Private Function makeColorsNode(ByVal STYLE_CODE As String) As XmlNode
        Dim retVal As XmlNode = XmlDoc.CreateElement("colors")
        For Each rowTABLE_NAME As DataRow In _Colors.Select("STYLE_CODE = '" & STYLE_CODE & "'")
            retVal.InnerText = retVal.InnerText + "||" + rowTABLE_NAME.Item("COLOR_DESC").ToString
        Next
        If retVal.InnerText.Length > 2 Then
            retVal.InnerText = retVal.InnerText.Substring(2, retVal.InnerText.Length - 2)
        End If
