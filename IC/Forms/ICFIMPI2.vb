Public Class ICFIMPI2
    ...
    Dim MISSING_PACK_FOUND As String = ""
    Dim rowSOTPARM1 As DataRow
    Dim ICTSTYL1 As String
    Dim ICTSTYC1 As String
    Dim ICTSTYV1 As String
    Dim ICTSTYL3 As String
    Dim ICTSTAT2 As String

    Dim sqlICTSTYL1 As String
...
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        With dst
            ...
            ' Create Work files for ICTSTYL1
            ASCMAIN1.sql = "Select * from ICTSTYL1 where rownum < 1"
            sqlICTSTYL1 = ASCMAIN1.sql

            ICTSTYL1 = ASCMAIN1.Temp_Table(ASCMAIN1.sql)

            ASCMAIN1.sql = "Select * from " & ICTSTYL1
            Create_TDA(.Tables.Add("ICTSTYL1"), ICTSTYL1, "**", 0, True)

            .Tables("ICTSTYL1").Columns.Add("EXISTING")


            ' Create Work files for ICTSTYC1
            ' Create_TDA(.Tables.Add, "ICTSTYC1", "*")

            ASCMAIN1.sql = "Select * from ICTSTYC1 where rownum < 1"
            sqlICTSTYC1 = ASCMAIN1.sql

            ICTSTYC1 = ASCMAIN1.Temp_Table(ASCMAIN1.sql)

            ASCMAIN1.sql = "Select * from " & ICTSTYC1
            Create_TDA(.Tables.Add("ICTSTYC1"), ICTSTYC1, "**", 0, True)


            ' Create Work files for ICTSTYV1

            '  Create_TDA(.Tables.Add, "ICTSTYV1", "*")
            ASCMAIN1.sql = "Select * from ICTSTYV1 where rownum < 1"
            sqlICTSTYV1 = ASCMAIN1.sql

            ICTSTYV1 = ASCMAIN1.Temp_Table(ASCMAIN1.sql)

            ASCMAIN1.sql = "Select * from " & ICTSTYV1
            Create_TDA(.Tables.Add("ICTSTYV1"), ICTSTYV1, "**", 0, True)

            ' Create Work files for ICTSTYL3
            'Create_TDA(.Tables.Add, "ICTSTYL3", "*")
            ASCMAIN1.sql = "Select * from ICTSTYL3 where rownum < 1"
            sqlICTSTYL3 = ASCMAIN1.sql

            ICTSTYL3 = ASCMAIN1.Temp_Table(ASCMAIN1.sql)

            ASCMAIN1.sql = "Select * from " & ICTSTYL3
            Create_TDA(.Tables.Add("ICTSTYL3"), ICTSTYL3, "**", 0, True)


            ' Create Work files for ICTSTAT2
            ' Create_TDA(.Tables.Add, "ICTSTAT2", "*")
            ASCMAIN1.sql = "Select * from ICTSTAT2 where rownum < 1"
            sqlICTSTAT2 = ASCMAIN1.sql

            ICTSTAT2 = ASCMAIN1.Temp_Table(ASCMAIN1.sql)

            ASCMAIN1.sql = "Select * from " & ICTSTAT2
            Create_TDA(.Tables.Add("ICTSTAT2"), ICTSTAT2, "**", 0, True)
    ...
    Overrides Sub Proceed_PreReq(ByVal eItemKey As String)
        ...
        Select Case eItemKey
            ...
            Case "Update"
                ...
                If MENU_ITEM_OBJECT <> "ICFIMPIU" Then
                ...
                Else
                    If MISSING_COLORS_FOUND = "Y" Then
                        ...
                        EXISTING_STYLES_FOUND = True
                    End If
                    If MISSING_PACK_FOUND = "Y" Then
                        EMsg &= vbCr & "Some Packing to be imported are not in the Packing Master Table"
                        EXISTING_STYLES_FOUND = True
                    End If
                End If
    ...
    Sub Load_Record()
        ...
        Else
            ASCMAIN1.sql = "TRUNCATE TABLE " & ICTSTYL1
            ASCDATA1.ExecuteSQL()

            ASCMAIN1.sql = "INSERT INTO " & ICTSTYL1 & " SELECT X.* FROM (" & sqlICTSTYL1 & ") X "
            ASCDATA1.ExecuteSQL()
    ...
    Sub Update_Record()
        ...
        If MENU_ITEM_OBJECT = "ICFIMPIU" Then
            ...
            ASCMAIN1.sql = "TRUNCATE TABLE " & ICTSTYC1
            ASCDATA1.ExecuteSQL()

            Update_Record_TDA("ICTSTYC1")

            ASCMAIN1.sql = "TRUNCATE TABLE " & ICTSTYV1
            ASCDATA1.ExecuteSQL()

            Update_Record_TDA("ICTSTYV1")

            ASCMAIN1.sql = "TRUNCATE TABLE " & ICTSTYL3
            ASCDATA1.ExecuteSQL()

            Update_Record_TDA("ICTSTYL3")

            ASCMAIN1.sql = "TRUNCATE TABLE " & ICTSTAT2
            ASCDATA1.ExecuteSQL()

            Update_Record_TDA("ICTSTAT2")
        ...
        If MENU_ITEM_OBJECT = "ICFIMPIU" Then
            ...
            ASCMAIN1.sql = "" _
              ...
              & " DELETE FROM ICTSTYL1 WHERE STYLE_CODE = R1.STYLE_CODE ; " & vbCrLf _
              & " DELETE FROM ICTSTYL3 WHERE STYLE_CODE = R1.STYLE_CODE ; " & vbCrLf _
              & " DELETE FROM ICTSTYV1 WHERE STYLE_CODE = R1.STYLE_CODE ; " & vbCrLf _
              & " End Loop; End; End;"
            ASCDATA1.ExecuteSQL()

            ASCMAIN1.sql = "" _
               & "Insert into ICTSTYL1 SELECT * FROM " & ICTSTYL1
            ASCDATA1.ExecuteSQL()

            ASCMAIN1.sql = "" _
                & "Insert into ICTSTYL3 SELECT * FROM " & ICTSTYL3
            ASCDATA1.ExecuteSQL()

            ASCMAIN1.sql = "" _
               & "Insert into ICTSTYV1 SELECT * FROM " & ICTSTYV1
            ASCDATA1.ExecuteSQL()


            ASCMAIN1.sql = "" _
            & "Insert into ICTSTYC1 SELECT * FROM " & ICTSTYC1 & " WHERE (STYLE_CODE, COLOR_CODE) NOT IN (SELECT STYLE_CODE, COLOR_CODE FROM ICTSTYC1) "
            ASCDATA1.ExecuteSQL()
                ...
                If datarec(i) <> " " Then
                    ...
                    Else
                        ...
                        With rowICTSTYL1
                            ...
                            .Item("STYLE_ASST_QTY") = Val(Trim(Replace(DATAROW(2), Chr(34), "")))
                            ...
                            If RESHIPBOX_L = "" Then
                                RESHIPBOX_CODE = ""
                            Else
                                If rowICTRSPBX.Length = 0 Then
                                    MsgBox(EMsg, MsgBoxStyle.OkOnly, "Style / Box Code" & Trim(Replace(DATAROW(0), " ", "") & RESHIPBOX_L & " Is Invalid"))
                                    MISSING_PACK_FOUND = "Y"
                                Else
                            ...
                            .Item("CARTON_PACK_QTY") = Val(Trim(DATAROW(6)))
                            ...
                            Dim EXCLUSIVE_STYLE As String = Trim(DATAROW(34))
                        ...
                        With rowICTSTYV1
                            ...
                            .Item("PO_COST") = Val(Trim(Replace(DATAROW(12), "$", "")))
                        ...
                        For C As Integer = 0 To 17
                            If Trim(DATAROW(C + 34)) <> "" Then
                                ...
                                With rowICTSTYC1
                                    ...
                                    Dim COLORROW As DataRow = LookUp("ICTSTYC1", New String() {rowICTSTYC1.Item("STYLE_CODE"), rowICTSTYC1.Item("COLOR_CODE")})
