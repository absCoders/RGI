Public Class SOROPEN5

    Dim optEDI As String = "A"
    Dim optDTL As String = "3"
    ...
    Protected Overrides Sub Build_Workfile()
        ...
        For Each ORDR_TYPE As String In New String() {"O"}
            If Absx1.optFor("OPTORDERS").Value = "A" Or Absx1.optFor("OPTORDERS").Value = ORDR_TYPE Then
                If ORDR_TYPE = "R" And optEDI = "E" Or Absx1.optFor("OPTSTATUS").Value = "P" Then
                Else
                    If ORDR_TYPE = "O" Then
                        If optEDI <> "A" Then sql_filter = " and SOTORDR1.ORDR_SOURCE = '" & optEDI & "'"
                    ...
                    Dim columnsSPTOPEN5 As String = ", SOTORDR2.STYLE_CODE, SOTORDR2.COLOR_CODE, SOTORDR2.ORDR_UNIT_PRICE" & vbCrLf _
                       & ", SOTORDR1.ORDR_SHIP_DATE, SOTORDR1.ORDR_CANCEL_DATE" & vbCrLf
                    ...
                    sql = "Select " & sql_SELECT_cols & vbCrLf _
                        ...
                        & " from " & "SOTORDR1, SOTORDR2" & sql_TABLE_NAMEs & vbCrLf _
                        ...
                        & ", " & OPT_G & "" & vbCrLf _
                        & ", " & OPT_S & ", " & OPT_C & "" & vbCrLf ' & columnsSPTOPEN5
                    ...
                    ASCDATA1.ExecuteSQL("Alter table " & ASTSRPT1 & " ADD " & columnsSPTOPEN5)
        ...
        'not worried about order type anymore?
        sql = "" _
        ...
        '    & ", SALES_DIVISION_CODE from SOTORDR0 " & vbCrLf _
        '    & " where ORDR_GROUP_NO in " _
        '    & " (Select Distinct ORDR_GROUP_NO from " & ASTSRPT1 & " where ORDR_TYPE = 'O')" & vbCrLf _
        '    & " union " & vbCrLf _
        '    & "Select 'O' ORDR_TYPE, RSRV_NO ORDR_GROUP_NO, CUST_CODE, TRUNC(INIT_DATE) ORDR_DATE" & vbCrLf _
        '    & ", ORDR_SHIP_DATE, ORDR_CANCEL_DATE, ORDR_CUST_PO, ORDR_DEPT" & vbCrLf _
        '    & ", 1 ORDR_CNT, 1 ORDR_CNT_OPEN, 0 ORDR_CNT_PICK, NULL CUST_DC_NO" & vbCrLf _
        '    & ", SALES_DIVISION_CODE from SOTRSRV1 " & vbCrLf _
        '    & " where RSRV_NO in " _
    ...
    Function Get_Dates(TYPE As String) As String
        ...
        For Each COLUMN_NAME As String In New String() {"ORDR_DATE_BOOKED", "ORDR_SHIP_DATE", "ORDR_CANCEL_DATE"}
            If Not Absx1.chkFor("CHK" & COLUMN_NAME & "_F").Checked Then
                sql = sql & " and A." & COLUMN_NAME & " >= '" & Format(Absx1.dteFor(COLUMN_NAME & "_F").Value, "dd-MMM-yyyy") & "'"
            End If
            If Not Absx1.chkFor("CHK" & COLUMN_NAME & "_L").Checked Then
                sql = sql & " and A." & COLUMN_NAME & " <= '" & Format(Absx1.dteFor(COLUMN_NAME & "_L").Value, "dd-MMM-yyyy") & "'"
            End If
        Next
        If TYPE = "R" Then
            sql = Replace(sql, "ORDR_DATE_BOOKED", "TRUNC(INIT_DATE)")
        End If
        Return sql
    End Function
    ...
    Public Overrides Sub Print_Report()
        ...
        If optEDI <> "A" Then SUBT &= Absx1.optFor("OPTEDI").Text & ", "
        ...
        CR_params.Add("CHKDTL", IIf(optDTL = "2", "1", "0"))
        CR_params.Add("OPTDTL", optDTL)
    ...
    Overrides Sub Verify_Special(ByVal eItemKey As String)
        ...
        Select Case eItemKey
            Case "Proceed"
                ...
                If optDTL = "2" And Val(rowASTDSQLA("SEQUENCE") & "") <> 0 Then
