...
#Region "ABS Standards"
    ...
    Public Overrides Sub Print_Report()
        ...
        Generate_Report("SORMTDV5", "Shipment Summary Recap", SUBT)
...
#End Region
    ...
    Private Function Prepare_Sales_Invoices(
        ...
        sql.AppendLine("SELECT ORDR_GROUP_NO")
        sql.AppendLine("FROM " & SOTINVHD)
        sql.AppendLine("GROUP BY ORDR_GROUP_NO")
        Dim tblGROUPS As DataTable = ASCDATA1.GetDataTable(sql.ToString(), String.Empty)
        Dim GRP_TOTAL As Int64 = tblGROUPS.Rows.Count
        Dim GRP_DONE As Int64 = 0
        Dim PCT_COMPLETE_LAST As String = ""
        For Each rowGROUPS As DataRow In tblGROUPS.Rows
            GRP_DONE += 1
            Dim PCT_COMPLETE As String = Format(((GRP_DONE / GRP_TOTAL) * 100), "###,###")
            If PCT_COMPLETE <> PCT_COMPLETE_LAST Then
                ASCMAIN1.Progress("Building Work File - SOTINVHD", PCT_COMPLETE & "%")
                PCT_COMPLETE_LAST = PCT_COMPLETE
            End If
            Dim ORDR_GROUP_NO As String = rowGROUPS.Item("ORDR_GROUP_NO").ToString & String.Empty
            ASCMAIN1.sql = "" _
                    & "Begin" _
                    ...
                    & "    AND SOTINVHD.ORDR_GROUP_NO = '" & ORDR_GROUP_NO & "'" _
