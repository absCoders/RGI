...
#Region "Declarations"
    ...
    Dim SkipProblemTables As Boolean = True
#End Region
...
#Region "ABS Standards"
    ...
    Public Overrides Sub Print_Report()
        ...
        If Absx1.chkFor("CHKSORMTDS3").Checked Then
            CR_params.Add("CHKCOSTS", IIf(Absx1.chkFor("CHKCOSTS").Checked, "1", "0"))
            CR_params.Add("CHKONLYTARF", IIf(Absx1.chkFor("CHKONLYTARF").Checked, "1", "0"))
            Generate_Report("SORMTDS3", "Sales Summary by Style", SUBT)
        End If

        If Absx1.chkFor("CHKSORMTDS4").Checked Then
            CR_params.Add("CHKCOSTS", IIf(Absx1.chkFor("CHKCOSTS").Checked, "1", "0"))
            Generate_Report("SORMTDS4", "Sales Summary by Customer", SUBT)
        End If

        If Absx1.chkFor("CHKSORMTDSG").Checked Then
            CR_params.Add("CHKCOSTS", "1")
            RPT_TITLE = "Sales by Customer w/out Groups"
            Generate_Report("SORMTDSG", RPT_TITLE, SUBT)
        End If

        If Absx1.chkFor("CHKSORMTDSY").Checked Then
            CR_params.Add("CHKCOSTS", "1")
            RPT_TITLE = "Daily Sales by Customer Recap"
            Generate_Report("SORMTDSY", RPT_TITLE, SUBT)
        End If

        If Absx1.chkFor("CHKSORINVHR").Checked Then
            Generate_Report("SORINVHR", "Credits Register", SUBT)
        End If

        If Not SkipProblemTables Then

            If Absx1.chkFor("CHKSORUPDT1").Checked Then
                Generate_Report("SORUPDT1", "Invoice Register", SUBT)
            End If

            If Absx1.chkFor("CHKSORMTDS1").Checked Then
                CR_params.Add("ONLY_THOSE_CUSTS", "1")
                Generate_Report("SORMTDS1", "Shipment Summary", SUBT)
            End If

            If Absx1.chkFor("CHKSORMTDSX").Checked Then
                CR_params.Add("CHKCOSTS", "1")
                CR_params.Add("ONLY_THOSE_CUSTS", "1")
            ...
            If Absx1.chkFor("CHKSORMTDSA").Checked Then
                CR_params.Add("CHKCOSTS", "1")
                CR_params.Add("ONLY_THOSE_CUSTS", "1")
    ...
    Overrides Sub Verify_Special(ByVal eItemKey As String)
        ...
        Select Case eItemKey
            Case "Proceed"
                ...
                End If
                If chkSORUPDT1.Checked Then
                    If chkSORMTDS3.Checked And (chkSORMTDS4.Checked Or chkSORMTDSG.Checked Or chkSORMTDSY.Checked Or chkSORINVHR.Checked) Then
                        EMsg &= vbCr & "Invoice Register Can Not Be Run At Same Time As Other Reports"
                    End If
                End If
        End Select
    End Sub
...
#End Region
    ...
    Private Sub chkINPROGRESS_CheckedChanged(sender As Object, e As EventArgs) Handles chkINPROGRESS.CheckedChanged
        If chkINPROGRESS.Checked Then
            'chkSORUPDT1.Visible = True
            chkSORMTDS1.Visible = True
            chkSORMTDSX.Visible = True
            chkSORMTDSA.Visible = True
        Else
            'chkSORUPDT1.Visible = False
            chkSORMTDS1.Visible = False
            chkSORMTDSX.Visible = False
            chkSORMTDSA.Visible = False
        End If

        If ASCMAIN1.USER_ID = "whr" Or ASCMAIN1.USER_ID = "wayne" Then
            'chkSORUPDT1.Enabled = True
            chkSORMTDS1.Enabled = True
            chkSORMTDSX.Enabled = True
            chkSORMTDSA.Enabled = True
        Else
            'chkSORUPDT1.Enabled = False
            chkSORMTDS1.Enabled = False
            chkSORMTDSX.Enabled = False
            chkSORMTDSA.Enabled = False
        End If
    End Sub

    Private Sub setSkipProblemTables()

        SkipProblemTables = True

        If chkSORUPDT1.Checked Then
            SkipProblemTables = False
        End If

        If chkSORMTDS1.Checked Then
            SkipProblemTables = False
        End If

        If chkSORMTDSX.Checked Then
            SkipProblemTables = False
        End If

        If chkSORMTDSA.Checked Then
            SkipProblemTables = False
        End If

    End Sub

    Private Sub chkSORUPDT1_CheckedChanged(sender As Object, e As EventArgs) Handles chkSORUPDT1.CheckedChanged
        setChkSORUPDT1()
    End Sub

    Private Sub setChkSORUPDT1()
        If chkSORUPDT1.Checked Then
            chkSORMTDS3.Checked = False
            chkSORMTDS4.Checked = False
            chkSORMTDSG.Checked = False
            chkSORMTDSY.Checked = False
            chkSORINVHR.Checked = False
        End If
    End Sub

    Private Sub chkSORMTDS3_CheckedChanged(sender As Object, e As EventArgs) Handles chkSORMTDS3.CheckedChanged
        If chkSORUPDT1.Checked And chkSORMTDS3.Checked Then
            chkSORUPDT1.Checked = False
        End If
    End Sub

    Private Sub chkSORMTDS4_CheckedChanged(sender As Object, e As EventArgs) Handles chkSORMTDS4.CheckedChanged
        If chkSORUPDT1.Checked And chkSORMTDS4.Checked Then
            chkSORUPDT1.Checked = False
        End If
    End Sub

    Private Sub chkSORMTDSG_CheckedChanged(sender As Object, e As EventArgs) Handles chkSORMTDSG.CheckedChanged
        If chkSORUPDT1.Checked And chkSORMTDSG.Checked Then
            chkSORUPDT1.Checked = False
        End If
    End Sub

    Private Sub chkSORMTDSY_CheckedChanged(sender As Object, e As EventArgs) Handles chkSORMTDSY.CheckedChanged
        If chkSORUPDT1.Checked And chkSORMTDSY.Checked Then
            chkSORUPDT1.Checked = False
        End If
    End Sub

    Private Sub chkSORINVHR_CheckedChanged(sender As Object, e As EventArgs) Handles chkSORINVHR.CheckedChanged
        If chkSORUPDT1.Checked And chkSORINVHR.Checked Then
            chkSORUPDT1.Checked = False
        End If
    End Sub
End Class