...
#Region "Declarations"
    ...
    Dim VandaleQuickMultiPeriod As Boolean = False
#End Region
...
#Region "ABS Standards"
    ...
    Protected Overrides Sub Build_Workfile()
        ...
        If CUST_CODE.Length > 0 Then
            sqlw &= "    and SOTINVH1.CUST_CODE = '" & CUST_CODE & "'"
        End If
        'TAC.SOCMAIN1.Prepare_Sales_Invoices(Me, sqlw, SOTINVH1, SOTINVH2)
        Performance.Clear()
        ...
        If (ASCMAIN1.Running_in_VS And ASCMAIN1.USER_ID = "wayne") Then
            For Each pf As KeyValuePair(Of String, Long) In Performance
                Debug.Print(pf.Key & " - " & pf.Value)
            Next
            Stop
        End If
    ...
    Public Overrides Sub Print_Report()
        If Absx1.chkFor("CHKXSAMPLES").Checked And Absx1.chkFor("CHKXTRANSF").Checked Then
            SUBT = SUBT & " Excluding Samples & Transfers"
        Else
            If Absx1.chkFor("CHKXTRANSF").Checked Then
                SUBT = SUBT & " Excluding Transfers"
            End If
            If Absx1.chkFor("CHKXSAMPLES").Checked Then
                SUBT = SUBT & " Excluding Samples"
            End If
        End If

        If Not VandaleQuickMultiPeriod Then
            Generate_Report("SORUPDT1", "Invoice Register", SUBT)
            'Generate_Report("SORINVHR", "Credits Register", SUBT)
            'Generate_Report("SORFINR1", "Factored Invoices Register", SUBT)
        End If
        Generate_Report("SORINVHR", "Credits Register", SUBT)

        For i As Integer = 1 To 4
            If i <> 2 Then
                If Not CR_params.Keys.Contains("CHKCOSTS") Then
                    CR_params.Add("CHKCOSTS", IIf(Absx1.chkFor("CHKCOSTS").Checked, "1", "0"))
                End If
                If i = 1 Then
                    If Not VandaleQuickMultiPeriod Then
                        If Not CR_params.Keys.Contains("ONLY_THOSE_CUSTS") Then
                            CR_params.Add("ONLY_THOSE_CUSTS", "1")
                        End If
                    End If
                End If
                If i = 3 Then
                    CR_params.Add("CHKONLYTARF", IIf(Absx1.chkFor("CHKONLYTARF").Checked, "1", "0"))
                Else
                    If CR_params.Keys.Contains("CHKONLYTARF") Then
                        CR_params.Remove("CHKONLYTARF")
                    End If
                End If
                If i = 1 Then RPT_TITLE = "Shipment Summary"
                If i = 2 Then RPT_TITLE = "Sales Summary by Item" ' not used
                If i = 3 Then RPT_TITLE = "Sales Summary by Style"
                If i = 4 Then RPT_TITLE = "Sales Summary by Customer"
                If i <> 4 Then
                    If Not VandaleQuickMultiPeriod Then
                        Generate_Report("SORMTDS" & Format(i, "0"), RPT_TITLE, SUBT)
                    End If
                Else
                    Generate_Report("SORMTDS" & Format(i, "0"), RPT_TITLE, SUBT)
                End If
        ...
        If Not VandaleQuickMultiPeriod Then
            If Absx1.chkFor("CHKEXRECAP").Checked Then
                CR_params.Add("CHKCOSTS", "1")
                CR_params.Add("ONLY_THOSE_CUSTS", "1")
                RPT_TITLE = "Shipment Summary By Date"
                Generate_Report("SORMTDSX", RPT_TITLE, SUBT)

                CR_params.Add("CHKCOSTS", "1")
                RPT_TITLE = "Daily Sales by Customer Recap"
                Generate_Report("SORMTDSY", RPT_TITLE, SUBT)
            End If

            If Absx1.chkFor("CHKSPRECAP").Checked Then
                CR_params.Add("CHKCOSTS", "1")
                CR_params.Add("ONLY_THOSE_CUSTS", "1")
                RPT_TITLE = "Shipment Summary Recap"
                Generate_Report("SORMTDSA", RPT_TITLE, SUBT)
            End If
        End If

        If Absx1.chkFor("CHKSBCNOGRP").Checked Then
            CR_params.Add("CHKCOSTS", "1")
            RPT_TITLE = "Sales By Customer"
            Generate_Report("SORMTDSG", RPT_TITLE, SUBT)
        End If
...
#End Region
    ...
    Private Function Prepare_Sales_Invoices( _
        sqlw As String, _
        ...
        'If MENU_ITEM_OBJECT = "SORUPDT1" Then
        ...
        'If MENU_ITEM_OBJECT = "SORMTDS1" Then
        'Else
        '    ASCMAIN1.sql = "Select SOTINVH2.* from " & SOTINVH2 & " SOTINVH2"
        '    dst.Tables.Add(ASCDATA1.GetDataTable("", "SOTINVH2", 3))
        'End If
        ...
        '    dst.Tables.Add(ASCDATA1.GetDataTable("", "SOTPICK1", 1))
        ...
        '    If dst.Tables("SOTPICK1").Select("ISNULL(PICK_STATUS,'?') <> 'F'").Length <> 0 Then
        '        Return "Unconfirmed Pick Tickets linked to Invoices; Call ABS"
        '    End If
        ...
        '    'dst.Tables.Add(ASCDATA1.GetDataTable("", "SOTPICK2", 2))
        '    Create_TDA(dst.Tables.Add, "SOTPICK2", "**", 0, True, "", 2, "PICK_QTY_BACK")
        '    Fill_Records("SOTPICK2")

        '    Create_Relation("SOTINVH1", "SOTPICK2", "PICK_NO")
        '    dst.Tables("SOTINVH1").Columns.Add("TOTAL_UNITS", GetType(System.Int64), "SUM(CHILD(SOTINVH1_SOTPICK2).PICK_QTY_CONF)")
        '    dst.Tables("SOTINVH1").Columns.Add("TOTAL_UNITS_CANC", GetType(System.Int64), "SUM(CHILD(SOTINVH1_SOTPICK2).PICK_QTY_CANC)")
        '    dst.Tables("SOTINVH1").Columns.Add("TOTAL_UNITS_BACK", GetType(System.Int64), "SUM(CHILD(SOTINVH1_SOTPICK2).PICK_QTY_BACK)")

        '    Create_TDA(dst.Tables.Add, "ARTOPEN1", "*")
        '    Create_TDA(dst.Tables.Add, "ARTPYMT1", "*")
        '    Create_TDA(dst.Tables.Add, "ARTPYMT2", "*")
        '    Create_TDA(dst.Tables.Add, "ARTPYMT3", "*")
        '    Create_TDA(dst.Tables.Add, "ARTCUST6", "*")
        'End If
        ...
        If Not VandaleQuickMultiPeriod Then
            Performance.Add("000001", DateDiff(DateInterval.Second, PerformanceStart, Now()))

            If chkCANCELGRP.Checked Then
                Dim SQLS As New System.Text.StringBuilder With {.Length = 0}
                SQLS.AppendLine(String.Format("Select Count(*) from ( Select DISTINCT ORDR_GROUP_NO from {0} )", CANCELS))
                ASCMAIN1.sql = SQLS.ToString()
                Dim GROUP_CNT As Int64 = Val(ASCDATA1.GetDataValue)
                Dim GROUP_CUR As Int64 = 0

                SQLS.Length = 0
                SQLS.AppendLine("SELECT DISTINCT ORDR_GROUP_NO")
                SQLS.AppendLine(String.Format("FROM {0}", CANCELS))
                Dim tblCANCELS As DataTable = ASCDATA1.GetDataTable(SQLS.ToString(), String.Empty)
                For Each rowtblCANCELS As DataRow In tblCANCELS.Rows
                    GROUP_CUR += 1
                    Dim GROUP_NO As String = rowtblCANCELS.Item("ORDR_GROUP_NO").ToString & String.Empty
                    ASCMAIN1.Progress("-", "C: " & GROUP_CUR & " of " & GROUP_CNT)
                    ASCMAIN1.sql = "" _
                    & "Begin" _
                    ...
                    & "    and SOTORDR1.ORDR_GROUP_NO = '" & GROUP_NO & "'" _
            ...
            '    ASCMAIN1.sql = "" _
            '        & "Begin" _
            '        & " Declare Cursor C1 is Select SOTINVHD.SHIP_BOL_NO_X, SOTINVHD.ORDR_GROUP_NO, SOTINVHD.SALES_DIVISION_CODE, SOTINVHD.STYLE_CODE, SOTINVHD.COLOR_CODE" _
            '        & "  , SUM(NVL(SOTORDR2.ORDR_QTY_CANC,0)) ORDR_QTY_CANC" _
            '        & "  , SUM(NVL(SOTORDR2.ORDR_QTY_CANC,0) * NVL(SOTORDR2.ORDR_UNIT_PRICE,0)) ORDR_AMT_CANC" _
            ...
            '        & "    AND SOTORDR2.STYLE_CODE = SOTINVHD.STYLE_CODE" _
            '        & "    AND SOTORDR2.COLOR_CODE = SOTINVHD.COLOR_CODE" _
            '        & "  group by SOTINVHD.SHIP_BOL_NO_X, SOTINVHD.ORDR_GROUP_NO, SOTINVHD.SALES_DIVISION_CODE, SOTINVHD.STYLE_CODE, SOTINVHD.COLOR_CODE;" _
            '        & " Begin" _
            '        & "  For R1 in C1 Loop" _
            '        & "   Begin" _
            '        & "    Update " & SOTINVHD & " Set " _
            ...
            '        & "     where SALES_DIVISION_CODE = R1.SALES_DIVISION_CODE" _
            '        & IIf(GROUP = "G",
            '                " and ORDR_GROUP_NO = R1.ORDR_GROUP_NO AND STYLE_CODE = R1.STYLE_CODE AND COLOR_CODE = R1.COLOR_CODE;",
            '                " and SHIP_BOL_NO_X = R1.SHIP_BOL_NO_X AND STYLE_CODE = R1.STYLE_CODE AND COLOR_CODE = R1.COLOR_CODE;") _
            '        & "   End;" _
            '        & "  End Loop;" _
            '        & " End;" _
            '        & "End;"
            '    ASCDATA1.ExecuteSQL()
            'Next
            Performance.Add("000002", DateDiff(DateInterval.Second, PerformanceStart, Now()))
        End If
        ...
        'S.AppendLine("SOTINVH1.INV_DATE,")
        ...
        'S.AppendLine("FROM SOTORDR1, SOTORDR2, ICTSTYL1, SOTINVH1")
        'S.AppendLine("WHERE SOTORDR1.ORDR_NO = SOTORDR2.ORDR_NO")
        'S.AppendLine("AND SOTORDR2.STYLE_CODE = ICTSTYL1.STYLE_CODE")
        'S.AppendLine("AND SOTORDR1.ORDR_NO = SOTINVH1.ORDR_NO")
        'S.AppendLine(String.Format("AND ORDR_GROUP_NO IN (SELECT ORDR_GROUP_NO FROM {0})", SOTINVHD))
    ...
    Private Sub UltraTextEditor63_ValueChanged(sender As Object, e As EventArgs) Handles UltraTextEditor63.ValueChanged

    End Sub

    Private Sub UltraTextEditor62_ValueChanged(sender As Object, e As EventArgs) Handles UltraTextEditor62.ValueChanged
