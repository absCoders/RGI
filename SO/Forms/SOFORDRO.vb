...
Public Class SOFORDRO
    ...
    Dim SQLColors As New StringBuilder With {.Length = 0}
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        With dst
            ...
            SQLColors.Length = 0
            SQLColors.AppendLine("SELECT * FROM")
            SQLColors.AppendLine("  (")
            SQLColors.AppendLine("   SELECT C1.STYLE_CODE, C1.COLOR_CODE,")
            SQLColors.AppendLine("   9999 AS ORDR_QTY,")
            SQLColors.AppendLine("   C2.COLOR_DESC AS COLOR_CODE_LONG,")
            SQLColors.AppendLine("   C1.STYLE_COLOR_STATUS,")
            SQLColors.AppendLine("   CASE WHEN")
            SQLColors.AppendLine("   SUM(")
            SQLColors.AppendLine("     CASE S2.WHSE_CODE WHEN 'MS'")
            SQLColors.AppendLine("     THEN (NVL(S2.WHSE_QTY_ON_HAND,0) - NVL(S2.WHSE_QTY_OPEN,0) - NVL(S2.WHSE_QTY_PICK,0))")
            SQLColors.AppendLine("     ELSE 0")
            SQLColors.AppendLine("     END) < 0")
            SQLColors.AppendLine("   THEN")
            SQLColors.AppendLine("     0")
            SQLColors.AppendLine("   ELSE")
            SQLColors.AppendLine("   SUM(")
            SQLColors.AppendLine("     CASE S2.WHSE_CODE")
            SQLColors.AppendLine("     WHEN 'MS'")
            SQLColors.AppendLine("     THEN (NVL(S2.WHSE_QTY_ON_HAND,0) - NVL(S2.WHSE_QTY_OPEN,0) - NVL(S2.WHSE_QTY_PICK,0))")
            SQLColors.AppendLine("     ELSE 0")
            SQLColors.AppendLine("     END)")
            SQLColors.AppendLine("   END AS MSOH,")
            SQLColors.AppendLine("   CASE WHEN")
            SQLColors.AppendLine("   SUM(")
            SQLColors.AppendLine("     CASE S2.WHSE_CODE WHEN 'MS'")
            SQLColors.AppendLine("     THEN (NVL(S2.WHSE_QTY_ON_ORDER,0) + NVL(S2.WHSE_QTY_TRAN,0))")
            SQLColors.AppendLine("     ELSE 0")
            SQLColors.AppendLine("     END) <= 0")
            SQLColors.AppendLine("   THEN")
            SQLColors.AppendLine("     0")
            ...
            SQLColors.AppendLine("       CASE S2.WHSE_CODE")
            SQLColors.AppendLine("       WHEN 'MS'")
            SQLColors.AppendLine("       THEN (NVL(S2.WHSE_QTY_ON_HAND,0) - NVL(S2.WHSE_QTY_OPEN,0) - NVL(S2.WHSE_QTY_PICK,0) + NVL(S2.WHSE_QTY_ON_ORDER,0) + NVL(S2.WHSE_QTY_TRAN,0))")
            SQLColors.AppendLine("       ELSE 0")
            SQLColors.AppendLine("       END) < 0")
            SQLColors.AppendLine("     THEN")
            SQLColors.AppendLine("       0")
            ...
            SQLColors.AppendLine("       THEN (NVL(S2.WHSE_QTY_ON_HAND,0) - NVL(S2.WHSE_QTY_OPEN,0) - NVL(S2.WHSE_QTY_PICK,0) + NVL(S2.WHSE_QTY_ON_ORDER,0) + NVL(S2.WHSE_QTY_TRAN,0))")
            SQLColors.AppendLine("       ELSE 0")
            SQLColors.AppendLine("       END) END")
            SQLColors.AppendLine("   END AS MSFT,")
            SQLColors.AppendLine(" CASE WHEN")
            SQLColors.AppendLine("   SUM(")
            SQLColors.AppendLine("     CASE S2.WHSE_CODE WHEN 'SW'")
            SQLColors.AppendLine("     THEN (NVL(S2.WHSE_QTY_ON_HAND,0) - NVL(S2.WHSE_QTY_OPEN,0) - NVL(S2.WHSE_QTY_PICK,0))")
            SQLColors.AppendLine("     ELSE 0")
            SQLColors.AppendLine("     END) < 0")
            SQLColors.AppendLine("   THEN")
            ...
            SQLColors.AppendLine("   SUM(")
            SQLColors.AppendLine("     CASE S2.WHSE_CODE")
            SQLColors.AppendLine("     WHEN 'SW'")
            SQLColors.AppendLine("     THEN (NVL(S2.WHSE_QTY_ON_HAND,0) - NVL(S2.WHSE_QTY_OPEN,0) - NVL(S2.WHSE_QTY_PICK,0))")
            SQLColors.AppendLine("     ELSE 0")
            SQLColors.AppendLine("     END)")
            SQLColors.AppendLine("   END AS SWOH,")
            SQLColors.AppendLine("   CASE WHEN")
            SQLColors.AppendLine("   SUM(")
            SQLColors.AppendLine("     CASE S2.WHSE_CODE WHEN 'SW'")
            SQLColors.AppendLine("     THEN (NVL(S2.WHSE_QTY_ON_ORDER,0) + NVL(S2.WHSE_QTY_TRAN,0))")
            ...
            SQLColors.AppendLine("       ELSE 0")
            SQLColors.AppendLine("       END) END")
            SQLColors.AppendLine("   END AS SWFT,")
            SQLColors.AppendLine("   C1.THEME_CODE")
            SQLColors.AppendLine("   FROM ICTSTYC1 C1")
            SQLColors.AppendLine("   LEFT JOIN ICTSTAT2 S2")
            SQLColors.AppendLine("   ON C1.STYLE_CODE  = S2.STYLE_CODE")
            SQLColors.AppendLine("   AND C1.COLOR_CODE = S2.COLOR_CODE")
            SQLColors.AppendLine("   INNER JOIN ICTCOLR1 C2")
            SQLColors.AppendLine("   ON C1.COLOR_CODE = C2.COLOR_CODE")
            SQLColors.AppendLine("   GROUP BY C1.STYLE_CODE, C1.COLOR_CODE, C2.COLOR_DESC, C1.STYLE_COLOR_STATUS, C1.THEME_CODE")
            SQLColors.AppendLine("  )")
            ...
            ASCMAIN1.sql = SQLColors.ToString
...
#Region "These Properties and Methods May Have Aplication In SKU Inquiry As Well"
    ...
    Private Sub FilterColors(STYLE_CODE As String)
        ...
        Dim STemp As String = SQLColors.ToString
        STemp = vbCrLf & STemp & $" WHERE STYLE_CODE = '{STYLE_CODE}'"
        If chkShowDiscColors.Checked = False Then
            STemp = vbCrLf & STemp & " AND (STYLE_COLOR_STATUS NOT IN ('D','N') or (MSOH <> 0) or (MSFT <> 0) or (SWOH <> 0)  or (SWFT <> 0))"
        End If
        Fill_Records("ICTSTYC1", , True, STemp)
...
#Region "Old Excel Functionality"
    ...
    Private Sub Excel_Make_Totals(ByRef oSheet As Excel.Worksheet)
        ...
        With oSheet.Range(Excel_Cell(LastRow, 1), Excel_Cell(LastRow, EndMark))
            ...
            .Value = "Effective Immediately: A temporary 25% surcharge now applies to all warehouse shipments; future adjustments may occur."
            .Font.Bold = True
