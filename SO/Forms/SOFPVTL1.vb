Public Class SOFPVTL1

#Region "Declarations"
    Dim CUST_CODE As String
    Dim PVT_LBL_CODE As String
...
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        With dst
            ASCMAIN1.sql = "Select ARTCUST1.PVT_LBL_CODE, ARTCUST1.CUST_CODE, ARTCUST1.CUST_NAME, nvl(SOTPVTL1.ITEM_COUNT, 0) ITEM_COUNT" & vbCrLf _
                & "from ARTCUST1, (Select PVT_LBL_CODE, COUNT(1) ITEM_COUNT from SOTPVTL1 group by PVT_LBL_CODE) SOTPVTL1" & vbCrLf _
                & "Where ARTCUST1.PVT_LBL_CODE = SOTPVTL1.PVT_LBL_CODE(+)" & vbCrLf _
                & "and ARTCUST1.PVT_LBL_CODE is not null"
            Create_TDA(.Tables.Add, "SOTPVTLX", "**", 0, False)

            ASCMAIN1.sql = "Select SOTPVTL1.*,ICTSTYL1.STYLE_DESC,ICTCOLR1.COLOR_DESC, ICTSTYL1.STYLE_UOM, ICTSTYL1.STYLE_RETAIL, ICTSTYC1.UPC_CODE" _
                & " from SOTPVTL1,ICTSTYL1,ICTCOLR1, ICTSTYC1" _
                & " where ICTSTYL1.STYLE_CODE = SOTPVTL1.STYLE_CODE" _
                & "   and ICTCOLR1.COLOR_CODE = SOTPVTL1.COLOR_CODE" _
                & "   and ICTSTYC1.STYLE_CODE = SOTPVTL1.STYLE_CODE" _
                & "   and ICTSTYC1.COLOR_CODE = SOTPVTL1.COLOR_CODE" _
                & "   and SOTPVTL1.PVT_LBL_CODE = :PARM1"
            Create_TDA(.Tables.Add, "SOTPVTL1", "**", 0, True, "V")
            .Tables("SOTPVTL1").Columns.Add("STATE")
            .Tables("SOTPVTL1").Columns.Add("SELECTED")
            .Tables("SOTPVTL1").Columns.Add("STYLE_CODE_ORIG")
            .Tables("SOTPVTL1").Columns.Add("BASE_PRICE", GetType(System.Int32))
            .Tables("SOTPVTL1").Columns.Add("QTY_OPEN", GetType(System.Int32))
            .Tables("SOTPVTL1").Columns.Add("FUT_QTY_OPEN", GetType(System.Int32))
            .Tables("SOTPVTL1").Columns.Add("FUT_QTY_DATE")
            .Tables("SOTPVTL1").Columns.Add("SKU", GetType(System.String), "STYLE_CODE + '-' + COLOR_CODE")
            .Tables("SOTPVTL1").Columns.Add("FILENAM")

            ASCMAIN1.sql = "Select * from SOTPVTL2"
            Create_TDA(.Tables.Add, "SOTPVTL2", "**", 0, True, "", 2)
            ...
            ASCMAIN1.sql = "Select SOTPVTL1.*,ICTSTYL1.STYLE_DESC,ICTCOLR1.COLOR_DESC" _
                & " from SOTPVTL1,ICTSTYL1,ICTCOLR1" _
                & " where ICTSTYL1.STYLE_CODE = SOTPVTL1.STYLE_CODE" _
                & "   and ICTCOLR1.COLOR_CODE = SOTPVTL1.COLOR_CODE"
            Create_TDA(.Tables.Add, "SOTPVTLO", "**", 0, False, "", 0)
            .Tables("SOTPVTLO").Columns.Add("STATE")
            .Tables("SOTPVTLO").Columns.Add("SELECTED")
        ...
        'TBL - used for packaging inst entered
        TBL.Columns.Add("PACKAGING_INST", GetType(String))

        grdSOTPVTLX.DataSource = dst.Tables("SOTPVTLX")
        grdSOTPVTL1.DataSource = dst.Tables("SOTPVTL1")
        grdSOTCSTYO.DataSource = dst.Tables("SOTPVTLO")


        grdSOTPVTL1.DisplayLayout.UseFixedHeaders = True
        With grdSOTPVTL1.DisplayLayout.Bands(0)
            For Each COLUMN_NAME As String In New String() {"SELECTED", "STYLE_CODE", "STYLE_DESC", "COLOR_CODE", "COLOR_DESC"}
                .Columns(COLUMN_NAME).Header.Fixed = True
            Next
        End With

        grdSOTPVTLX.DisplayLayout.UseFixedHeaders = True
        With grdSOTPVTLX.DisplayLayout.Bands(0)
            For Each COLUMN_NAME As String In New String() {"PVT_LBL_CODE", "CUST_CODE"}
                .Columns(COLUMN_NAME).Header.Fixed = True
            Next
        End With
        ...
        Create_Summary(grdSOTPVTLX, "PVT_LBL_CODE", "Count")
        Create_Summary(grdSOTPVTL1, "STYLE_CODE", "Count")
    ...
    Overrides Sub Proceed_PreReq(ByVal eItemKey As String)
        ...
        Select Case eItemKey
            ...
            Case "New", "Edit", "View"
                ...
                Else
                    ...
                    If rowARTCUST1 IsNot Nothing Then
                        ...
                        PVT_LBL_CODE = rowARTCUST1("PVT_LBL_CODE")
                    Else
                ...
                If EMsg = "" Then
                    If Not ASCMAIN1.Logical_Lock("SOTPVTL1", PVT_LBL_CODE) Then Exit Sub
                End If

            Case "Update"

                If Not splSOTCSTY1.Panel2Collapsed Then
                    If MsgBox("OK to continue with the Update leaving the EDI Styles Screen Up without Updating?",
                              MsgBoxStyle.YesNo,
                              "Maybe you forgot to click the big Update button in the Lower Left?") = MsgBoxResult.No Then Exit Sub
                End If

                For Each rowSOTPVTL1 As DataRow In dst.Tables("SOTPVTL1").Select("")
                    rowSOTPVTL1.AcceptChanges()
                    If rowSOTPVTL1.Item("STATE") & "" = "Added" Then
                        rowSOTPVTL1.SetAdded()
                    ElseIf rowSOTPVTL1.Item("STATE") & "" = "Edited" Then
                        rowSOTPVTL1.SetModified()
                    End If
                    If rowSOTPVTL1.Item("STATE") & "" = "Added" Or rowSOTPVTL1.Item("STATE") & "" = "Edited" Then
                        Dim rowICTSTYC1 As DataRow = LookUp("ICTSTYC1", New String() {rowSOTPVTL1.Item("STYLE_CODE"), rowSOTPVTL1.Item("COLOR_CODE")})
                        If rowICTSTYC1 Is Nothing Then
                            EMsg &= vbCr & "Bad Style/Color for Customer Style " & rowSOTPVTL1.Item("CUST_STYLE_CODE")
                        End If
                    End If
                Next
                '  Stop
            ...
            Case "Cancel"
                If EMsg = "" Then
                    If MsgBox("Do you really want to Cancel all changes made to this record?",
            ...
            Case "Delete"
                ...
                If EMsg = "" Then
                    If MsgBox("Do you really want to Delete all Pricing Data for Customer " & PVT_LBL_CODE & "?",
    ...
    Overrides Sub Mode_Settings(ByVal tf As Boolean, Optional ByVal MODE_description As String = "")
        ...
        ' grdSOTPVTLX.Visible = Not ScreenMode
        ...
        If ScreenMode Then
            For Each grd As UltraWinGrid.UltraGrid In New UltraWinGrid.UltraGrid() {grdSOTPVTL1}
                If EntryMode = "V" Then
    ...
    Sub Clear_Record()
        ...
        Absx1.txtFor("PVT_LBL_CODE").Text = ""
        CUST_CODE = ""
        PVT_LBL_CODE = ""
        txtFindBy.Text = ""
        ...
        grdSOTPVTL1.Rows.ColumnFilters.ClearAllFilters()
    ...
    Sub Load_Record()
        ...
        Fill_Records("SOTPVTL1", PVT_LBL_CODE)
        Fill_Records("SOTPVTL2")

        For Each rowSOTPVTL1 As DataRow In dst.Tables("SOTPVTL1").Select("")
            If rowSOTPVTL1("STYLE_RETAIL") & "" = "" And rowSOTPVTL1("BASE_PRICE") & "" = "" Then
                rowICTSTYL1 = LookUp("ICTSTYL1", rowSOTPVTL1("STYLE_CODE"))
                rowSOTPVTL1("STYLE_RETAIL") = Val(rowICTSTYL1.Item("STYLE_PRICE") & "")
            End If
            If rowSOTPVTL1("BASE_PRICE") & "" = "" And Val(rowSOTPVTL1("STYLE_RETAIL") & "") <> 0 Then
                rowSOTPVTL1("BASE_PRICE") = (Val(rowSOTPVTL1("STYLE_RETAIL")) * Val(rowARTCUST1.Item("PVT_LBL_DISC_PCT") & "") / 100)
            End If
        Next
        dst.Tables("SOTPVTL1").AcceptChanges()

        Sort_grdColumns(grdSOTPVTL1, "STYLE_CODE")
...
#Region "Popup_Menus"
    ...
    Overrides Sub Load_Popup_Menus()
        Load_Popup_Menu(grdSOTPVTLX, "SSS", "Show Filter", "Show GroupBox", "Show Pins")
        'Load_Popup_Menu(grdICTUPCHX, "SSS", "Show Filter", "Show GroupBox", "Show Pins")
        Load_Popup_Menu(grdSOTPVTL1, "SSSBSBB", "Show Filter", "Show GroupBox", "Show Pins",
    ...
    Public Overrides Sub tlb_ToolClick(ByVal sender As System.Object, ByVal e As Infragistics.Win.UltraWinToolbars.ToolClickEventArgs)
        ...
        Select Case e.Tool.Key
            Case "Show Selected Only"
                Dim dvw As DataView = DirectCast(grdSOTPVTL1.DataSource, DataTable).DefaultView
            ...
            Case "Load from Spreadsheet"
                MsgBox("Spreadsheet has not been defined", MsgBoxStyle.OkOnly, "Call Rick")
                Exit Sub
                Try
                    ...
                    If FILENAME <> "" Then
                        ...
                        Do While r < rmax ' oSheet.Cells(r, 0).Value & "" <> ""
                            ...
                            If LookUp("ICTSTYL1", STYLE_CODE) IsNot Nothing AndAlso LookUp("ICTSTYC1", New String() {STYLE_CODE, COLOR_CODE}) IsNot Nothing Then
                                If grdSOTPVTL1.ActiveRow IsNot Nothing AndAlso grdSOTPVTL1.ActiveRow.IsAddRow Then
                                    grdSOTPVTL1.ActiveRow.CancelUpdate()
                                End If
                                ...
                                If rowSOTCSTY1 IsNot Nothing Then
                                    ...
                                    End If
                                Else
                                    grdSOTPVTL1.DisplayLayout.Bands(0).AddNew.Activate()
                                    With grdSOTPVTL1.ActiveRow
                        ...
                        grdSOTPVTL1.Visible = True
                        ASCMAIN1.Progress("")

                        ' grdICTRSTYM.ResumeLayout()

                        Sort_grdColumns(grdSOTPVTL1, "CUST_STYLE_CODE")
...
#Region "ABSColumn Controls"
    ...
    Public Overrides Sub opt_ValueChanged(sender As Object, e As System.EventArgs)
        ...
        Select Case COLUMN_NAME
            'Case "PVT_LBL_CODE"
            '    For Each row As DataRow In dst.Tables("SOTPVTLX").Select($"PVT_LBL_CODE = '{txtctl.Text}'")
            '        Absx1.txtFor("CUST_CODE").Text = row("CUST_CODE").Value
            '        Exit For
            '    Next
    ...
    Public Overrides Sub txt_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Select Case COLUMN_NAME
            Case "PVT_LBL_CODE"
                For Each row As DataRow In dst.Tables("SOTPVTLX").Select($"PVT_LBL_CODE = '{txtctl.Text}'")
                    Absx1.txtFor("CUST_CODE").Text = row("CUST_CODE")
                    Exit For
                Next
...
#End Region
    ...
    Sub Load_SOTPVTLX(Optional PARM1 As String = "", Optional CUST_CODE As String = "")
        Me.Cursor = Cursors.WaitCursor
        ASCMAIN1.Progress("Now Loading Customers", "")

        Fill_Records("SOTPVTLX")
        Sort_grdColumns(grdSOTPVTLX, "PVT_LBL_CODE")
        grdSOTPVTLX.Visible = True

        Me.Cursor = Cursors.Default
        ASCMAIN1.Progress("", "")
    End Sub

    Private Sub grdSOTPVTLX_DoubleClickRow(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.DoubleClickRowEventArgs) Handles grdSOTPVTLX.DoubleClickRow
        If Not ScreenMode Then
            Absx1.txtFor("PVT_LBL_CODE").Text = e.Row.Cells("PVT_LBL_CODE").Value
...
#Region "grdSOTPVTL1"
    ...
    Private Sub grdSOTPVTL1_AfterCellUpdate(sender As Object, e As Infragistics.Win.UltraWinGrid.CellEventArgs) Handles grdSOTPVTL1.AfterCellUpdate
        With grdSOTPVTL1.ActiveRow
            Select Case e.Cell.Column.Key
                ...
                Case "COLOR_CODE"
                    ...
                    If COLOR_CODE <> "" Then
                        ...
                        End If
                        Dim STYLE_CODE As String = .Cells("STYLE_CODE").Value
                        ASCMAIN1.sql = "SELECT UPC_CODE FROM ICTSTYC1 WHERE STYLE_CODE = :PARM1 and COLOR_CODE = :PARM2"
                        For Each row As DataRow In ASCDATA1.GetDataTable(ASCMAIN1.sql, "", "VV", New Object() {STYLE_CODE, COLOR_CODE}).Select("")
                            .Cells("UPC_CODE").Value = row("UPC_CODE")
                        Next
                    End If

                Case "PACKAGING_INST"
                    Dim PACKAGING_INST As String = e.Cell.Value & ""
                    Dim UOM_CODE As String = .Cells("UOM_CODE").Value & ""
                    If PACKAGING_INST <> "" Then
                        For Each row As DataRow In dst.Tables("SOTPVTL2").Select($"UOM_CODE = '{UOM_CODE}' and PACKAGING_INST = '{PACKAGING_INST}'")
                            .Cells("LABOR_FEE").Value = Val(row("LABOR_FEE") * "")
                            .Cells("PACKAGING_COST").Value = Val(row("PACKAGING_COST") * "")
                            Exit For
                        Next
                    End If
    ...
    Private Sub grdSOTPVTL1_AfterRowUpdate(sender As Object, e As Infragistics.Win.UltraWinGrid.RowEventArgs) Handles grdSOTPVTL1.AfterRowUpdate

    End Sub

    Private Sub grdSOTPVTL1_BeforeCellUpdate(sender As Object, e As Infragistics.Win.UltraWinGrid.BeforeCellUpdateEventArgs) Handles grdSOTPVTL1.BeforeCellUpdate

    End Sub

    Private Sub grdSOTPVTL1_BeforeExitEditMode(sender As Object, e As Infragistics.Win.UltraWinGrid.BeforeExitEditModeEventArgs) Handles grdSOTPVTL1.BeforeExitEditMode
        If grdSOTPVTL1.ActiveCell IsNot Nothing Then
            With grdSOTPVTL1.ActiveCell
    ...
    Private Sub grdSOTPVTL1_BeforeRowsDeleted(sender As Object, e As Infragistics.Win.UltraWinGrid.BeforeRowsDeletedEventArgs) Handles grdSOTPVTL1.BeforeRowsDeleted

    End Sub

    Private Sub grdSOTPVTL1_BeforeRowUpdate(sender As Object, e As Infragistics.Win.UltraWinGrid.CancelableRowEventArgs) Handles grdSOTPVTL1.BeforeRowUpdate

        Validate_Columns(grdSOTPVTL1, "STYLE_CODE", e.Cancel)
        If e.Cancel Then
            MsgBox("Issue with Style Code")
            Exit Sub
        End If
        If Not e.Cancel Then
            Validate_Columns(grdSOTPVTL1, "COLOR_CODE", e.Cancel)
            If e.Cancel Then
                MsgBox("Issue with Color Code, or Style-Color Setup")
                Exit Sub
            End If
            If String.IsNullOrEmpty(grdSOTPVTL1.ActiveRow.Cells("SET_QTY").Text) OrElse grdSOTPVTL1.ActiveRow.Cells("SET_QTY").Value = 0 Then
                e.Cancel = True
        ...
        If e.Row.IsAddRow Then
            e.Row.Cells("PVT_LBL_CODE").Value = PVT_LBL_CODE
    ...
    Private Sub grdSOTPVTL1_ClickCellButton(sender As Object, e As Infragistics.Win.UltraWinGrid.CellEventArgs) Handles grdSOTPVTL1.ClickCellButton

        If grdSOTPVTL1.DisplayLayout.Override.AllowUpdate = DefaultableBoolean.False And grdSOTPVTL1.DisplayLayout.Override.AllowAddNew = UltraWinGrid.AllowAddNew.No Then
            Exit Sub 'Found in grdClickCellButton - need it for custom views
        End If
        With e.Cell.Row
            Select Case e.Cell.Column.Key
                Case "STYLE_CODE"
                    Dim sql_where As String = ""
                    grdClickCellButton(grdSOTPVTL1, sql_where)

                Case "COLOR_CODE"
                    Dim sql_where As String = "COLOR_CODE IN (Select COLOR_CODE from ICTSTYC1 where STYLE_CODE ='" & grdSOTPVTL1.ActiveRow.Cells("STYLE_CODE").Value & "')"

                    grdClickCellButton(grdSOTPVTL1, sql_where)

                Case "UOM_CODE"
                    Dim sql_where As String = "STYLE_UOM IN (Select STYLE_UOM from ICTUOMF1)"

                    grdClickCellButton(grdSOTPVTL1, sql_where,, "UOM_CODE", "STYLE_UOM")

                Case "PACKAGING_INST"
                    'Dim sql_where As String = $"STYLE_CODE in (select min(STYLE_CODE) from SOTPVTL1  WHERE PVT_LBL_CODE = '{PVT_LBL_CODE}' group by PACKAGING_INST) and PVT_LBL_CODE = '{PVT_LBL_CODE}'"
                    ...
                    With grdSOTPVTL1
                        ...
                        Else
                            ...
                            If .ActiveRow.IsAddRow OrElse (.ActiveRow.Band.Override.AllowUpdate <> DefaultableBoolean.False AndAlso .ActiveCell.CanEnterEditMode) Then
                                ...
                                TBL.Rows.Clear()
                                For Each row As DataRow In dst.Tables("SOTPVTL1").Select("")
                                    If TBL.Select($"PACKAGING_INST = '{row("PACKAGING_INST")}'").Count = 0 Then
                                        TBL.Rows.Add(row("PACKAGING_INST"))
                                    End If
                                Next

                                ASCMAIN1.CodeSelector.SQL = ASCMAIN1.CodeSelector.Get_SQL("PACKAGING_INST")
                                ...
                                Try
                                    ...
                                    If ASCMAIN1.CodeSelector.Selections <> 0 Then
                                        For Each PACKAGING_INST As String In ASCMAIN1.CodeSelector.SelectedCodes
                                            .ActiveCell.Value = PACKAGING_INST
                                            Exit For
                                        Next
                                    End If
                                Catch ex As Exception
                                    MsgBox(ex.Message, MsgBoxStyle.OkOnly, "No Codes")
                                End Try
                            End If
                        End If
    ...
    Private Sub grdSOTPVTL1_InitializeRow(sender As Object, e As Infragistics.Win.UltraWinGrid.InitializeRowEventArgs) Handles grdSOTPVTL1.InitializeRow
        'If e.Row.Cells("STATE").Value & "" = "Added" Then
        '    e.Row.Cells("STATE").Appearance.BackColor = Drawing.Color.LightGreen
        'ElseIf e.Row.Cells("STATE").Value & "" = "Edited" Then
        '    e.Row.Cells("STATE").Appearance.BackColor = Drawing.Color.Blue
        'Else
...
#End Region
    ...
    Function Validate_Style(STYLE_CODE_z As String) As String
        ...
        If EMsg <> "" And grdSOTPVTL1.ActiveRow.IsAddRow Then
    ...
    Private Sub cmdUpdate_Click(sender As System.Object, e As System.EventArgs) Handles cmdUpdate.Click
        If Not chkAddNewStyles.Checked Then
