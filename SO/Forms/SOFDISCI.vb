...
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
    ...
    Overrides Sub Proceed_PreReq(ByVal eItemKey As String)
        ...
        Select Case eItemKey
            ...
            Case "Cancel"
                If MessageBox.Show("Do you want to Cancel any changes?", _
                                    "Cancel Changes", MessageBoxButtons.YesNo, MessageBoxIcon.Question, _
            ...
            Case "Save Changes"
                If Not releasedInventoryShortages Then
                    If MessageBox.Show("Do you want to Print the Letters for the selected Sales orders?", _
                                       "Print Letters", MessageBoxButtons.YesNo, MessageBoxIcon.Question, _
                                       MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.No Then
                        Exit Sub
                ...
                Else
                    If MessageBox.Show("Do you want to Save your Changes? This menu choice does not cancel items off the Sales Orders.", _
    ...
    Private Sub Update_Record()
        ...
        Try
            ...
            For Each rowSOTORDR1 As DataRow In dst.Tables("SOTORDR1").Select("", "CUST_CODE,ORDR_NO")
                ...
                For Each rowSOTORDR2 As DataRow In dst.Tables("SOTORDR2").Select("ORDR_NO = '" & ORDR_NO & "'")
                    ORDR_LNO = rowSOTORDR2.Item("ORDR_LNO") & String.Empty
            ...
            For Each row As DataRow In dst.Tables("SOTORDR1").Select("SELECTED = '1'", "CUST_CODE,ORDR_NO")
                If dst.Tables("SOTORDR2").Select("ORDR_NO = '" & row.Item("ORDR_NO") & "' AND SELECTED = '1'").Length = 0 Then
                    Try
                        ...
                        ASCDATA1.ExecuteSQL("DELETE FROM SOTORDRC WHERE ORDR_NO = '" & row.Item("ORDR_NO") & "'")
                    ...
                    Catch ex As Exception
                        Rollback()
                        MessageBox.Show("Error setting orders with no details (1): " & ex.Message, "Update", MessageBoxButtons.OK, MessageBoxIcon.Error)
                        Exit Sub
                    End Try
                End If
            Next
        ...
        Try
            ...
            For Each row As DataRow In dst.Tables("SOTORDR1").Select("SELECTED = '1'", "CUST_CODE, ORDR_NO")
                ...
                filter = "{SOTORDR1.ORDR_NO}='" & row.Item("ORDR_NO") & "' AND {SOTORDR2.SELECTED}='1'"

                ASCMAIN1.Progress("-", row.Item("ORDR_NO"))

                If EmailCustomer(row.Item("ORDR_NO"), fileAttached, emailedTo, filter, False) Then
                    ...
                    ASCDATA1.ExecuteSQL("UPDATE SOTORDRC SET EMAILED_TO_CUST = '1' WHERE ORDR_NO = '" & row.Item("ORDR_NO") & "'")
                End If
        ...
        Try
            ...
            For Each row As DataRow In dst.Tables("SOTORDRC").Select("EMAILED_TO_CUST = '1' OR PRINTED = '1'")
                ORDR_NO = row.Item("ORDR_NO") & String.Empty
            ...
            For Each row As DataRow In dst.Tables("SOTORDR1").Select("SELECTED = '1'", "CUST_CODE,ORDR_NO")
                ...
                ASCDATA1.ExecuteSQL("UPDATE SOTORDRC SET PRINTED = '1' WHERE ORDR_NO = '" & row.Item("ORDR_NO") & "'")
            Next
        ...
        Try
            ...
            For Each row As DataRow In ASCDATA1.SelectDistinct("SOTORDR1", New String() {"SREP_CODE"}).Select("", "SREP_CODE")
                ...
                filter = "{SOTORDR1.SREP_CODE}='" & SREP_CODE & "' AND {SOTORDR1.SELECTED}='2' AND {SOTORDR2.SELECTED}='1'"

                If EmailCustomer(SREP_CODE, fileAttached, emailedTo, filter, True) Then
                    Try
                        BeginTrans()
                        For Each rowwk As DataRow In dst.Tables("SOTORDR1").Select("SELECTED = '2' AND SREP_CODE = '" & SREP_CODE & "'")
                            TAC.TACMAIN1.Record_Event("SOTORDR1", _
                                 rowwk.Item("ORDR_NO"), _
                                 Now, _
                                 ASCMAIN1.USER_ID, _
                                 "CANCL", _
                            ...
                            'EMAILED_TO_CUST, EMAILED_TO_SREP, PRINTED
                            ASCDATA1.ExecuteSQL("UPDATE SOTORDRC SET EMAILED_TO_SREP = '1' WHERE ORDR_NO = '" & rowwk.Item("ORDR_NO") & "'")
                        Next
...
#Region "Popup_Menus"
    ...
    Overrides Sub Load_Popup_Menus()
        ...
        Load_Popup_Menu(grdSOTORDRX, "SSSPBBBB", "Show Filter", "Show GroupBox", "Show Pins", _
                        "Select All", "De-Select All", "Sales Order Inquiry")

        Load_Popup_Menu(grdSOTORDRX2, "SSSPBBBB", "Show Filter", "Show GroupBox", "Show Pins", _
...
#End Region
    ...
    Private Function EmailCustomer(ByVal ORDR_NO As String, ByRef fileAttachment As String, ByRef emailedTo As String, ByRef filter As String, ByVal emailToSalesRep As Boolean) As Boolean
        ...
        Try
            ...
            fileAttachment = ASCMAIN1.Folders("Temp") & attachFileName & ".pdf"
    ...
    Private Sub grdSOTORDRX_InitializeLayout(sender As Object, e As UltraWinGrid.InitializeLayoutEventArgs) Handles grdSOTORDRX.InitializeLayout
