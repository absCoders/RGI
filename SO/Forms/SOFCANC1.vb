...
Public Class SOFCANC1

    Const maxStyles As Integer = 800
    ...
    Dim SupressSummary As Boolean = True
...
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        With dst
            ...
            MyBase.Create_TDA(.Tables.Add, "SOTCANCY", "**", 0, False, "", 6)
            With .Tables("SOTCANCY")
                .Columns.Add("CANCEL", GetType(System.String)) ', "IIF(ordr_qty - (ordr_qty_open + ordr_qty_canc) = 0 ,'1','0')")
                .Columns("CANCEL").DefaultValue = "0"
            End With
            ...
            'ASCMAIN1.sql = "Select X.STYLE_CODE, X.COLOR_CODE" & vbCrLf _
            '    & ", ICTSTYL1.STYLE_DESC, X.ORDR_QTY, X.ORDR_QTY_OPEN" & vbCrLf _
            '    & ", X.ORDR_QTY_PICK, X.ORDR_QTY_SHIP, X.ORDR_QTY_CANC" & vbCrLf _
            '    & " from (Select STYLE_CODE, COLOR_CODE" & vbCrLf _
            ...
            '    & " from " & SOTCANCY & " group by STYLE_CODE, COLOR_CODE) X, ICTSTYL1" & vbCrLf _
            '    & " where ICTSTYL1.STYLE_CODE = X.STYLE_CODE"

            ASCMAIN1.sql = "Select X.STYLE_CODE, X.COLOR_CODE, X.CUST_SIZE_CODE, X.CUST_UPC" & vbCrLf _
                & ", ICTSTYL1.STYLE_DESC" & vbCrLf _
                & " from (Select STYLE_CODE, COLOR_CODE, CUST_SIZE_CODE, CUST_UPC" & vbCrLf _
                & ", Sum (ORDR_QTY) ORDR_QTY" & vbCrLf _
                & ", Sum (ORDR_QTY_OPEN) ORDR_QTY_OPEN" & vbCrLf _
                & ", Sum (ORDR_QTY_PICK) ORDR_QTY_PICK" & vbCrLf _
                & ", Sum (ORDR_QTY_SHIP) ORDR_QTY_SHIP" & vbCrLf _
                & ", Sum (ORDR_QTY_CANC) ORDR_QTY_CANC" & vbCrLf _
                & " from " & SOTCANCY & " group by STYLE_CODE, COLOR_CODE, CUST_SIZE_CODE, CUST_UPC) X, ICTSTYL1" & vbCrLf _
                & " where ICTSTYL1.STYLE_CODE = X.STYLE_CODE"

            Dim sqlICTSTATX As String = "" _
                & "Select STYLE_CODE, COLOR_CODE" & vbCrLf _
                & ", Sum (WHSE_QTY_ON_HAND) WHSE_QTY_ON_HAND" & vbCrLf _
                & ", Sum (WHSE_QTY_ON_ORDER) WHSE_QTY_ON_ORDER" & vbCrLf _
                & ", Sum (WHSE_QTY_TRAN) WHSE_QTY_TRAN" & vbCrLf _
                & ", Sum (WHSE_QTY_OPEN) WHSE_QTY_OPEN" & vbCrLf _
                & ", Sum (WHSE_QTY_PICK) WHSE_QTY_PICK" & vbCrLf _
                & " from ICTSTAT2" & vbCrLf _
                & " where (STYLE_CODE, COLOR_CODE, WHSE_CODE) in " & vbCrLf _
                & " (Select Distinct STYLE_CODE, COLOR_CODE, WHSE_CODE from " & SOTCANCY & ")" & vbCrLf _
                & " group by STYLE_CODE, COLOR_CODE"
            ASCMAIN1.sql = "Select A.*, B.WHSE_QTY_ON_HAND, B.WHSE_QTY_ON_ORDER, B.WHSE_QTY_TRAN, B.WHSE_QTY_OPEN, B.WHSE_QTY_PICK from (" & ASCMAIN1.sql & ") A,(" & sqlICTSTATX & ") B" & vbCrLf _
                & " where B.STYLE_CODE (+) = A.STYLE_CODE And B.COLOR_CODE (+) = A.COLOR_CODE"
            MyBase.Create_TDA(.Tables.Add, "ICTSTYLX", "**", 0, False, String.Empty, 4)

            Create_Relation("ICTSTYLX", "SOTCANCY", "STYLE_CODE,COLOR_CODE, CUST_SIZE_CODE, CUST_UPC")
            ...
            With .Tables("ICTSTYLX")
                ...
                '.Columns("SELECTED").DefaultValue = "1"
                .Columns.Add("OTS_INV", GetType(System.Int64), "ISNULL(WHSE_QTY_ON_HAND,0) - ISNULL(WHSE_QTY_PICK,0)")
                .Columns.Add("NET_POS", GetType(System.Int64), "OTS_INV - ISNULL(WHSE_QTY_OPEN,0)")
            End With
            ...
            'ASCMAIN1.sql = "Select CUST_STORE_NO, ORDR_NO, ORDR_STATUS from SOTORDR1" & vbCrLf _
            '    & " where ORDR_NO in (Select ORDR_NO from " & SOTCANCY & ")"
            'Create_TDA(.Tables.Add, "SOTCANCS", "**", 0, False, String.Empty, 2)
            'With .Tables("SOTCANCS").Columns
        ...
        Show_Filter(grdICTSTYLX, True)

        With grdICTSTYLX.DisplayLayout.Bands(0)
            For Each COLUMN_NAME As String In New String() {"STYLE_CODE", "COLOR_CODE", "STYLE_DESC"}
                .Columns(COLUMN_NAME).Header.Fixed = True
            Next
            For Each GCOL As UltraWinGrid.UltraGridColumn In .Columns
                If GCOL.Key = "SELECTED" Then
                ...
                GCOL.Header.Appearance.BackColor2 = System.Drawing.Color.White
                GCOL.Header.Appearance.BackGradientStyle = GradientStyle.ForwardDiagonal
                If New String() {"STYLE_CODE", "COLOR_CODE", "STYLE_DESC", "CUST_SIZE_CODE", "CUST_UPC"}.Contains(GCOL.Key) Then
                    GCOL.Header.Appearance.BackColor2 = System.Drawing.Color.LightGreen
                ElseIf GCOL.Key.StartsWith("ORDR_QTY") Then
                    GCOL.Header.Appearance.BackColor2 = System.Drawing.Color.Gold
                    GCOL.Width = 65
        ...
        With grdSOTCANCY.DisplayLayout.Bands(0)
            For Each COLUMN_NAME As String In New String() {"CANCEL", "CUST_STORE_NO", "ORDR_NO", "ORDR_STATUS"}
                .Columns(COLUMN_NAME).Header.Fixed = True
            Next
            For Each GCOL As UltraWinGrid.UltraGridColumn In .Columns
        ...
        Select Case eItemKey
            ...
            Case "View", "Edit"
                ...
                If eItemKey = "Edit" Then
                    If optCancelMode.Value = "O" Or optCancelMode.Value = "G" Then
                        ' CHECK TO SEE IF ANY OF THESE ORDERS RELIEVED RESERVATIONS 
                        ' BECAUSE THE BY ORDER MODE UPDATE CURRENTLY DOES NOT SUPPORT
                        ' RESTORING THE RESERVATION IF CANCELLING THE ORDER
            ...
            Case "Update"
                ...
                If optCancelMode.Value = "S" Or optCancelMode.Value = "G" Then
    ...
    Overrides Sub Mode_Settings(ByVal tf As Boolean, Optional ByVal Mode_Desc As String = "")
        ...
        If UltraExplorerBar1.Groups.Count > 0 Then
            With UltraExplorerBar1
                With .Groups("Screen Control")
                    ...
                    If optCancelMode.Value = "S" Then
                        .Items("Update").Text = "Update"
                    Else
                        .Items("Update").Text = "Update Cancellation"
                    End If
                ...
                Setup_tabSOTCANCX()
                .Groups("Cancellation Mode").Visible = Not ScreenMode
        ...
        If SupressSummary Then tabSOTCANCX.Tabs(1).Visible = False

        MyBase.Set_Read_Only(UltraGroupBox1, ScreenMode)

        splSOTCANCX.Visible = ScreenMode And (optCancelMode.Value <> "O")
        grdSOTORDR0.Visible = Not ScreenMode Or (optCancelMode.Value = "O")
        'splCancelType.Panel1Collapsed = optCancelMode.Value <> "S"
        'splCancelType.Panel2Collapsed = optCancelMode.Value <> "G"

        grdSOTORDR0.DisplayLayout.Bands(0).Columns("SELECTED").Hidden = ScreenMode

        If ScreenMode Then
            If optCancelMode.Value = "O" Then

            Else

                For Each grd As UltraWinGrid.UltraGrid _
                    In New UltraWinGrid.UltraGrid() _
                    {grdICTSTYLX, grdSOTCANCX, grdSOTCANCY}
                    ...
                    For Each GCOL As UltraWinGrid.UltraGridColumn In .Columns
                        ...
                        ElseIf GCOL.Key = "CANCEL" Then
                            ...
                            Else
                                ...
                                GCOL.CellActivation = UltraWinGrid.Activation.AllowEdit
                            End If
                        ElseIf GCOL.Key = "COUNT" Then
                            If optCancelMode.Value = "S" Then
                                GCOL.Hidden = True
                            Else
                                GCOL.Hidden = False
                            End If
                        ElseIf GCOL.Key = "CUST_STORE_NO" Or GCOL.Key = "ORDR_NO" Or GCOL.Key = "ORDR_STATUS" Then
                            If optCancelMode.Value = "G" Then
                                GCOL.Hidden = True
                            Else
                                GCOL.Hidden = False
                            End If
    ...
    Private Sub Clear_Record()
        ...
        For Each TABLE_NAME As String In New String() _
            {"SOTCANCX", "ICTSTYLX", "SOTORDR0", "SOTCANCY", "SOTORDR1", "SOTORDR2", "SOTRSRVX", "SOTRSRV1", "SOTRSRV2", "ICTSTATO"}
        ...
        dteORDR_DATE.Value = DBNull.Value
        ...
        '    Absx1.txtFor("CUST_CODE").Text = "KMART"
        '    Absx1.txtFor("WHSE_CODE").Text = "NJE"
        '    dteORDR_DATE.Value = CDate("10/24/2016")
        'End If
    ...
    Private Sub Load_Record()
        ...
        If optCancelMode.Value = "O" Then
            ...
            ASCDATA1.DeleteRows(dst.Tables("SOTORDR0"), "SELECTED <> '1'")
        ...
        Else
            ...
            Else
                ASCMAIN1.sql = "" _
            & "Select '000000' CUST_STORE_NO, ORDR_GROUP_NO ORDR_NO, ORDR_CUST_PO, ORDR_GROUP_NO" & vbCrLf _
            & ", STYLE_CODE, COLOR_CODE, max(NVL(CUST_SIZE_CODE,'N_A')) CUST_SIZE_CODE " & vbCrLf _
            & ", max(NVL(CUST_UPC,'N_A')) CUST_UPC, WHSE_CODE, max(ORDR_STATUS) ORDR_STATUS" & vbCrLf _
            & ", sum(ORDR_QTY) ORDR_QTY" & vbCrLf _
            & ", sum(ORDR_QTY_OPEN) ORDR_QTY_OPEN" & vbCrLf _
            & ", sum(ORDR_QTY_PICK) ORDR_QTY_PICK" & vbCrLf _
            & ", sum(ORDR_QTY_SHIP) ORDR_QTY_SHIP" & vbCrLf _
            & ", sum(ORDR_QTY_CANC) ORDR_QTY_CANC" & vbCrLf _
            & ", sum(ORDR_QTY_OPEN) ORIG_QTY_OPEN" & vbCrLf _
            & ", COUNT(1) COUNT" & vbCrLf _
            & " from " & SOTCANCY & vbCrLf _
            & "GROUP BY ORDR_GROUP_NO, ORDR_CUST_PO, ORDR_GROUP_NO" & vbCrLf _
            & ", STYLE_CODE, COLOR_CODE " & vbCrLf _
            & ", WHSE_CODE" & vbCrLf

                Fill_Records("SOTCANCY", "", True, ASCMAIN1.sql)
                For Each row As DataRow In dst.Tables("SOTCANCY").Select("")
                    If Val(row.Item("ORDR_QTY_OPEN") & "") = 0 And Val(row.Item("ORDR_QTY_CANC") & "") <> 0 Then
                        row.Item("CANCEL") = "1"
                    End If
                Next
            End If

            If Not SupressSummary Then Fill_Records("SOTCANCX")
            Sort_grdColumns(grdSOTCANCX, "CUST_STORE_NO")

            Fill_Records("ICTSTYLX")
            Sort_grdColumns(grdICTSTYLX, "STYLE_CODE,COLOR_CODE")

            If Not SupressSummary Then
                With grdSOTCANCX.DisplayLayout.Bands(0)
                    .Summaries.Clear()
                    ...
                    For Each rowICTSTYLX As DataRow In dst.Tables("ICTSTYLX").Select("", "STYLE_CODE,COLOR_CODE,CUST_SIZE_CODE,CUST_UPC")
                        ...
                        Dim CUST_SIZE_CODE As String = rowICTSTYLX.Item("CUST_SIZE_CODE") & ""
                        Dim CUST_UPC As String = rowICTSTYLX.Item("CUST_UPC") & ""
                        iCol += 1
                        Dim C As String = "Q" & Format(iCol, "00")
                        With .Columns(C)
                            .Hidden = False
                            .Header.Caption = STYLE_CODE & vbCrLf & COLOR_CODE & vbCrLf & CUST_SIZE_CODE & " - " & CUST_UPC
                            .Width = 80
                            .Format = "#,##0"
                            .Header.ToolTipText = STYLE_DESC
                        End With
                        Create_Summary(grdSOTCANCX, C)
                        iSC.Add(STYLE_CODE & "-" & COLOR_CODE & " - " & CUST_SIZE_CODE & " - " & CUST_UPC, iCol)
                    Next
                End With
                ...
                If grdICTSTYLX.ActiveRow Is Nothing Then
                    Highlight_Style("", "")
                Else
                    Highlight_Style(grdICTSTYLX.ActiveRow.Cells("STYLE_CODE").Value, grdICTSTYLX.ActiveRow.Cells("COLOR_CODE").Value)
                End If
            End If
    ...
    Private Sub Update_Record()
        ...
        ASCMAIN1.sql = "Select ORDR_GROUP_NO from SOTORDR0 where ORDR_GROUP_NO in (" & Mid(sqlORDR_GROUP_NOs, 2) & ")"
        Dim SOTORDR0 As String = ASCMAIN1.Temp_Table
        ASCMAIN1.sql = "Select ORDR_NO, WHSE_CODE from SOTORDR1 where ORDR_GROUP_NO in (" & Mid(sqlORDR_GROUP_NOs, 2) & ")"
        Dim SOTORDR1 As String = ASCMAIN1.Temp_Table
        ...
        Try
            ...
            If optCancelMode.Value = "O" Then
                ...
                ASCMAIN1.sql = "" _
                    ...
                    & "  Select SOTORDR1.WHSE_CODE, SOTORDR2.STYLE_CODE, SOTORDR2.COLOR_CODE, SUM (SOTORDR2.ORDR_QTY_OPEN) OPEN " & vbCrLf _
                    ...
                    & "   Update ICTSTAT2 Set WHSE_QTY_OPEN = NVL(WHSE_QTY_OPEN,0) - NVL(R1.OPEN,0)" & vbCrLf _
                    ...
                    & "     Insert into ICTSTAT2 (WHSE_CODE, STYLE_CODE, COLOR_CODE, WHSE_QTY_OPEN) Values (R1.WHSE_CODE, R1.STYLE_CODE, R1.COLOR_CODE, R1.OPEN);" & vbCrLf _
                    & "   End If;" & vbCrLf _
                    & "  End Loop;" & vbCrLf _
                    & " End;" & vbCrLf _
                    & "End;"
                ...
                Dim sqlw1 As String = " where ORDR_NO in (Select ORDR_NO from " & SOTORDR1 & ")"
                ASCDATA1.ExecuteSQL("Update SOTORDR2 Set ORDR_QTY_CANC = NVL(ORDR_QTY_CANC,0) + NVL(ORDR_QTY_OPEN,0)" & sqlw1)
                ASCDATA1.ExecuteSQL("Update SOTORDR2 Set ORDR_QTY_OPEN = 0" & sqlw1)
                ASCDATA1.ExecuteSQL("Update SOTORDR2 Set ORDR_STATUS = " & vbCrLf _
                                    & " (CASE WHEN NVL(ORDR_QTY_OPEN,0) <> 0 THEN 'O' ELSE " & vbCrLf _
                                    & " (CASE WHEN NVL(ORDR_QTY_PICK,0) <> 0 THEN 'P' ELSE " & vbCrLf _
                                    & " (CASE WHEN NVL(ORDR_QTY_SHIP,0) <> 0 THEN 'F' ELSE 'C' END) END) END)" & vbCrLf _
                                    & sqlw1)
                ...
                ASCMAIN1.sql = "" _
                    ...
                    & " Begin " & vbCrLf _
                    & "  For R1 in C1 Loop" & vbCrLf _
                    & "   Update SOTORDR1 Set ORDR_STATUS = " & vbCrLf _
                    & "    (CASE WHEN NVL(R1.OPEN,0) <> 0 THEN 'O' ELSE " & vbCrLf _
                    & "    (CASE WHEN NVL(R1.PICK,0) <> 0 THEN 'P' ELSE " & vbCrLf _
                    & "    (CASE WHEN NVL(R1.SHIP,0) <> 0 THEN 'F' ELSE 'C' END) END) END)" & vbCrLf _
                ...
                ORDR_GROUP_NOs = O_ORDR_GROUP_NOs

            ElseIf optCancelMode.Value = "G" Then
                ASCMAIN1.Progress("Now Retracting Commitments")
                sqlORDR_GROUP_NOs = ""

                For Each row As DataRow In dst.Tables("SOTCANCY").Select("ORDR_QTY_OPEN <> ORIG_QTY_OPEN", "ORDR_NO, CUST_STORE_NO")
                    Dim cancel As String = row.Item("CANCEL")
                    ...
                    & "   where SOTORDR2.ORDR_NO = SOTORDR1.ORDR_NO" & vbCrLf _
                    & "   and SOTORDR1.ORDR_GROUP_NO = '" & row.Item("ORDR_GROUP_NO") & "'" & vbCrLf _
                    & "   and SOTORDR2.STYLE_CODE = '" & row.Item("STYLE_CODE") & "'" & vbCrLf _
                    & "   and SOTORDR2.COLOR_CODE = '" & row.Item("COLOR_CODE") & "'" & vbCrLf _
                    & "   group by SOTORDR1.WHSE_CODE, SOTORDR2.STYLE_CODE, SOTORDR2.COLOR_CODE;" & vbCrLf _
                    & " Begin " & vbCrLf _
                    & "  For R1 in C1 Loop" & vbCrLf _
                    & "   Update ICTSTAT2 Set WHSE_QTY_OPEN = NVL(WHSE_QTY_OPEN,0) -1 * NVL(R1.OPEN,0)" & vbCrLf _
                    & "    where WHSE_CODE = R1.WHSE_CODE and STYLE_CODE = R1.STYLE_CODE and COLOR_CODE = R1.COLOR_CODE;" & vbCrLf _
                    & "   If SQL%NOTFOUND Then" & vbCrLf _
                    & "     Insert into ICTSTAT2 (WHSE_CODE, STYLE_CODE, COLOR_CODE, WHSE_QTY_OPEN) Values (R1.WHSE_CODE, R1.STYLE_CODE, R1.COLOR_CODE, -1 * NVL(R1.OPEN,0));" & vbCrLf _
                    & "   End If;" & vbCrLf _
                    & "  End Loop;" & vbCrLf _
                    & " End;" & vbCrLf _
                    & "End;"
                    ASCDATA1.ExecuteSQL(updateSql)

                    Dim sqlw1 As String = " where ORDR_NO in (Select ORDR_NO from SOTORDR1 where ORDR_GROUP_NO = '" & row.Item("ORDR_GROUP_NO") & "')" & vbCrLf _
                    & "   and SOTORDR2.STYLE_CODE = '" & row.Item("STYLE_CODE") & "'" & vbCrLf _
                    & "   and SOTORDR2.COLOR_CODE = '" & row.Item("COLOR_CODE") & "'"
                    If cancel = "1" Then
                        ASCDATA1.ExecuteSQL("Update SOTORDR2 Set ORDR_QTY_CANC = NVL(ORDR_QTY,0) -  NVL(ORDR_QTY_SHIP,0) -  NVL(ORDR_QTY_PICK,0), ORDR_QTY_OPEN = 0" & sqlw1)

                    Else
                        ASCDATA1.ExecuteSQL("Update SOTORDR2 Set ORDR_QTY_OPEN = NVL(ORDR_QTY,0) -  NVL(ORDR_QTY_SHIP,0) -  NVL(ORDR_QTY_PICK,0), ORDR_QTY_CANC = 0" & sqlw1)
                    End If
                    ...
                    ASCDATA1.ExecuteSQL(Replace(updateSql, "-1 * NVL(R1.OPEN,0)", "+1 * NVL(R1.OPEN,0)"))

                    ORDR_GROUP_NO = row.Item("ORDR_GROUP_NO")
                ...
                ASCMAIN1.sql = "" _
                    ...
                    & "   from SOTORDR2, SOTORDR1" & vbCrLf _
                    & "   where SOTORDR2.ORDR_NO = SOTORDR1.ORDR_NO" & vbCrLf _
                    & "   and SOTORDR1.ORDR_GROUP_NO in (" & Mid(sqlORDR_GROUP_NOs, 2) & ")" & vbCrLf _
                    & "   group by SOTORDR2.ORDR_NO;" & vbCrLf _
                    & " Begin " & vbCrLf _
                    & "  For R1 in C1 Loop" & vbCrLf _
                    & "   Update SOTORDR1 Set ORDR_STATUS = " & vbCrLf _
            ...
            Else
                ...
                For Each row As DataRow In dst.Tables("SOTCANCY").Select("ORDR_QTY_OPEN <> ORIG_QTY_OPEN", "ORDR_NO, CUST_STORE_NO")
                    ...
                    End If
                    Ordr2Select = String.Format("ORDR_NO = '{0}' and STYLE_CODE = '{1}' and COLOR_CODE = '{2}' and ISNULL(CUST_SIZE_CODE,'N_A') = '{3}' and ISNULL(CUST_UPC,'N_A') = '{4}'", ORDR_NO, row.Item("STYLE_CODE"), row.Item("COLOR_CODE"), row.Item("CUST_SIZE_CODE"), row.Item("CUST_UPC"))
                    Dim ROWS() As DataRow = dst.Tables("SOTORDR2").Select(Ordr2Select)
                    If ROWS.Count <> 1 Then
                        MsgBox("Error Consolidating Order details, please call Rick", MsgBoxStyle.Critical, "Error in Cancelation")
                        Throw New Exception("Error Consolidating Order details, Please take a screenshot and call Rick")
                    End If
                    ...
                    For Each rowSOTORDR2 In ROWS
                        ...
                        ElseIf rowSOTORDR2.Item("ORDR_QTY_PICK") <> 0 Then
                            Status = "P"
                        ElseIf rowSOTORDR2.Item("ORDR_QTY_SHIP") <> 0 Then
                            Status = "F"
                        Else
                            Status = "C"
                        End If
...
#Region "Popup_Menus"
    ...
    Overrides Sub Load_Popup_Menus()
        Load_Popup_Menu(grdICTSTYLX, "SBBBBB", "Show Filter", "Cancel Style", "Cancel Selected Styles", "Restore Style", "Restore Selected Styles", "Style Status Inquiry")
    ...
    Public Overrides Sub tlb_BeforeToolDropdown(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinToolbars.BeforeToolDropdownEventArgs)
        ...
        Select Case e.SourceControl.Name
            ...
            Case "grdICTSTYLX"
                tlb_pop.Tools("Cancel Style").SharedProps.Visible = grd.Selected.Rows.Count() = 0
                tlb_pop.Tools("Cancel Selected Styles").SharedProps.Visible = grd.Selected.Rows.Count() > 0
                tlb_pop.Tools("Restore Style").SharedProps.Visible = grd.Selected.Rows.Count() = 0
                tlb_pop.Tools("Restore Selected Styles").SharedProps.Visible = grd.Selected.Rows.Count() > 0
                If grd.Selected.Rows.Count() > 0 Then
                    tlb_pop.Tools("Restore Selected Styles").SharedProps.Caption = "Restore " & grd.Selected.Rows.Count() & " Selected Styles"
                    tlb_pop.Tools("Cancel Selected Styles").SharedProps.Caption = "Cancel " & grd.Selected.Rows.Count() & " Selected Styles"
                End If
            Case "grdSOTORDR0"

            Case "grdSOTCANCY"
                tlb_pop.Tools("Show GroupBox").SharedProps.Visible = optCancelMode.Value = "S"
                For Each BTN_NAME As String In New String() {"Clear Qtys for", "Restore Qtys for"}
                    tlb_btn = DirectCast(tlb_pop.Tools(BTN_NAME), UltraWinToolbars.ButtonTool)
                    If grdSOTCANCY.Selected.Rows.Count <> 1 Then
                        tlb_btn.SharedProps.Visible = False
                    Else
                        If grdSOTCANCY.Selected.Rows(0).IsGroupByRow Then
                            Dim gbrow As UltraWinGrid.UltraGridGroupByRow = DirectCast(grdSOTCANCY.Selected.Rows(0), UltraWinGrid.UltraGridGroupByRow)
                            tlb_btn.SharedProps.Caption = BTN_NAME & " " & gbrow.Description
                            tlb_btn.SharedProps.Visible = True
                        ElseIf grdSOTCANCY.Selected.Rows(0).IsDataRow Then
                            tlb_btn.SharedProps.Caption = BTN_NAME & " Order " & grdSOTCANCY.Selected.Rows(0).Cells("ORDR_NO").Text
                            tlb_btn.SharedProps.Visible = True
                        Else
                            tlb_btn.SharedProps.Visible = False
                        End If
        ...
        Select Case e.Tool.Key
            ...
            Case "Select Selected", "De-Select Selected"
                For Each grow As UltraWinGrid.UltraGridRow In grd.Selected.Rows
                    ...
                    grow.Update()
                Next
            Case "Clear Qtys", "Clear Qtys for", "Cancel Style", "Cancel Selected Styles"
                ...
                If e.Tool.Key = "Cancel Style" Then

                    For Each grow As UltraWinGrid.UltraGridRow In grdSOTCANCY.Rows
                        If grow.IsDataRow Then
                            'grdSOTCANCY.ActiveCell = grow.Cells("ORDR_QTY_OPEN")
                            If optCancelMode.Value = "S" Then
                                grow.Cells("ORDR_QTY_OPEN").Value = 0
                            Else
                                grow.Cells("CANCEL").Value = "1"
                            End If
                            grow.Update()
                        End If
                    Next
                ElseIf e.Tool.Key = "Cancel Selected Styles" Then
                    For Each sgrow As UltraWinGrid.UltraGridRow In grdICTSTYLX.Selected.Rows
                        If sgrow.IsFilteredOut = False Then
                            Dim STYLE_CODE As String = sgrow.Cells("STYLE_CODE").Value
                            Dim COLOR_CODE As String = sgrow.Cells("COLOR_CODE").Value
                            Dim CUST_SIZE_CODE As String = sgrow.Cells("CUST_SIZE_CODE").Value & ""
                            Dim CUST_UPC As String = sgrow.Cells("CUST_UPC").Value & ""
                            Highlight_Style(STYLE_CODE, COLOR_CODE)

                            Dim SQLW As String = "STYLE_CODE = '" & STYLE_CODE & "' and COLOR_CODE = '" & COLOR_CODE & "' and CUST_SIZE_CODE = '" & CUST_SIZE_CODE & "' and CUST_UPC = '" & CUST_UPC & "'"
                            If optStatus.Value <> "ALL" Then
                                SQLW = SQLW & " and ORDR_STATUS = 'O'"
                            End If
                            Dim dvw As DataView
                            dvw = DirectCast(grdSOTCANCY.DataSource, DataTable).DefaultView
                            dvw.RowFilter = SQLW
                            For Each grow As UltraWinGrid.UltraGridRow In grdSOTCANCY.Rows
                                If grow.IsDataRow Then
                                    'grdSOTCANCY.ActiveCell = grow.Cells("ORDR_QTY_OPEN")
                    ...
                    Next
                    Set_SOTCANCY()
                ElseIf e.Tool.Key = "Clear Qtys for" Then

                    For Each grow As UltraWinGrid.UltraGridRow In grdSOTCANCY.Selected.Rows
                        ...
                        Else
                            If grow.IsGroupByRow Then
                                ...
                                For Each grow2 As UltraWinGrid.UltraGridRow In gbrow.Rows
                                    grow2.Cells("ORDR_QTY_OPEN").Value = 0
                                    grow2.Update()
                                Next
                            End If
                        End If
            ...
            Case "Restore Qtys", "Restore Qtys for", "Restore Style", "Restore Selected Styles"
                ...
                'For Each row As DataRow In DirectCast(grdSOTCANCY.DataSource, DataTable).DefaultView.ToTable.Select("")

                'Next
                'For Each row As DataRow In dst.Tables("SOTCANCY").Select("STYLE_CODE = '" & "" & "' and COLOR_CODE ")
                ...
                ASCMAIN1.Progress("Now Restoring")
                If e.Tool.Key = "Restore Style" Then
                    ...
                    For Each grow As UltraWinGrid.UltraGridRow In grdSOTCANCY.Rows
                        If grow.IsDataRow Then
                            ...
                            Else
                                grow.Cells("CANCEL").Value = "0"
                            End If
                            grow.Update()
                        End If
                    Next
                ElseIf e.Tool.Key = "Restore Selected Styles" Then
                    For Each sgrow As UltraWinGrid.UltraGridRow In grdICTSTYLX.Selected.Rows
                        If sgrow.IsFilteredOut = False Then
                            Dim STYLE_CODE As String = sgrow.Cells("STYLE_CODE").Value
                            Dim COLOR_CODE As String = sgrow.Cells("COLOR_CODE").Value
                    ...
                    Next
                    Set_SOTCANCY()
                ElseIf e.Tool.Key = "Restore Qtys for" Then
                    ...
                    For Each grow As UltraWinGrid.UltraGridRow In grdSOTCANCY.Selected.Rows
                        ...
                        Else
                            If grow.IsGroupByRow Then
                                Dim gbrow As UltraWinGrid.UltraGridGroupByRow = DirectCast(grow, UltraWinGrid.UltraGridGroupByRow)
                                For Each grow2 As UltraWinGrid.UltraGridRow In gbrow.Rows
                                    grow2.Cells("ORDR_QTY_OPEN").Value += grow2.Cells("ORDR_QTY_CANC").Value
                                    grow2.Update()
                                Next
                            End If
                        End If
...
#End Region
    ...
    Private Sub Set_SOTCANCY()
        If Not grdICTSTYLX.ActiveRow Is Nothing AndAlso grdICTSTYLX.ActiveRow.IsDataRow Then
            Dim STYLE_CODE As String = grdICTSTYLX.ActiveRow.Cells("STYLE_CODE").Value
            Dim COLOR_CODE As String = grdICTSTYLX.ActiveRow.Cells("COLOR_CODE").Value
            Dim CUST_SIZE_CODE As String = grdICTSTYLX.ActiveRow.Cells("CUST_SIZE_CODE").Value & ""
            Dim CUST_UPC As String = grdICTSTYLX.ActiveRow.Cells("CUST_UPC").Value & ""
            Highlight_Style(STYLE_CODE, COLOR_CODE)
            ...
            If optCancelMode.Value = "S" Then
                grdSOTCANCY.Text = "Order Qtys by Store for " & STYLE_CODE & "-" & COLOR_CODE & "-" & CUST_SIZE_CODE & "-" & CUST_UPC
            Else
                grdSOTCANCY.Text = "Order Qtys by Order Group for " & STYLE_CODE & "-" & COLOR_CODE & "-" & CUST_SIZE_CODE & "-" & CUST_UPC
            End If
...
#Region "grdICTSTYLX"
    ...
    Private Sub grdICTSTYLX_AfterRowUpdate(sender As Object, e As Infragistics.Win.UltraWinGrid.RowEventArgs) Handles grdICTSTYLX.AfterRowUpdate
        If SupressSummary Then Exit Sub
...
#End Region
    ...
    Sub Set_SOTORDR0()
        ...
        For Each row As DataRow In dst.Tables("SOTORDR0").Select("ORDR_QTY <> (ORDR_QTY_OPEN + ORDR_QTY_CANC)")
            row("SELECTED") = "0"
        Next
    ...
    Sub Create_Temp_Tables(initialize As Boolean)
        ...
        ASCMAIN1.sql = "" _
            & "Select SOTORDR1.CUST_STORE_NO, SOTORDR1.ORDR_NO, SOTORDR1.ORDR_CUST_PO, SOTORDR1.ORDR_GROUP_NO" & vbCrLf _
            & ", SOTORDR2.STYLE_CODE, SOTORDR2.COLOR_CODE, NVL(SOTORDR2.CUST_SIZE_CODE,'N_A') CUST_SIZE_CODE " & vbCrLf _
            & ", NVL(SOTORDR2.CUST_UPC,'N_A') CUST_UPC, SOTORDR1.WHSE_CODE, SOTORDR1.ORDR_STATUS" & vbCrLf _
        ...
        Else
            ...
            'If optCancelMode.Value = "G" Then
            '    ASCMAIN1.sql = Replace(ASCMAIN1.sql, "NVL(SOTORDR2.CUST_UPC,'N_A')", "'N_A'")
            '    ASCMAIN1.sql = Replace(ASCMAIN1.sql, "NVL(SOTORDR2.CUST_SIZE_CODE,'N_A')", "'N_A'")
            'End If
    ...
    Sub Highlight_Style(STYLE_CODE As String, COLOR_CODE As String)
        ...
        For Each STYLE_COLOR As String In iSC.Keys
            ...
            If STYLE_COLOR = STYLE_CODE & "-" & COLOR_CODE Then
    ...
    Sub Load_Qtys(Optional initial_load As Boolean = False)
        ...
        For Each rowSOTCANCX As DataRow In dst.Tables("SOTCANCX").Select("")
            ...
            For Each rowSOTCANCY As DataRow In dst.Tables("SOTCANCY").Select(SQLW)
                Dim STYLE_CODE As String = rowSOTCANCY.Item("STYLE_CODE")
                Dim COLOR_CODE As String = rowSOTCANCY.Item("COLOR_CODE")
                Dim CUST_SIZE_CODE As String = rowSOTCANCY.Item("CUST_SIZE_CODE") & ""
                Dim CUST_UPC As String = rowSOTCANCY.Item("CUST_UPC") & ""
                Dim i As Integer = iSC(STYLE_CODE & "-" & COLOR_CODE & " - " & CUST_SIZE_CODE & " - " & CUST_UPC)
    ...
    Private Sub grdSOTCANCY_AfterRowActivate(sender As Object, e As EventArgs) Handles grdSOTCANCY.AfterRowActivate
        If Not grdSOTCANCY.ActiveRow.IsDataRow Then
            Exit Sub
        End If
        If grdSOTCANCY.ActiveRow.Cells("ORDR_STATUS").Value = "O" And optCancelMode.Value = "S" Then
