...
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
        ...
        With grdSOTRSRV2.DisplayLayout.Bands(0)
            For Each gcol As UltraWinGrid.UltraGridColumn In .Columns
                ...
                If New String() {"RSRV_QTY", "RSRV_QTY_OPEN", "RSRV_QTY_ALLO", "RSRV_QTY_USED", "RSRV_QTY_CANC"}.Contains(gcol.Key) Then
                    gcol.Header.Appearance.BackColor = Drawing.Color.LightBlue
                ...
                ElseIf New String() {"RSRV_AMT", "RSRV_AMT_OPEN", "RSRV_AMT_ALLO", "RSRV_AMT_USED", "RSRV_AMT_CANC"}.Contains(gcol.Key) Then
                    gcol.Header.Appearance.BackColor = Drawing.Color.LightGreen
                    gcol.Header.Appearance.BackGradientStyle = GradientStyle.GlassBottom20
                ElseIf New String() {"STYLE_CODE", "STYLE_DESC", "COLOR_CODE", "COLOR_DESC"}.Contains(gcol.Key) Then
                    gcol.Header.Appearance.BackColor = Drawing.Color.Pink
                    gcol.Header.Appearance.BackGradientStyle = GradientStyle.GlassBottom20
                Else
        ...
        Select Case eItemKey
            ...
            Case "New"
                ...
                If EMsg = "" Then
                    ...
                    If SREP_CODE <> "" Then rowSOTSREP1 = LookUp("SOTSREP1", rowARTCUST1.Item("SREP_CODE") & "")
    ...
    Overrides Sub Mode_Settings(ByVal tf As Boolean, Optional ByVal MODE_description As String = "")
        ...
        With UltraExplorerBar1
            ...
            .Groups("Screen Control").Items("Done").Visible = (EntryMode = "L" And ScreenMode)
            .Groups("Screen Control").Items("Print").Visible = False ' ScreenMode
            .Groups("Screen Control").Items("Update").Visible = (Not (EntryMode = "L") Or Not ScreenMode)
            .Groups("Screen Control").Items("Delete").Visible = (EntryMode = "E")
            .Groups("Screen Control").Items("Cancel").Visible = (Not (EntryMode = "L") Or Not ScreenMode)
        ...
        splICTCOLRM.Panel2Collapsed = True

        grdSOTRSRVX.Visible = Not tf

        Set_Read_Only(UltraGroupBox1, ScreenMode)
        Set_Read_Only_for_ctl(Absx1.txtFor("ORDR_CUST_PO"), Not (EntryMode = "E" Or EntryMode = "N"))
    ...
    Sub Load_Record()
        ...
        If EntryMode = "N" Then
            ...
            With rowSOTRSRV1
                ...
                Dim WHSE_CODE As String = ROWs("SOTPARM1").Item("SO_PARM_DEF_ALLO_WHSE")
                If rowARTCUST1.Item("DEF_WHSE_CODE") & "" <> "" Then WHSE_CODE = rowARTCUST1.Item("DEF_WHSE_CODE")
...
#Region "Popup_Menus"
    ...
    Overrides Sub Load_Popup_Menus()
        ...
        Load_Popup_Menu(grdSOTRSRV2, "BS", "Style Status Inquiry", "Style Multi-Color")
    End Sub
    ...
    Public Overrides Sub tlb_BeforeToolDropdown(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinToolbars.BeforeToolDropdownEventArgs)
        ...
        Select Case grd.Name
            Case "grdSOTRSRV2"
                tlb_sbt = DirectCast(tlb_pop.Tools("Style Multi-Color"), UltraWinToolbars.StateButtonTool)
                tlb_sbt.SharedProps.Visible = (EntryMode = "E" Or EntryMode = "N")
                If (EntryMode = "E" Or EntryMode = "N") Then
                    tlb_sbt.Checked = Not splICTCOLRM.Panel2Collapsed
                End If
        End Select
    ...
    Public Overrides Sub tlb_ToolClick(ByVal sender As System.Object, ByVal e As Infragistics.Win.UltraWinToolbars.ToolClickEventArgs)
        ...
        Select Case e.Tool.Key
            Case "Style Multi-Color"
                ...
                splICTCOLRM.Panel2Collapsed = Not tlb_sbt.Checked
...
#End Region
    ...
    Sub Cancel_Order()
        ...
        If EntryMode = "E" Then
            ...
            EMsg = "Order " & RSRV_NO & " has been Cancelled"
        End If
    ...
    Sub Cancel_Order_1(RSRV_NO As String)
        ...
        ASCMAIN1.sql = "" _
            & "Begin " _
            & " Declare Cursor C1 as Select * from SOTRSRV2 where RSRV_NO = '" & RSRV_NO & "' for Update;" _
            & " Begin " _
            & "  For R1 in C1 Loop" _
            & "   Update SOTRSRV2" _
            & "    Set RSRV_QTY_CANC = NVL(RSRV_QTY_CANC,0) + NVL(R1.RSRV_QTY_OPEN,0)" _
            & "      , RSRV_QTY_OPEN = 0, RSRV_STATUS = 'C'" _
        ...
        ASCMAIN1.sql = "Update SOTRSRV1 Set RSRV_STATUS = :PARM1" _
            & " where RSRV_NO = :PARM2"
        ASCDATA1.ExecuteSQL(ASCMAIN1.sql, "VVVV", New Object() {"C", RSRV_NO})
    End Sub
    ...
    Sub Delete_Order_1(RSRV_NO As String)
        ...
        ASCMAIN1.sql = "" _
            ...
            & "  For R1 in C1 Loop" _
            & "   Update SOTRSRV2" _
            & "    Set RSRV_QTY_OPEN = 0, RSRV_STATUS = '" & "D" & "'" _
            & "    where Current of C1;" _
            & "  End Loop;" _
            & " End;" _
            & "End;"
            ...
            Select Case COLUMN_NAME
                ...
                Case "RSRV_QTY"
                    ...
                    If Val(.Cells("RSRV_QTY").Value & "") < 0 Then
                        MsgBox("Qty May Not be Negative", vbOKOnly, "Invalid Order Quantity")
                        Cancel = True
                    End If
            End Select
        End With
    ...
    Function Validate_Style(STYLE_CODE_z As String) As String
        ...
        If E <> "" And grdSOTRSRV2.ActiveRow.IsAddRow Then
...
#Region "grdSOTRSRV2"
    ...
    Private Sub grdSOTRSRV2_BeforeCellUpdate(sender As Object, e As Infragistics.Win.UltraWinGrid.BeforeCellUpdateEventArgs) Handles grdSOTRSRV2.BeforeCellUpdate
        ...
        '        .Value = CStr(e.Cell.Value & "").ToUpper
        '    End If
        ...
        '                e.Cancel = True
        '            End If
        '            'This check removed by WR on 3/10/05 to allow mixing of styles on divisions.
        '            '                If absx1.txtfor("SALES_DIVISION_CODE")).Text <> "" And _
        '            '                   absx1.txtfor("SALES_DIVISION_CODE")).Text <> SALES_DIVISION_CODE And grdSOWRSRV2.Rows > 1 Then
        '            '                    MsgBox "Cannot Mix Divisions on a Single Order", vbOKOnly, "Cannot Perform Requested Action"
        '            '                    Cancel = True
        '            '                    Exit Sub
        '            '                End If
        ...
        '            End If
        '            If Val(.Value & "") < 0 Then
        '                .Value = System.Math.Abs(Val(.Value & ""))
        '            End If

        '        Case "RSRV_QTY_OPEN"
        '            If e.Cell.Value & "" = "" _
        '            Or Val(e.Cell.Value & "") < 0 _
        '            Then
        '                e.Cell.Value = "0"
        '            End If
        '            Dim q As Int64 = Val(e.Cell.Row.Cells("RSRV_QTY_USED").Value & "") _
        '                           + Val(e.Cell.Row.Cells("RSRV_QTY_CANC").Value & "")
        '            If Val(e.Cell.Value & "") > Val(e.Cell.Row.Cells("RSRV_QTY").Value & "") - q Then
        '                .Value = Val(e.Cell.Row.Cells("RSRV_QTY").Value & "") - q
        '            End If
        '    End Select
        'End With
    End Sub
    ...
    Private Sub grdSOTRSRV2_BeforeExitEditMode(sender As Object, e As Infragistics.Win.UltraWinGrid.BeforeExitEditModeEventArgs) Handles grdSOTRSRV2.BeforeExitEditMode
        ...
        'If grdSOTRSRV2.ActiveCell Is Nothing OrElse grdSOTRSRV2.ActiveCell.Value Is Nothing OrElse grdSOTRSRV2.ActiveCell.Value & "" = "" Then Exit Sub
        'With grdSOTRSRV2.ActiveCell

        '    If .Column.Key = "STYLE_CODE" Or .Column.Key = "COLOR_CODE" Then
        '        .Value = CStr(.Value & "").ToUpper
        '    End If

        '    Select Case .Column.Key
        '        Case "STYLE_CODE"
        '            Dim STYLE_CODE As String = Validate_Style(.Value & "")
        '            If STYLE_CODE = "" Then
        '                e.Cancel = True
        '            End If
        '            'This check removed by WR on 3/10/05 to allow mixing of styles on divisions.
        ...
        '            Dim q As Int64 = Val(.Row.Cells("RSRV_QTY_USED").Value & "") _
        '                           + Val(.Row.Cells("RSRV_QTY_CANC").Value & "")

        '            If Val(.Value & "") > Val(.Row.Cells("RSRV_QTY").Value & "") - q Then
        '                .Value = Val(.Row.Cells("RSRV_QTY").Value & "") - q
        '            End If
        '    End Select
        'End With
...
#End Region
    ...
    Private Sub cmdAdd2Order_Click(sender As System.Object, e As System.EventArgs) Handles cmdAdd2Order.Click
        If dst.Tables("ICTCOLRM").Select("ISNULL(QTY,0)<>0").Length = 0 Then
            MsgBox("No Qty's Entered", MsgBoxStyle.OkOnly, "Cannot Add to Order")
            Exit Sub
        End If
        For Each rowICTCOLRM As DataRow In dst.Tables("ICTCOLRM").Select("ISNULL(QTY,0)<>0", "COLOR_CODE")
            grdSOTRSRV2.DisplayLayout.Bands(0).AddNew()
            With grdSOTRSRV2.ActiveRow
                .Cells("STYLE_CODE").Value = Absx1.txtFor("STYLE_CODE").Text
