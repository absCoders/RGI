...
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        With dst
            ...
            SQLB.Length = 0
            SQLB.AppendLine("SELECT C1.STYLE_CODE, S1.STYLE_DESC, S1.CARTON_PACK_QTY,S1.INNER_PACK_QTY, S1.STYLE_UOM,")
            SQLB.AppendLine("LISTAGG(COLOR_CODE, ',')")
            SQLB.AppendLine("WITHIN GROUP")
            SQLB.AppendLine("(ORDER BY C1.STYLE_CODE, S1.STYLE_DESC, S1.CARTON_PACK_QTY, S1.INNER_PACK_QTY, S1.STYLE_UOM) COLOR_CODE")
            SQLB.AppendLine("FROM ICTSTYL1 S1, ICTSTYC1 C1")
            SQLB.AppendLine("WHERE S1.STYLE_CODE = C1.STYLE_CODE")
            SQLB.AppendLine("AND S1.STYLE_CODE IN")
            SQLB.AppendLine("(")
            SQLB.AppendLine("select STYLE_CODE from WHRPRIC1_TEMP")
            SQLB.AppendLine(")")
            SQLB.AppendLine("GROUP BY C1.STYLE_CODE, S1.STYLE_DESC, S1.CARTON_PACK_QTY, S1.INNER_PACK_QTY, S1.STYLE_UOM")
            SQLB.AppendLine("ORDER BY C1.STYLE_CODE")
            ASCMAIN1.sql = SQLB.ToString
            sqlSOFPRIC1 = ASCMAIN1.sql
...
#End Region
    ...
    Private Sub Fill_Pricing()
        Dim rowARTCUST1 As DataRow = Nothing
        For Each rowSOTPRIC1 As DataRow In dst.Tables("SOTPRIC1").Select()
            Dim Discounts As List(Of DISCOUNTS)
            Discounts = SOCMAIN2.Price_Discounts(Me, "", rowARTCUST1, rowSOTPRIC1.Item("STYLE_CODE").ToString & "", False)
            rowSOTPRIC1.Item("PB1") = Format$(Discounts(3).DISCOUNT_PRICE, "###,##0.00")
            rowSOTPRIC1.Item("PB2") = Format$(Discounts(2).DISCOUNT_PRICE, "###,##0.00")
            rowSOTPRIC1.Item("PB3") = Format$(Discounts(1).DISCOUNT_PRICE, "###,##0.00")
            rowSOTPRIC1.Item("PB4") = Format$(Discounts(0).DISCOUNT_PRICE, "###,##0.00")
        Next
    End Sub
