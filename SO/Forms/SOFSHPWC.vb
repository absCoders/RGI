...
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        With dst
            ...
            SQ.AppendLine(" MAX(IX.INV_DATE) AS INV_DATE,")
            SQ.AppendLine(" SUM(O2.ORDR_QTY_ORIG) AS ORDR_QTY_ORIG,")
            SQ.AppendLine(" SUM(O2.ORDR_QTY) AS ORDR_QTY,")
            SQ.AppendLine(" SUM(O2.ORDR_QTY_CANC) AS ORDR_QTY_CANC,")
            SQ.AppendLine(" SUM(O2.ORDR_QTY_ALLO) AS ORDR_QTY_ALLO,")
            SQ.AppendLine(" SUM((O2.ORDR_QTY_CANC - (O2.ORDR_QTY - O2.ORDR_QTY_ALLO))) AS ORDR_QTY_CANC_WHSE,")
            SQ.AppendLine(" SUM(O2.ORDR_QTY_SHIP) AS ORDR_QTY_SHIP,")
            SQ.AppendLine(" SUM(IX.INVOICED) AS INVOICED")
            SQ.AppendLine(" FROM SOTORDR1 O1, SOTORDR2 O2, ARTCUST1 C1,")
            SQ.AppendLine(" (")
            SQ.AppendLine(" SELECT")
            SQ.AppendLine(" I1.ORDR_NO,")
            SQ.AppendLine(" I2.INV_LNO,")
            SQ.AppendLine(" I1.INV_NO,")
            SQ.AppendLine(" I1.INV_DATE,")
            SQ.AppendLine(" SUM(NVL(I2.ORDR_QTY_SHIP,0)) AS INVOICED")
            SQ.AppendLine(" FROM SOTINVH1 I1, SOTINVH2 I2, SOTORDR1 O1")
            SQ.AppendLine(" WHERE I1.ORDR_NO = O1.ORDR_NO")
            SQ.AppendLine(" AND I1.INV_TYPE = I2.INV_TYPE")
            ...
            SQ.AppendLine(" AND O2.ORDR_NO = IX.ORDR_NO (+)")
            SQ.AppendLine(" AND O2.ORDR_LNO = IX.INV_LNO (+)")
            ...
            .Tables("SOTSHPWC").Columns.Add("NET_POS", GetType(System.Int64))

            SQ.Length = 0
            SQ.AppendLine("SELECT")
            SQ.AppendLine("S2.STYLE_CODE,")
            SQ.AppendLine("S2.COLOR_CODE,")
            SQ.AppendLine("NVL(S2.WHSE_QTY_ON_HAND,0) AS WHSE_QTY_ON_HAND,")
            SQ.AppendLine("NVL(S2.WHSE_QTY_PICK,0) AS WHSE_QTY_PICK,")
            SQ.AppendLine("(NVL(S2.WHSE_QTY_ON_HAND,0) - NVL(S2.WHSE_QTY_PICK,0)) AS OTS,")
            SQ.AppendLine("NVL(S2.WHSE_QTY_TRAN,0) AS WHSE_QTY_TRAN,")
            SQ.AppendLine("NVL(S2.WHSE_QTY_ON_ORDER,0) AS WHSE_QTY_ON_ORDER,")
            SQ.AppendLine("NVL(S2.WHSE_QTY_OPEN,0) AS WHSE_QTY_OPEN,")
            SQ.AppendLine("(NVL(S2.WHSE_QTY_ON_HAND,0) - NVL(S2.WHSE_QTY_PICK,0) + NVL(S2.WHSE_QTY_TRAN,0) + NVL(S2.WHSE_QTY_ON_ORDER,0) - NVL(S2.WHSE_QTY_OPEN,0)) AS NET_POS")
            SQ.AppendLine("FROM ICTSTAT2 S2")
            SQ.AppendLine("WHERE (STYLE_CODE, COLOR_CODE ) IN")
            SQ.AppendLine("(SELECT")
            SQ.AppendLine("O2.STYLE_CODE,")
            SQ.AppendLine("O2.COLOR_CODE")
            SQ.AppendLine("FROM SOTORDR1 O1, SOTORDR2 O2")
            SQ.AppendLine("WHERE O1.ORDR_NO = O2.ORDR_NO")
            SQ.AppendLine("AND O1.CUST_CODE = 'WALMART'")
            SQ.AppendLine("AND (O1.ORDR_DATE >= :PARM1 AND O1.ORDR_DATE <= :PARM2)")
            SQ.AppendLine("GROUP BY")
            ...
            SQ.AppendLine("O2.COLOR_CODE)")
            ASCMAIN1.sql = SQ.ToString
            Create_TDA(.Tables.Add, "ICTNTPOS", "**", 0, False, "DD", 0)
...
#Region "Form Controls"
    ...
    Private Sub btnFETCH_Click(sender As Object, e As EventArgs) Handles btnFETCH.Click
        ...
        If dteORDR_DATE_FROM.DateTime > dteORDR_DATE_TO.DateTime Then
            ...
            Exit Sub
        End If
        If DateDiff(DateInterval.Day, dteORDR_DATE_FROM.DateTime, dteORDR_DATE_TO.DateTime) > 31 Then
            MsgBox("You Can Not Request More Than 31 Days Of Data.", vbExclamation, "Too Much!!")
            Exit Sub
        End If
    ...
    Private Sub chkNetPos_CheckedChanged(sender As Object, e As EventArgs) Handles chkNetPos.CheckedChanged
        Me.Cursor = Cursors.WaitCursor
        If chkNetPos.Checked Then
            Dim FDate As Date = CDate(dteORDR_DATE_FROM.DateTime.Date.ToShortDateString)
            Dim TDate As String = CDate(dteORDR_DATE_TO.DateTime.Date.ToShortDateString)
            Fill_Records("ICTNTPOS", New String() {FDate, TDate})
            For Each rowSOTSHPWC As DataRow In dst.Tables("SOTSHPWC").Select()
                Dim NET_POS As Int64 = 0
                Dim STYLE_CODE As String = rowSOTSHPWC.Item("STYLE_CODE").ToString & String.Empty
                Dim COLOR_CODE As String = rowSOTSHPWC.Item("COLOR_CODE").ToString & String.Empty
                Dim flt As String = $"STYLE_CODE = '{STYLE_CODE}' AND COLOR_CODE = '{COLOR_CODE}'"
                Dim rowICTNTPOS As DataRow = dst.Tables.Item("ICTNTPOS").Select(flt).FirstOrDefault
                If Not IsNothing(rowICTNTPOS) Then
                    NET_POS = Val(rowICTNTPOS.Item("NET_POS").ToString & String.Empty)
                End If
                rowSOTSHPWC.Item("NET_POS") = NET_POS
            Next
            grdSOTSHPWC.DisplayLayout.Bands(0).Columns.Item("NET_POS").Hidden = False
        Else

            grdSOTSHPWC.DisplayLayout.Bands(0).Columns.Item("NET_POS").Hidden = True
        End If
        Me.Cursor = Cursors.Default
    End Sub
#End Region
