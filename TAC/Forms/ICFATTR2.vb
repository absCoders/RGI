...
Public Class ICFATTR2
    Dim STYLE_CLASS_CODE As String
    Dim WHSE_CODE As String
    Public STYLE_CODE As String
    Dim sqlcols As String = "ICTSTYL1.STYLE_CODE,ICTSTYL1.STYLE_STATUS,ICTSTYL1.STYLE_DESC,ICTSTYL1.INNER_PACK_QTY,ICTSTYL1.CARTON_PACK_QTY,ICTSTYL1.STYLE_UOM,ICTSTYL1.STYLE_PRICE,ICTSTYL1.STYLE_CLASS_CODE,ICTSTYL1.SIZE_CODE, ICTSTYL1.VEND_CODE, ICTSTYL1.CASE_CUBE"
    ...
    Dim FFME As ASFBASE1

    Private Sub ICFATTR2_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub Form_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        With dst
            ...
            With .Tables("ICTSTYL1")
                ...
                .Columns.Add("IMAGE_LOC", GetType(System.String))
        ...
        With grdICTSTYL1.DisplayLayout.Bands(0)
            ...
            For Each gcol As UltraWinGrid.UltraGridColumn In .Columns
                ...
                ElseIf New String() {"PBV1", "PBV2", "PBV3", "PBV4", "FEPRICE", "FDPRICE", "FEMPRICE", "FDMPRICE"}.Contains(gcol.Key) Then
                    ...
                    gcol.Format = "####,##0.00"
                    gcol.Hidden = True
                ElseIf New String() {"IMAGE_LOC"}.Contains(gcol.Key) Then
...
#Region "Popup_Menus"
    ...
    Overrides Sub Load_Popup_Menus()
        Load_Popup_Menu(grdICTSTYL1, "SSSSBBBBSS", "Show Filter", "Show GroupBox", "Show Pins", "Show All Status Qtys", "Select All", "De-Select All", "Select Selected", "De-Select Selected", "Calc Price Breaks", "Show Factory/Port")
    ...
    Overrides Sub tlb_ToolClick(ByVal sender As System.Object, ByVal e As Infragistics.Win.UltraWinToolbars.ToolClickEventArgs)
        ...
        Select Case e.Tool.Key
            ...
            Case "Calc Price Breaks"
                ...
                For Each COLUMN_NAME As String In New String() {"PBH1", "PBV1", "PBH2", "PBV2", "PBH3", "PBV3", "PBH4", "PBV4", "FEPRICE", "FDPRICE", "FEMPRICE", "FDMPRICE", "IMAGE_LOC"}
                    grdICTSTYL1.DisplayLayout.Bands(0).Columns(COLUMN_NAME).Hidden = Not tlb_sbt.Checked
                Next
                For Each rowICTSTYL1 As DataRow In dst.Tables("ICTSTYL1").Select("", "STYLE_CODE, COLOR_CODE")
                    ...
                    Dim FEFD As New FEFDPrice(FFME, rowICTSTYL1.Item("STYLE_CODE").ToString)
                    ...
                    Dim IMAGE_LOC As String = GetImageLocation(rowICTSTYL1.Item("STYLE_CODE").ToString, rowICTSTYL1.Item("COLOR_CODE").ToString)
                    If IMAGE_LOC.Length > 0 Then
                        rowICTSTYL1.Item("IMAGE_LOC") = IMAGES_FOLDER & IMAGE_LOC
                    End If
                Next
...
#Region "Form Control Events"
    ...
    Private Sub cmdFind_Click(sender As System.Object, e As System.EventArgs) Handles cmdFind.Click
        ...
        If rowICTWHSE1 Is Nothing Then
            MsgBox("You Must Select a valid Warehouse Code", MsgBoxStyle.OkOnly, "Cannot Find")
            Exit Sub
        End If

        Dim selectedAttributes() = dst.Tables("ICTATTR1_1").Select("ISNULL(SEL,'0') = '1'")
        If selectedAttributes.Length = 0 Then
            MsgBox("You Must Select (at minimum) one Attribute", MsgBoxStyle.OkOnly, "Cannot Find")
            Exit Sub
        End If
        ...
        ElseIf tabAttributes.SelectedTab.Key = "2" Then
            ...
            If LAST_ATTR = "Size" Or LAST_ATTR = "1" Then
                ...
                ASCMAIN1.sql = "Select Distinct ICTSTYL3.ATTR_CODE from ICTSTYL3,ICTSTYL1" _
                    ...
                    & IIf(ATTR_CODE_1s = "", "", "   and ICTSTYL3.STYLE_CODE in (Select STYLE_CODE from ICTSTYL3 where ATTR_CODE in (" & Mid(ATTR_CODE_1s, 2) & "))")

                ASCMAIN1.sql = "Select ATTR_CODE, ATTR_DESC from ICTATTR1 where ATTR_CODE in (" & ASCMAIN1.sql & ")"
    ...
    Private Sub btnExcel_Click(sender As System.Object, e As System.EventArgs) Handles btnExcel.Click
        ...
        If EMsg.Length > 0 Then
            MsgBox(EMsg, MsgBoxStyle.OkOnly, "Cannot Perform Requested Action")
            Exit Sub
        End If
...
#End Region
    ...
    Sub Find_Styles(Optional ByVal FetchFromOrder As Boolean = False, Optional AllDiscontinued As Boolean = False)
        Dim QD As String = ""
        ...
        If FetchFromOrder Then
            ...
            ASCMAIN1.sql = "SELECT X.*, NVL(Y.ATTR_CODE,'NONE') AS ATTR_CODE,ICTSTYC1.UPC_CODE,ICTSTYC1.STYLE_COLOR_STATUS, Z.COLOR_CODE, Z.ONH, Z.ONPO, Z.OPEN, Z.TRAN, Z.PICK, Z.COLOR_CODE_LONG AS LONG_COLOR, NVL(Z.COLOR_GROUP_CODE,'') AS COLOR_GROUP_CODE, NVL(ICTSTYC1.THEME_CODE,'') AS THEME_CODE from ICTSTYC1, (" & vbCrLf _
                & ASCMAIN1.sql _
                & ") X," & vbCrLf _
                & "(SELECT ICTSTYL3.STYLE_CODE, MIN (ATTR_CODE) ATTR_CODE from ICTSTYL3 " & IIf(ATTR_CODE_1s = "", "", " where ATTR_CODE in (" & Mid(ATTR_CODE_1s, 2) & ")") & " group by ICTSTYL3.STYLE_CODE) Y," & vbCrLf _
        ...
        btnEmail.Enabled = (grdICTSTYL1.Rows.Count > 0)
        btnExcel.Enabled = (grdICTSTYL1.Rows.Count > 0)
        btnZip.Enabled = (grdICTSTYL1.Rows.Count > 0)
    ...
    Sub Initialize_tabAttributes()
        ...
        For Each TABLE_NAME As String In New String() {"ICTATTR1_1", "ICTSIZE1", "ICTATTR1_1"}
        ...
        cmdReset.Enabled = tabAttributes.Visible
        txtATTR_CODE_1.Focus()
        ...
        For Each row As DataRow In dst.Tables("ICTSTYL1").Select("SEL = '1'", sby)
            ...
            If rowICTCLAS1 IsNot Nothing Then
                ...
                If rowICTDISC1 IsNot Nothing Then
                    ...
                    For I As Integer = 1 To 4
                        ...
                        If chkPBFE.Checked Then
                            ...
                            If Factor < -14 Then
                                Factor = 1
                            End If
                            Dim FEFD As New FEFDPrice(FFME, rowICTSTYL1.Item("STYLE_CODE").ToString, Factor)
                            If FEFD.ErrorMsg = "" Then
                                Select Case Pos
                                    Case Is = 4
                    ...
                    Else
                        XWS.Cells(R + 4, 10).VALUE = CDec(rowICTSTYL1.Item("STYLE_PRICE") & "")
            ...
            If rowICTSTDQ2 IsNot Nothing Then
                For I As Integer = 1 To 4
                    ...
                    If rowICTSTDQ2.Item(DC) & "" <> "" Then
                        ...
                        If I = 1 Then
                            If FilterAlloc(STYLE_CODE, COLOR_CODE, WHSE_CODE) = True Then
                                QTY_A = 0
                            Else
    ...
    Sub InsertPictureInRange(ByVal PictureFileName As String, _
                    ...
                    ByVal XWS As Microsoft.Office.Interop.Excel.Worksheet, _
        ...
        If STYLE_CODE <> "" Then
            XWS.Hyperlinks.Add(Anchor:=pp, Address:= _
...
Optional ByVal absolute As Boolean = False) As String
    ...
    Sub Send_Email(ByVal attachments As Dictionary(Of String, String))
        Using frmTAFSEND1 As New TAFSEND1(frmASFBASE1)
            With frmTAFSEND1
                .EMAIL_KEY = "STY"
                ...
                Email_From = IIf(rowASTUSER1_EMAIL_FROM.Item("USER_EMAIL") & "" = "", _
                                 "donotreply" & "@" & ASCMAIN1.rowASTPARM1.Item("AS_PARM_DEFAULT_EMAIL_DOMAIN"), _
...
#Region "Grid Events"
    ...
    Private Sub grdICTSTYL1_BeforeRowFilterDropDown(sender As Object, e As Infragistics.Win.UltraWinGrid.BeforeRowFilterDropDownEventArgs) Handles grdICTSTYL1.BeforeRowFilterDropDown
...
#End Region
    ...
    Public Sub New(ByRef FF As ASFBASE1)
        FFME = FF
    ...
    Private Sub grdICTSTYL1_InitializeLayout(sender As System.Object, e As Infragistics.Win.UltraWinGrid.InitializeLayoutEventArgs) Handles grdICTSTYL1.InitializeLayout
